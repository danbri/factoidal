...
| `xml_runner` (`bin/linux-x86_64/xml_runner`, no `build-ocaml.sh` args — standalone `mktemp`-scratch compile, see `bin/xml-runner/README.md`) | W3C XML Conformance Test Suite (`xmlconf`, third_party/testing/xml/xmlconf/, jclark/sun/ibm/oasis/eduni/japanese collections). Assesses `Parser.XML.fst` + `XML.Wellformedness.fst` (informational NCName check only — its NCName production excludes `:`, so it's RDF/XML-domain, not generic XML). Baseline 2026-07-05 (wave 1): **1358 pass, 84 fail, 1143 skip (of 2585)**. `valid` bucket (labelled "wf-accept" — no DTD validation implemented) is 0/0/812: every `valid` test in this suite carries a DOCTYPE, so it's 100% `Skip "DOCTYPE/DTD not parsed"` — this project's parser has no DTD support at all. `invalid`/`error` (275 tests) are always skip by design. `not-wf` is 1358 pass/84 fail/56 skip (of 1498); 1165 of the passes are flagged "vacuous" (rejection forced by an unrelated DOCTYPE-not-supported gap, not necessarily the documented construct). 84 fails cluster into: non-character codepoints in content/char-refs (19), XML declaration grammar — ordering/required parts/value lexicons (18), namespace-spec violations (17, arguably out of `Parser.XML.fst`'s stated scope), epilog/trailing-content after the root not rejected (11), comment `--` rule (5), `]]>`-in-text (4), attribute value/uniqueness (4), PI/decl target-name casing (3), 3 singletons — full table + fix-candidate ordering in `bin/xml-runner/README.md`; raw log `.claude-runs/xml-runner-2026-07-05.log` | stdout |
