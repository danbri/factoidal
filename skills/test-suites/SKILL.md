...
| `jsonld_runner` | W3C JSON-LD 1.1 toRdf manifest (467 tests; not yet folded into generate-report — run directly). Score 2026-08-22 (re-measured, darwin-arm64): 467 pass, 0 fail, 0 skip (out of 467). The Lean 4 port's `lake exe l4jsonld-probe` (run from the repo root) scores the same 467 of 467 and additionally checks negative tests' error codes (103 of 106 match `expectErrorCode`). History: 2026-07-05 wave 9 was 399 pass, 57 fail, 11 skip | stdout |
