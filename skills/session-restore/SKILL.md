...
## Mid-session rollback to an older snapshot (2026-07-10)

Distinct from behind-origin above: there the container is merely
stale. Here the container **reverts to an EARLIER disk snapshot mid-
session**, so local `.git`, in-flight worktrees, and any local-only
commits vanish outright — this recurred ~5 times on 2026-07-10.
Detection: local `claude/main` tip is suddenly OLDER than
`origin/claude/main` (e.g. shows `7727c91` while origin is `62577c3+`);
`git worktree list` shows a different/older set than the agents you
were actually running; `git cat-file -t <recent-local-sha>` returns
"could not get object info". `git ls-remote origin
refs/heads/claude/main` reads the TRUE origin tip even when `git
fetch` times out.

Why it hurts: a subagent that finished but only committed **locally**
(never pushed) is unrecoverable after a rollback — on 2026-07-10 a
completed, verified MathML serializer commit was lost this exact way.

The discipline that survived it: **push every agent's branch the
instant it commits** — never leave a finished commit local-only. For
landings needing a rebuild (extract+compile of committed binaries),
split into two pushes: (1) a SOURCE-checkpoint branch immediately
after resolving the merge, BEFORE the long build; (2) rebuild, then
amend/add binaries and force-with-lease push. Only the rebuild output
is then rollback-fragile, never the merge work. Origin is the only
durable store (Iron Rule #9; the "repo is the only durable store" line
above).

Recovery after a rollback: `git ls-remote origin` to inventory
surviving branches; re-create worktrees from `origin/claude/main`;
agents reconstruct unpushed work from their own transcript context
onto the current origin tip — proven on 2026-07-10, when the GeoSPARQL
agent regenerated byte-identical output this way. Never rebuild on the
stale local base.
