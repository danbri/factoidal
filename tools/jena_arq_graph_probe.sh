...
GRAPH_ROOT="${1:-/tmp/jena/jena-arq/testing/DAWG-Final/graph}"
TMPDIR="${TMPDIR:-/tmp}"
LIMIT="${LIMIT:-0}"
...
if [[ ! -d "${GRAPH_ROOT}" ]]; then
  echo "Jena ARQ graph root not found at ${GRAPH_ROOT}" >&2
  exit 1
fi

manifest_rows() {
  python3 - "$GRAPH_ROOT/manifest.ttl" <<'PY'
import sys
...
for test in sorted(g.subjects(RDF.type, MF.QueryEvaluationTest), key=lambda t: str(t)):
    ...
    if query and result and str(result).endswith(".ttl"):
        rows.append((
...
expected_count() {
  local result_file="$1"
  grep -c 'rs:solution' "${result_file}" || true
}
...
while IFS=$'\t' read -r test_name query_file data_file result_file graph_files; do
  ...
  query_path="${GRAPH_ROOT}/${query_file}"
  result_path="${GRAPH_ROOT}/${result_file}"
  output_file="${TMPDIR}/factoidal_jena_graph_${query_file%.rq}.out"
  err_file="${TMPDIR}/factoidal_jena_graph_${query_file%.rq}.err"
  expected="$(expected_count "${result_path}")"
  name="${test_name}"

  args=(--query "${query_path}")
  if [[ "${data_file}" != "-" ]]; then
    args+=(--data "${GRAPH_ROOT}/${data_file}")
  fi
  ...
  if [[ "${graph_files}" != "-" ]]; then
    ...
    for graph_file in "${named_graphs[@]}"; do
      ...
      abs_graph="${GRAPH_ROOT}/${graph_file}"
