#!/usr/bin/env bash
# Verify the PostgreSQL bytea host boundary for a complete SBM0 Shardborough
# collection. PostgreSQL persists opaque manifest/IBK2 bytes; Lean verifies,
# opens and queries the retrieved collection.
set -euo pipefail
...
for source_path in "$source_dir"/manifest.sbm0 "$source_dir"/predicate-*.ibk2; do
