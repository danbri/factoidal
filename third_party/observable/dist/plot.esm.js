...
function cumsum(values2, valueof2) {
  var sum3 = 0, index2 = 0;
  return Float64Array.from(values2, valueof2 === void 0 ? (v2) => sum3 += +v2 || 0 : (v2) => sum3 += +valueof2(v2, index2++, values2) || 0);
}
...
var Path = class {
  ...
  arcTo(x12, y12, x2, y2, r) {
    ...
    if (this._x1 === null) {
      ...
      let x20 = x2 - x06, y20 = y2 - y06, l21_2 = x21 * x21 + y21 * y21, l20_2 = x20 * x20 + y20 * y20, l21 = Math.sqrt(l21_2), l01 = Math.sqrt(l01_2), l = r * Math.tan((pi - Math.acos((l21_2 + l01_2 - l20_2) / (2 * l21 * l01))) / 2), t01 = l / l01, t21 = l / l21;
...
function contours_default() {
  ...
  function smoothLinear(ring, values2, value) {
    ring.forEach(function(point6) {
      var x2 = point6[0], y2 = point6[1], xt = x2 | 0, yt = y2 | 0, v1 = valid(values2[yt * dx + xt]);
      if (x2 > 0 && x2 < dx && xt === x2) {
        point6[0] = smooth1(x2, valid(values2[yt * dx + xt - 1]), v1, value);
      }
      if (y2 > 0 && y2 < dy && yt === y2) {
        point6[1] = smooth1(y2, valid(values2[(yt - 1) * dx + xt]), v1, value);
      }
    });
  }
...
function density_default() {
  var x2 = defaultX, y2 = defaultY, weight = defaultWeight, dx = 960, dy = 500, r = 20, k2 = 2, o = r * 3, n = dx + o * 2 >> k2, m = dy + o * 2 >> k2, threshold2 = constant_default4(20);
  function grid(data) {
  ...
  density2.x = function(_) {
    return arguments.length ? (x2 = typeof _ === "function" ? _ : constant_default4(+_), density2) : x2;
  };
  density2.y = function(_) {
    return arguments.length ? (y2 = typeof _ === "function" ? _ : constant_default4(+_), density2) : y2;
  };
  density2.weight = function(_) {
    return arguments.length ? (weight = typeof _ === "function" ? _ : constant_default4(+_), density2) : weight;
  };
  ...
  density2.thresholds = function(_) {
    return arguments.length ? (threshold2 = typeof _ === "function" ? _ : Array.isArray(_) ? constant_default4(slice.call(_)) : constant_default4(_), density2) : threshold2;
  };
  ...
  *neighbors(i) {
  ...
  _clipSegment(x06, y06, x12, y12, c0, c1) {
    ...
    while (true) {
      ...
      let x2, y2, c4 = c0 || c1;
      if (c4 & 8) x2 = x06 + (x12 - x06) * (this.ymax - y06) / (y12 - y06), y2 = this.ymax;
      else if (c4 & 4) x2 = x06 + (x12 - x06) * (this.ymin - y06) / (y12 - y06), y2 = this.ymin;
      else if (c4 & 2) y2 = y06 + (y12 - y06) * (this.xmax - x06) / (x12 - x06), x2 = this.xmax;
  ...
  _project(x06, y06, vx, vy) {
    ...
    if (vy < 0) {
      ...
      if ((c4 = (this.ymin - y06) / vy) < t) y2 = this.ymin, x2 = x06 + (t = c4) * vx;
    } else if (vy > 0) {
      if (y06 >= this.ymax) return null;
      if ((c4 = (this.ymax - y06) / vy) < t) y2 = this.ymax, x2 = x06 + (t = c4) * vx;
    }
    if (vx > 0) {
      if (x06 >= this.xmax) return null;
      if ((c4 = (this.xmax - x06) / vx) < t) x2 = this.xmax, y2 = y06 + (t = c4) * vy;
    } else if (vx < 0) {
      if (x06 <= this.xmin) return null;
      if ((c4 = (this.xmin - x06) / vx) < t) x2 = this.xmin, y2 = y06 + (t = c4) * vy;
    }
    return [x2, y2];
  }
  _edgecode(x2, y2) {
...
var Delaunay = class _Delaunay {
  ...
  *neighbors(i) {
  ...
  find(x2, y2, i = 0) {
    ...
    const i0 = i;
    let c4;
    while ((c4 = this._step(i, x2, y2)) >= 0 && c4 !== i && c4 !== i0) i = c4;
    return c4;
  }
  _step(i, x2, y2) {
...
function centroidRingPoint(lambda, phi) {
  lambda *= radians2, phi *= radians2;
  var cosPhi = cos(phi), x2 = cosPhi * cos(lambda), y2 = cosPhi * sin(lambda), z = sin(phi), cx = y0 * z - z0 * y2, cy = z0 * x2 - x0 * z, cz = x0 * y2 - y0 * x2, m = hypot(cx, cy, cz), w = asin(m), v2 = m && -w / m;
  X2.add(v2 * cx);
  Y2.add(v2 * cy);
  Z2.add(v2 * cz);
  W1 += w;
...
function centroid_default(object) {
  ...
  if (m < epsilon22) {
    x2 = X1, y2 = Y1, z = Z1;
    if (W1 < epsilon5) x2 = X0, y2 = Y0, z = Z0;
    m = hypot(x2, y2, z);
...
function polygonContains_default(polygon, point6) {
  ...
  for (var i = 0, n = polygon.length; i < n; ++i) {
    ...
    for (var j = 0; j < m; ++j, lambda0 = lambda1, sinPhi0 = sinPhi1, cosPhi0 = cosPhi1, point0 = point1) {
      ...
      sum3.add(atan2(k2 * sign3 * sin(absDelta), cosPhi0 * cosPhi1 + k2 * cos(absDelta)));
...
function compareIntersection(a2, b) {
  return ((a2 = a2.x)[0] < 0 ? a2[1] - halfPi - epsilon5 : halfPi - a2[1]) - ((b = b.x)[0] < 0 ? b[1] - halfPi - epsilon5 : halfPi - b[1]);
}
  ...
  return {
    ...
    point: function(lambda1, phi1) {
      ...
      if (abs2(delta - pi2) < epsilon5) {
        ...
        if (abs2(lambda0 - sign0) < epsilon5) lambda0 -= sign0 * epsilon5;
...
function clipAntimeridianIntersect(lambda0, phi0, lambda1, phi1) {
  ...
  return abs2(sinLambda0Lambda1) > epsilon5 ? atan((sin(phi0) * (cosPhi1 = cos(phi1)) * sin(lambda1) - sin(phi1) * (cosPhi0 = cos(phi0)) * sin(lambda0)) / (cosPhi0 * cosPhi1 * sinLambda0Lambda1)) : (phi0 + phi1) / 2;
}
...
function circle_default(radius2) {
  var cr = cos(radius2), delta = 2 * radians2, smallRadius = cr > 0, notHemisphere = abs2(cr) > epsilon5;
  ...
  function intersect(a2, b, two) {
    ...
    var delta2 = lambda1 - lambda0, polar = abs2(delta2 - pi2) < epsilon5, meridian = polar || delta2 < epsilon5;
    if (!polar && phi1 < phi0) z = phi0, phi0 = phi1, phi1 = z;
    if (meridian ? polar ? phi0 + phi1 > 0 ^ q[1] < (abs2(q[0] - lambda0) < epsilon5 ? phi0 : phi1) : phi0 <= q[1] && q[1] <= phi1 : delta2 > pi2 ^ (lambda0 <= q[0] && q[0] <= lambda1)) {
...
function graticuleX(y06, y12, dy) {
  var y2 = range(y06, y12 - epsilon5, dy).concat(y12);
  return function(x2) {
...
function graticuleY(x06, x12, dx) {
  var x2 = range(x06, x12 - epsilon5, dx).concat(x12);
  return function(y2) {
...
function resample(project2, delta2) {
  function resampleLineTo(x06, y06, lambda0, a0, b0, c0, x12, y12, lambda1, a1, b1, c1, depth, stream) {
    ...
    if (d2 > 4 * delta2 && depth--) {
      var a2 = a0 + a1, b = b0 + b1, c4 = c0 + c1, m = sqrt(a2 * a2 + b * b + c4 * c4), phi2 = asin(c4 /= m), lambda2 = abs2(abs2(c4) - 1) < epsilon5 || abs2(lambda0 - lambda1) < epsilon5 ? (lambda0 + lambda1) / 2 : atan2(b, a2), p = project2(lambda2, phi2), x2 = p[0], y2 = p[1], dx2 = x2 - x06, dy2 = y2 - y06, dz = dy * dx2 - dx * dy2;
...
function projectionMutator(projectAt) {
  var project2, k2 = 150, x2 = 480, y2 = 250, lambda = 0, phi = 0, deltaLambda = 0, deltaPhi = 0, deltaGamma = 0, rotate, alpha = 0, sx = 1, sy = 1, theta = null, preclip = antimeridian_default, x06 = null, y06, x12, y12, postclip = identity_default3, delta2 = 0.5, projectResample, projectTransform, projectRotateTransform, cache, cacheStream;
...
function albersUsa_default() {
  ...
  albersUsa.translate = function(_) {
    ...
    alaskaPoint = alaska.translate([x2 - 0.307 * k2, y2 + 0.201 * k2]).clipExtent([[x2 - 0.425 * k2 + epsilon5, y2 + 0.12 * k2 + epsilon5], [x2 - 0.214 * k2 - epsilon5, y2 + 0.234 * k2 - epsilon5]]).stream(pointStream);
    hawaiiPoint = hawaii.translate([x2 - 0.205 * k2, y2 + 0.212 * k2]).clipExtent([[x2 - 0.214 * k2 + epsilon5, y2 + 0.166 * k2 + epsilon5], [x2 - 0.115 * k2 - epsilon5, y2 + 0.234 * k2 - epsilon5]]).stream(pointStream);
    return reset();
  };
...
function azimuthalRaw(scale3) {
  return function(x2, y2) {
    var cx = cos(x2), cy = cos(y2), k2 = scale3(cx * cy);
...
function azimuthalInvert(angle) {
  return function(x2, y2) {
    var z = sqrt(x2 * x2 + y2 * y2), c4 = angle(z), sc = sin(c4), cc2 = cos(c4);
    return [
      atan2(x2 * sc, z * cc2),
      asin(z && y2 * sc / z)
    ];
...
function conicConformalRaw(y06, y12) {
  ...
  function project2(x2, y2) {
    if (f > 0) {
      if (y2 < -halfPi + epsilon5) y2 = -halfPi + epsilon5;
    } else {
      if (y2 > halfPi - epsilon5) y2 = halfPi - epsilon5;
    }
...
function powish(transform2) {
  var scale3 = transform2(identity3, identity3), exponent = 1;
  function rescale() {
...
function quantile2() {
  ...
  scale3.invertExtent = function(y2) {
...
function transformer3() {
  ...
  scale3.domain = function(_) {
    return arguments.length ? ([x06, x12, x2] = _, t03 = transform2(x06 = +x06), t13 = transform2(x12 = +x12), t22 = transform2(x2 = +x2), k10 = t03 === t13 ? 0 : 0.5 / (t13 - t03), k21 = t13 === t22 ? 0 : 0.5 / (t22 - t13), s2 = t13 < t03 ? -1 : 1, scale3) : [x06, x12, x2];
  };
  scale3.clamp = function(_) {
...
function line_default2(x2, y2) {
  ...
  line2.x = function(_) {
    return arguments.length ? (x2 = typeof _ === "function" ? _ : constant_default5(+_), line2) : x2;
  };
  line2.y = function(_) {
    return arguments.length ? (y2 = typeof _ === "function" ? _ : constant_default5(+_), line2) : y2;
  };
  line2.defined = function(_) {
    return arguments.length ? (defined2 = typeof _ === "function" ? _ : constant_default5(!!_), line2) : defined2;
  };
...
function area_default3(x06, y06, y12) {
  ...
  x06 = typeof x06 === "function" ? x06 : x06 === void 0 ? x : constant_default5(+x06);
  y06 = typeof y06 === "function" ? y06 : y06 === void 0 ? constant_default5(0) : constant_default5(+y06);
  y12 = typeof y12 === "function" ? y12 : y12 === void 0 ? y : constant_default5(+y12);
  function area2(data) {
  ...
  area2.x = function(_) {
    return arguments.length ? (x06 = typeof _ === "function" ? _ : constant_default5(+_), x12 = null, area2) : x06;
  };
  area2.x0 = function(_) {
    return arguments.length ? (x06 = typeof _ === "function" ? _ : constant_default5(+_), area2) : x06;
  };
  area2.x1 = function(_) {
    return arguments.length ? (x12 = _ == null ? null : typeof _ === "function" ? _ : constant_default5(+_), area2) : x12;
  };
  area2.y = function(_) {
    return arguments.length ? (y06 = typeof _ === "function" ? _ : constant_default5(+_), y12 = null, area2) : y06;
  };
  area2.y0 = function(_) {
    return arguments.length ? (y06 = typeof _ === "function" ? _ : constant_default5(+_), area2) : y06;
  };
  area2.y1 = function(_) {
    return arguments.length ? (y12 = _ == null ? null : typeof _ === "function" ? _ : constant_default5(+_), area2) : y12;
  };
  ...
  area2.defined = function(_) {
    return arguments.length ? (defined2 = typeof _ === "function" ? _ : constant_default5(!!_), area2) : defined2;
  };
...
function mid(x12, x2) {
  return {
    transform(data) {
      ...
      const X23 = x2.transform(data);
      return isTemporal(X13) || isTemporal(X23) ? map2(X13, (_, i) => new Date((+X13[i] + +X23[i]) / 2)) : map2(X13, (_, i) => (+X13[i] + +X23[i]) / 2, Float64Array);
    },
    label: x12.label
  };
}
...
function groupn(x2, y2, {
  ...
  return {
    ...
    ...basic(options, (data, facets, plotOptions) => {
      const X3 = maybeApplyInterval(valueof(data, x2), plotOptions?.x);
      const Y3 = maybeApplyInterval(valueof(data, y2), plotOptions?.y);
...
function channelDomain(data, facets, channels, facetChannels, options) {
  ...
  for (const x2 in options) {
    ...
    let { value: y2, order = defaultOrder, reverse: reverse3 = defaultReverse, reduce = defaultReduce, limit = defaultLimit } = maybeValue(options[x2]);
    const negate2 = y2?.startsWith("-");
    if (negate2) y2 = y2.slice(1);
    order = order === void 0 ? negate2 !== (y2 === "width" || y2 === "height") ? descendingGroup : ascendingGroup : maybeOrder(order);
...
function difference(channels, k1, k2) {
  ...
  return map2(X23, (x2, i) => Math.abs(x2 - X13[i]), Float64Array);
}
...
function applyPosition(channels, scales, { projection: projection3 }) {
  ...
  let position3 = {};
  if (x2) position3.x = x2;
  if (y2) position3.y = y2;
  position3 = valueObject(position3, scales);
  if (projection3 && x2?.scale === "x" && y2?.scale === "y") project("x", "y", position3, projection3);
  if (x2) position3.x = coerceNumbers(position3.x);
  if (y2) position3.y = coerceNumbers(position3.y);
  return position3;
}
...
:where(.${className}-ramp text) {
  ...
  let x2;
  const applyRange = round2 ? (x3, range4) => x3.rangeRound(range4) : (x3, range4) => x3.range(range4);
  ...
  if (interpolate) {
    ...
    svg.append("g").attr("fill-opacity", opacity2).selectAll().data(domain).enter().append("rect").attr("x", x2).attr("y", marginTop).attr("width", Math.max(0, x2.bandwidth() - 1)).attr("height", height - marginTop - marginBottom).attr("fill", scale3);
    tickAdjust = () => {
    };
  }
...
var RuleX = class extends Mark {
  ...
  render(index2, scales, channels, dimensions, context) {
    ...
    return create2("svg:g", context).call(applyIndirectStyles, this, dimensions, context).call(applyTransform, this, { x: X3 && x2 }, offset, 0).call(
      (g) => g.selectAll().data(index2).enter().append("line").call(applyDirectStyles, this).attr("x1", X3 ? (i) => X3[i] : (marginLeft + width - marginRight) / 2).attr("x2", X3 ? (i) => X3[i] : (marginLeft + width - marginRight) / 2).attr("y1", Y13 && !isCollapsed(y2) ? (i) => Y13[i] + insetTop : marginTop + insetTop).attr(
        "y2",
        Y23 && !isCollapsed(y2) ? y2.bandwidth ? (i) => Y23[i] + y2.bandwidth() - insetBottom : (i) => Y23[i] - insetBottom : height - marginBottom - insetBottom
...
var RuleY = class extends Mark {
  ...
  render(index2, scales, channels, dimensions, context) {
    ...
    return create2("svg:g", context).call(applyIndirectStyles, this, dimensions, context).call(applyTransform, this, { y: Y3 && y2 }, 0, offset).call(
      (g) => g.selectAll().data(index2).enter().append("line").call(applyDirectStyles, this).attr("x1", X13 && !isCollapsed(x2) ? (i) => X13[i] + insetLeft : marginLeft + insetLeft).attr(
        "x2",
        X23 && !isCollapsed(x2) ? x2.bandwidth ? (i) => X23[i] + x2.bandwidth() - insetRight : (i) => X23[i] - insetRight : width - marginRight - insetRight
...
function defaultWidth(text2, start2 = 0, end = text2.length) {
  ...
  for (let i = start2; i < end; i = readCharacter(text2, i)) {
    sum3 += defaultWidthMap[text2[i]] ?? (isPictographic(text2, i) ? 120 : defaultWidthMap.e);
  }
  return sum3;
}
function monospaceWidth(text2, start2 = 0, end = text2.length) {
  let sum3 = 0;
  for (let i = start2; i < end; i = readCharacter(text2, i)) {
    sum3 += isPictographic(text2, i) ? 126 : 63;
  }
  return sum3;
}
...
function stackX(stackOptions = {}, options = {}) {
  ...
  const [transform2, Y3, x12, x22] = stack(y2, x2, "y", "x", stackOptions, rest);
...
function stackX1(stackOptions = {}, options = {}) {
  ...
  const [transform2, Y3, X3] = stack(y2, x2, "y", "x", stackOptions, options);
...
function stackY1(stackOptions = {}, options = {}) {
  ...
  const [transform2, X3, Y3] = stack(x2, y2, "x", "y", stackOptions, options);
...
function stack(x2, y2 = one2, kx2, ky2, { offset: offset2, order, reverse: reverse3 }, options) {
  if (y2 === null) throw new Error(`stack requires ${ky2}`);
  const z = maybeZ(options);
  const [X3, setX] = maybeColumn(x2);
  const [Y13, setY1] = column(y2);
  const [Y23, setY2] = column(y2);
      ...
      for (const facet of facets) {
        ...
        for (const stack2 of stacks) {
          ...
          for (const i of stack2) {
            const y3 = Y3[i];
            if (y3 < 0) yn = Y24[i] = (Y14[i] = yn) + y3;
            else if (y3 > 0) yp = Y24[i] = (Y14[i] = yp) + y3;
            else Y24[i] = Y14[i] = yp;
          }
        }
...
var Rect = class extends Mark {
  ...
  render(index2, scales, channels, dimensions, context) {
    ...
    if ((X13 || X23) && !projection3 && isCollapsed(x2)) X13 = X23 = null;
    if ((Y13 || Y23) && !projection3 && isCollapsed(y2)) Y13 = Y23 = null;
...
function outerRange(scale3) {
  ...
  let x2 = scale3(domain[domain.length - 1]);
  if (x2 < x12) [x12, x2] = [x2, x12];
  return [x12, x2 + scale3.bandwidth()];
}
...
function binY(outputs = { x: "count" }, options = {}) {
  [outputs, options] = mergeOptions2(outputs, options);
  const { x: x2, y: y2 } = options;
  return binn(null, maybeBinValue(y2, options, identity6), x2, null, outputs, maybeInsetY(options));
}
...
function maybeBin(options) {
  ...
  const bin2 = (data) => {
    ...
    if (isTemporal(V) || isTimeThresholds(thresholds)) {
      ...
      if (typeof t === "number") t = utcTickInterval(min4, max3, t);
      if (isInterval(t)) {
        if (domain === extent) {
          min4 = t.floor(min4);
...
function areaX(data, options) {
  ...
  return new Area(data, maybeStackX(maybeIdentityX({ ...rest, y1: y2, y2: void 0 }, y2 === indexOf ? "x2" : "x")));
}
function areaY(data, options) {
  const { x: x2 = indexOf, ...rest } = maybeDenseIntervalX(options);
  return new Area(data, maybeStackY(maybeIdentityY({ ...rest, x1: x2, x2: void 0 }, x2 === indexOf ? "y2" : "y")));
}
...
function link2(data, { x: x2, x1: x12, x2: x22, y: y2, y1: y12, y2: y22, ...options } = {}) {
  ...
  render(index2, scales, channels, dimensions, context) {
    ...
    return create2("svg:g", context).call(applyIndirectStyles, this, dimensions, context).call(applyTransform, this, scales).call(
      (g) => g.selectAll().data(index2).enter().append("path").call(applyDirectStyles, this).attr("d", (i) => {
        let x12 = X13[i], y12 = Y13[i], x2 = X23[i], y2 = Y23[i];
        ...
        const bendAngle = this.sweep(x12, y12, x2, y2) * bend * radians3;
...
var AbstractBar = class extends Mark {
  ...
  render(index2, scales, channels, dimensions, context) {
    ...
    return create2("svg:g", context).call(applyIndirectStyles, this, dimensions, context).call(this._transform, this, scales).call(
      (g) => g.selectAll().data(index2).enter().call(
        rx1y1 || rx1y2 || rx2y1 || rx2y2 ? (g2) => g2.append("path").call(applyDirectStyles, this).call(applyRoundedRect, x2, y2, add(x2, w), add(y2, h), this).call(applyChannelStyles, this, channels) : (g2) => g2.append("rect").call(applyDirectStyles, this).attr("x", x2).attr("width", w).attr("y", y2).attr("height", h).call(applyAttr, "rx", rx).call(applyAttr, "ry", ry).call(applyChannelStyles, this, channels)
      )
    ).node();
  }
...
var BarX = class extends AbstractBar {
  ...
  _x({ x: x2 }, { x1: X13, x2: X23 }, { marginLeft }) {
    const { insetLeft } = this;
    return isCollapsed(x2) ? marginLeft + insetLeft : (i) => Math.min(X13[i], X23[i]) + insetLeft;
  }
  _width({ x: x2 }, { x1: X13, x2: X23 }, { marginRight, marginLeft, width }) {
    const { insetLeft, insetRight } = this;
    return isCollapsed(x2) ? width - marginRight - marginLeft - insetLeft - insetRight : (i) => Math.max(0, Math.abs(X23[i] - X13[i]) - insetLeft - insetRight);
  }
};
var BarY = class extends AbstractBar {
  ...
  _y({ y: y2 }, { y1: Y13, y2: Y23 }, { marginTop }) {
    const { insetTop } = this;
    return isCollapsed(y2) ? marginTop + insetTop : (i) => Math.min(Y13[i], Y23[i]) + insetTop;
  }
  _height({ y: y2 }, { y1: Y13, y2: Y23 }, { marginTop, marginBottom, height }) {
    const { insetTop, insetBottom } = this;
    return isCollapsed(y2) ? height - marginTop - marginBottom - insetTop - insetBottom : (i) => Math.max(0, Math.abs(Y23[i] - Y13[i]) - insetTop - insetBottom);
  }
};
...
function dotX(data, { x: x2 = identity6, ...options } = {}) {
  return new Dot(data, maybeIntervalMidY({ ...options, x: x2 }));
}
function dotY(data, { y: y2 = identity6, ...options } = {}) {
...
function lineX(data, { x: x2 = identity6, y: y2 = indexOf, ...options } = {}) {
...
function autoSpec(data, options) {
  ...
  const { x: x2, y: y2, color: color3, size } = options;
...
function normalizeOptions({ x: x2, y: y2, color: color3, size, fx, fy, mark } = {}) {
  ...
  return { x: x2, y: y2, color: color3, size, fx, fy, mark };
}
...
function mapX(mapper, options = {}) {
  ...
  if (x2 === void 0 && x12 === void 0 && x22 === void 0) options = { ...options, x: x2 = identity6 };
  const outputs = {};
...
function reduceMean(k2, s2, strict) {
  if (strict) {
    const sum3 = reduceSum2(k2, s2, strict);
    return {
      mapIndex(I, S, T) {
        sum3.mapIndex(I, S, T);
        for (let i = 0, n = I.length - k2 + 1; i < n; ++i) {
    ...
    return {
      mapIndex(I, S, T) {
        ...
        for (let i = 0, j = Math.min(n, k2 - s2 - 1); i < j; ++i) {
          let v2 = S[I[i]];
          if (v2 !== null && !isNaN(v2 = +v2)) sum3 += v2, ++count3;
        }
        for (let i = -s2, j = n - s2; i < j; ++i) {
          let a2 = S[I[i + k2 - 1]];
          let b = S[I[i]];
          if (a2 !== null && !isNaN(a2 = +a2)) sum3 += a2, ++count3;
          T[I[i + s2]] = sum3 / count3;
          if (b !== null && !isNaN(b = +b)) sum3 -= b, --count3;
        }
      }
    };
  }
...
function bollingerX(data, {
  ...
  return marks(
    ...
    isNoneish(stroke) ? null : lineX(data, map3({ x: bollinger(options) }, { x: x2, y: y2, stroke, strokeOpacity, strokeWidth, ...options }))
  );
}
function bollingerY(data, {
  ...
  return marks(
    ...
    isNoneish(stroke) ? null : lineY(data, map3({ y: bollinger(options) }, { x: x2, y: y2, stroke, strokeOpacity, strokeWidth, ...options }))
  );
}
...
function tickX(data, { x: x2 = identity6, ...options } = {}) {
  return new TickX(data, { ...options, x: x2 });
}
function tickY(data, { y: y2 = identity6, ...options } = {}) {
...
function boxX(data, {
  ...
  const group3 = y2 != null ? groupY : groupZ;
  return marks(
    ruleY(data, group3({ x1: loqr1, x2: hiqr2 }, { x: x2, y: y2, stroke, strokeOpacity, ...options })),
    barX(data, group3({ x1: "p25", x2: "p75" }, { x: x2, y: y2, fill, fillOpacity, ...options })),
    tickX(data, group3({ x: "p50" }, { x: x2, y: y2, stroke, strokeOpacity, strokeWidth, sort: sort3, ...options })),
    dot(data, map3({ x: oqr }, { x: x2, y: y2, z: y2, r, stroke, strokeOpacity, ...options }))
  );
}
function boxY(data, {
  y: y2 = identity6,
  ...
  const group3 = x2 != null ? groupX : groupZ;
  return marks(
    ruleX(data, group3({ y1: loqr1, y2: hiqr2 }, { x: x2, y: y2, stroke, strokeOpacity, ...options })),
    barY(data, group3({ y1: "p25", y2: "p75" }, { x: x2, y: y2, fill, fillOpacity, ...options })),
    tickY(data, group3({ y: "p50" }, { x: x2, y: y2, stroke, strokeOpacity, strokeWidth, sort: sort3, ...options })),
    dot(data, map3({ y: oqr }, { x: x2, y: y2, z: x2, r, stroke, strokeOpacity, ...options }))
  );
}
...
var AbstractRaster = class extends Mark {
  constructor(data, channels, options = {}, defaults22) {
    ...
    if (data != null && width != null && height != null) {
      if (x2 === void 0 && x12 != null && x22 != null) x2 = denseX(x12, x22, width, height);
    ...
    this.interpolate = x2 == null || y2 == null ? null : maybeInterpolate(interpolate);
  }
};
...
function renderBounds({ x1: x12, y1: y12, x2, y2 }, dimensions, { projection: projection3 }) {
  const { width, height, marginTop, marginRight, marginBottom, marginLeft } = dimensions;
  return [
    x12 && projection3 == null ? x12[0] : marginLeft,
    y12 && projection3 == null ? y12[0] : marginTop,
    x2 && projection3 == null ? x2[0] : width - marginRight,
    y2 && projection3 == null ? y2[0] : height - marginBottom
  ];
}
...
function pick(random) {
  return (a2, ca3, b, cb, c4, cc2, x2, y2) => {
    const u4 = random(x2, y2);
...
var Contour = class extends AbstractRaster {
  ...
  filter(index2, { x: x2, y: y2, value, ...channels }, values2) {
...
function contourGeometry({ thresholds, interval: interval2, ...options }) {
  ...
  return initializer(options, function(data, facets, channels, scales, dimensions, context) {
    ...
    const contourFacets = [];
    for (const V2 of VV) {
      contourFacets.push(range(contourData.length, contourData.push(...map2(T, (t) => contour2(V2, t)))));
    }
...
function maybeTicks(thresholds, V, min4, max3) {
  if (typeof thresholds?.range === "function") return thresholds.range(thresholds.floor(min4), max3);
  if (typeof thresholds === "function") thresholds = thresholds(V, min4, max3);
...
function pxpy(k2, i) {
  ...
  return function(data, facets, { x: x12, y: y12, px, py, ...c1 }, ...args) {
    const { channels: { x: x2, y: y2, ...c4 } = {}, ...rest } = i.call(this, data, facets, { ...c1, x: px, y: py }, ...args);
    return {
      channels: {
        ...c4,
        ...x2 && { px: x2, ...k2 === "x" && { x: x2 } },
  ...
  render(index2, scales, channels, dimensions, context) {
    ...
    function links(index3) {
      ...
      for (let i2 = 0; i2 < halfedges.length; ++i2) {
        ...
        if (j > i2) link3(triangles[i2], triangles[j]);
      }
...
var Voronoi2 = class extends Mark {
  ...
  render(index2, scales, channels, dimensions, context) {
    ...
    return create2("svg:g", context).call(applyIndirectStyles, this, dimensions, context).call(applyTransform, this, { x: X3 && x2, y: Y3 && y2 }).call((g) => {
...
function differenceK(k2, data, {
  ...
  y1: y12,
  y2,
  x: x3 = x12 === void 0 && x2 === void 0 ? k2 === "y" ? indexOf : identity6 : void 0,
  y: y3 = y12 === void 0 && y2 === void 0 ? k2 === "x" ? indexOf : identity6 : void 0,
  fill,
  // ignored
  positiveFill = "#3ca951",
  negativeFill = "#4269d0",
...
var Image = class extends Mark {
  constructor(data, options = {}) {
    let { x: x2, y: y2, r, width, height, rotate, src, preserveAspectRatio, crossOrigin, frameAnchor, imageRendering } = options;
    if (r == null) r = void 0;
...
function position2(X3, W, R, x2, w, r) {
...
function ibetainv(p, a2, b) {
  ...
  var b1 = b - 1;
  var j = 0;
  var lna, lnb, pp, t, u4, err, x2, al, h, w, afac;
  if (p <= 0) return 0;
  if (p >= 1) return 1;
  if (a2 >= 1 && b >= 1) {
    pp = p < 0.5 ? p : 1 - p;
  ...
  for (; j < 10; j++) {
    ...
    u4 = err / t;
    x2 -= t = u4 / (1 - 0.5 * Math.min(1, u4 * (a1 / x2 - b1 / (1 - x2))));
    if (x2 <= 0) x2 = 0.5 * (x2 + t);
...
function betacf(x2, a2, b) {
  ...
  for (; m <= 100; m++) {
    ...
    d = 1 / d;
    h *= d * c4;
    aa2 = -(a2 + m) * (qab + m) * x2 / ((a2 + m2) * (qap + m2));
    d = 1 + aa2 * d;
    if (Math.abs(d) < fpmin) d = fpmin;
    c4 = 1 + aa2 / c4;
    if (Math.abs(c4) < fpmin) c4 = fpmin;
...
var LinearRegressionX = class extends LinearRegression {
  ...
  _renderBand(I, X3, Y3) {
    ...
    return area_default3().y((y3) => y3).x0((y3) => g(y3, -1)).x1((y3) => g(y3, 1))(range(y12, y2 - precision / 2, precision).concat(y2));
  }
  _renderLine(I, X3, Y3) {
    const [y12, y2] = extent(I, (i) => Y3[i]);
    const f = linearRegressionF(I, Y3, X3);
...
var LinearRegressionY = class extends LinearRegression {
  ...
  _renderBand(I, X3, Y3) {
    ...
    return area_default3().x((x3) => x3).y0((x3) => g(x3, -1)).y1((x3) => g(x3, 1))(range(x12, x2 - precision / 2, precision).concat(x2));
  }
  _renderLine(I, X3, Y3) {
    const [x12, x2] = extent(I, (i) => X3[i]);
    const f = linearRegressionF(I, X3, Y3);
...
function linearRegressionX(data, { y: y2 = indexOf, x: x2 = identity6, stroke, fill = isNoneish(stroke) ? "currentColor" : stroke, ...options } = {}) {
...
function wafflePolygon(y2, options) {
  const x2 = y2 === "y" ? "x" : "y";
  const y12 = `${y2}1`;
  const y22 = `${y2}2`;
  return initializer(waffleRender(options), function(data, facets, channels, scales, dimensions) {
    ...
    const xy = valueObject({ ...x2 in channels && { [x2]: channels[x2] }, [y12]: channels[y12], [y22]: channels[y22] }, scales);
    const barwidth = this[y2 === "y" ? "_width" : "_height"](scales, xy, dimensions);
    const barx = this[y2 === "y" ? "_x" : "_y"](scales, xy, dimensions);
    const scale3 = unit3 * scaleof(scales.scales[y2]);
    ...
    const y06 = scales[y2](0);
    const transform2 = y2 === "y" ? ([x3, y3]) => [x3 * cx, -y3 * cy] : ([x3, y3]) => [y3 * cy, x3 * cx];
    ...
    return {
      channels: {
        ...
        [`c${x2}`]: { value: [cx, x06], source: null, filter: null },
        [`c${y2}`]: { value: [cy, y06], source: null, filter: null },
        [x2]: { value: X3, scale: null, source: null },
