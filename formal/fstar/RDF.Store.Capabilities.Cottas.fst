...
let caps_of_cottas (cods : cottas_ondisk_store) (scope : cottas_ondisk_graph_scope)
  : Tot store_caps =
  {
    ...
    //
    // 2026-07-06 follow-up (BOUND-side): the bound itself is now built
    // via `cottas_ondisk_build_bound_qp_tok` (direct term -> token
    // serialization, no dictionary) instead of `cottas_ondisk_build_
    // bound_qp_opt` (term -> id via the corpus-wide revmap/Bet7). The
    // tok builder never returns `None` (serialization always succeeds;
    // see its own banner comment for why the old "term absent from
    // corpus" early-exit isn't needed for correctness), so there is no
    // `option` to match here — `cottas_ondisk_search_tok` always runs,
    // and a genuinely-absent bound term still yields zero rows cheaply
    // via the row-group dict-page prune inside it.
    sc_solve =
      (fun b ->
        let bound = cottas_ondisk_build_bound_qp_tok b.bs b.bp b.bo scope in
        cottas_ondisk_rows_tok_to_triples (cottas_ondisk_search_tok cods bound));
    ...
    sc_solve_limited =
      (fun b n ->
        let bound = cottas_ondisk_build_bound_qp_tok b.bs b.bp b.bo scope in
        cottas_ondisk_rows_tok_to_triples (cottas_ondisk_search_limited_tok cods bound n));
    ...
    sc_estimate =
      (fun b ->
        let bound = cottas_ondisk_build_bound_qp_tok b.bs b.bp b.bo scope in
        cottas_ondisk_estimate_tok cods bound);
    ...
    sc_count_exact =
      (fun b ->
        let bound = cottas_ondisk_build_bound_qp_tok b.bs b.bp b.bo scope in
        cottas_ondisk_count_exact_tok cods bound);
