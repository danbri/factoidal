...
// Tuple binder via `match xy with | (x, y) ->`, inner fold_left captures x.
// Expected: FAIL, reproducing the bug (per background).
...
val outer_sound (seed : list int) : Lemma
  ...
  (ensures inv (List.Tot.fold_left
                  (fun (acc : list int) (xy : nat * nat) ->
                     match xy with
                     | (x, y) -> List.Tot.fold_left (fun a z -> (x + z) :: a) acc [1; 2])
                  seed items))
...
let outer_sound seed =
  let outer_step : list int -> (nat * nat) -> list int =
    fun (acc : list int) (xy : nat * nat) ->
      match xy with
      | (x, y) -> List.Tot.fold_left (fun a z -> (x + z) :: a) acc [1; 2] in
  ...
  with _ . begin
    match xy with
    | (x, y) -> fold_left_inv inv (fun a z -> (x + z) :: a) [1; 2] acc
  end;
