...
// Tuple binder, but inner fold_left captures NEITHER x nor y (ignores
// the destructured components entirely). Isolates closure-capture vs
// mere destructure.
...
val outer_sound (seed : list int) : Lemma
  ...
  (ensures inv (List.Tot.fold_left
                  (fun (acc : list int) (xy : nat * nat) ->
                     let (x, y) = xy in
                     List.Tot.fold_left (fun a z -> z :: a) acc [1; 2])
                  seed items))
...
let outer_sound seed =
  let outer_step : list int -> (nat * nat) -> list int =
    fun (acc : list int) (xy : nat * nat) ->
      let (x, y) = xy in
      List.Tot.fold_left (fun a z -> z :: a) acc [1; 2] in
  ...
  with _ . begin
    let (x, y) = xy in
    fold_left_inv inv (fun a z -> z :: a) [1; 2] acc
  end;
