...
//   engine ledger    THIS FILE, landing 4: every owl_rule_* function
//                    in OWL.Closure.fsti mapped to its table row or
//                    named as an extension with its justification's
//                    home -- see the ledger section at the end
// ===================================================================
...
(** ================================================================= **)
(** Landing 4: THE ENGINE LEDGER — every OWL.Closure rule function    **)
(** accounted for.                                                    **)
(**                                                                   **)
(** "All formalized rules made explicit" (goal, 2026-08-04) means no  **)
(** engine rule may exist without a declarative account: either it    **)
(** implements a W3C table row transcribed above, or it is a NAMED    **)
(** EXTENSION whose soundness justification has a stated home. This   **)
(** ledger is that account, one line per `owl_rule_*` in              **)
(** OWL.Closure.fsti, taken from the implementations' own banners.    **)
(**                                                                   **)
(** Legend:  [row]  implements the named table row(s) above;          **)
(**          [ext]  sound extension — justification in the rule's     **)
(**          banner in OWL.Closure.fsti (comprehension-witness layer  **)
(**          entries cite OWL 2 RDF-Based Semantics section 8.2's     **)
(**          comprehension conditions; RDF-Based entries cite the     **)
(**          Table 5.x semantic condition they realise);              **)
(**          [axm]  materialises an axiomatic-triple table, not a     **)
(**          rule;  [mode] fires only under a catalog semantics mode. **)
(**                                                                   **)
(**   equivalent_class                    [row] scm-eqc1 (emissions   **)
(**       are the two subClassOf conclusions; the cax-eqc1/eqc2       **)
(**       EFFECT is reached via cax-sco downstream -- corrected       **)
(**       2026-08-04 while writing the licensing lemma, which is      **)
(**       stated against scm_eqc1_derives)                            **)
(**   equivalent_property                 [row] scm-eqp1 (emissions   **)
(**       are the two subPropertyOf conclusions; the prp-eqp1/eqp2    **)
(**       EFFECT is reached via prp-spo1 downstream -- corrected      **)
(**       2026-08-04, same claim drift as equivalent_class above,     **)
(**       caught by the same licensing-lemma discipline)              **)
(**   scm_eqc2                            [row] scm-eqc2              **)
(**   scm_eqp2                            [row] scm-eqp2              **)
(**   symmetric_property                  [row] prp-symp              **)
(**   transitive_property                 [row] prp-trp               **)
(**   inverseOf_domain_range_flip         [ext] inverse semantics:    **)
(**       domain/range of p become range/domain of its inverse        **)
(**   inverse_of                          [row] prp-inv1 + prp-inv2   **)
(**   sameAs_reflexivity                  [row] eq-ref                **)
(**   sameAs_symmetry                     [row] eq-sym                **)
(**   differentFrom_symmetry              [ext] Table 5.13's          **)
(**       differentFrom condition is symmetric in its arguments       **)
(**   sameAs_transitivity                 [row] eq-trans              **)
(**   sameAs_replace_subject              [row] eq-rep-s              **)
(**   sameAs_replace_object               [row] eq-rep-o              **)
(**   sameAs_replace_predicate            [row] eq-rep-p              **)
(**   functional                          [row] prp-fp                **)
(**   inverse_functional                  [row] prp-ifp               **)
(**   pdw_to_differentFrom                [ext] disjoint properties   **)
(**       force distinct subjects on shared objects (banner proof)    **)
(**   pdw_shared_value_to_differentFrom   [ext] sibling of the above  **)
(**   fp_diff_to_diff                     [ext] functional property + **)
(**       different objects force different subjects                  **)
(**   ifp_diff_to_diff                    [ext] inverse-functional    **)
(**       mirror of the above                                         **)
(**   disjoint_with_propagation           [ext] disjointness inherits **)
(**       down rdfs:subClassOf (banner cites cax-dw + cax-sco)        **)
(**   disjoint_to_complement              [ext] disjointWith into     **)
(**       complementOf scaffolding for the clash checker              **)
(**   svf2_existential_witness            [ext] comprehension layer   **)
(**   minc1_bridge                        [ext] comprehension layer   **)
(**   cls_hv1                             [row] cls-hv1               **)
(**   cls_hv2                             [row] cls-hv2               **)
(**   cls_svf2_qualified                  [ext] comprehension layer   **)
(**   cls_minc_qual1                      [ext] comprehension layer   **)
(**   cls_hasself1                        [ext] ObjectHasSelf         **)
(**       semantics (Table 5.x); hasSelf has no RL table row          **)
(**   cls_hasself2_synth                  [ext] sibling of the above  **)
(**   cls_svf_thing_materialize           [ext] cls-svf2-adjacent     **)
(**       comprehension entry                                         **)
(**   cls_svf_thing_witness               [ext] comprehension layer   **)
(**   cax_dw_to_differentFrom             [ext] disjoint classes      **)
(**       force differentFrom on their members                        **)
(**   cls_maxqc1                          [row] cls-maxqc1            **)
(**   cls_exactqc1                        [ext] exact qualified       **)
(**       cardinality decomposes into min+max (no RL row of its own)  **)
(**   cls_maxc2                           [row] cls-maxc2             **)
(**   cls_maxqc_comp                      [ext] the #236 anchor       **)
(**       machinery — KNOWN sound-but-narrow, see CLAUDE.md           **)
(**   cls_avf1                            [row] cls-avf               **)
(**   reflexive_property                  [ext] ReflexiveProperty     **)
(**       semantics; the RL profile has no prp-rfl row                **)
(**   scm_cls_restriction                 [ext] scm-cls extended to   **)
(**       restriction nodes                                           **)
(**   property_chain_2                    [row] prp-spo2 (n = 2)      **)
(**   property_chain_n                    [row] prp-spo2 (n >= 3)     **)
(**   chain_to_transitive                 [ext] a chain p.p -> p IS   **)
(**       transitivity — bridge, banner proof                         **)
(**   transitive_to_chain                 [ext] converse bridge       **)
(**   named_sameAs_to_equivClass          [ext] RDF-Based semantics:  **)
(**       sameAs classes are equivalentClass (Table 5.x)              **)
(**   named_equivClass_to_sameAs_mode     REMOVED 2026-09-07 as       **)
(**   named_equivClass_to_sameAs          UNSOUND. It emitted          **)
(**       `C owl:sameAs D` from `C owl:equivalentClass D` for named    **)
(**       classes under a DIRECT gate. Coextension of two class        **)
(**       expressions is not identity of the two resources, under      **)
(**       either semantics; the [mode] tag recorded a catalog gate     **)
(**       where a LICENCE was required, and this ledger admitted it    **)
(**       on that tag alone. See the removal note in                   **)
(**       OWL.Closure.fsti and                                         **)
(**       docs/designissues/2026-09-07-fstar-owl-soundness-audit.md.   **)
(**   cls_int1                            [row] cls-int2 (MISNAMED:   **)
(**       reads x typed into the intersection C, emits x typed into   **)
(**       every member Ci -- cls-int2's shape, not cls-int1's (which  **)
(**       synthesises C-membership from the batched `types_all`       **)
(**       premise, all Ci already held). No engine function realizes **)
(**       the literal cls-int1 row. Corrected 2026-08-05 by the       **)
(**       licensing program, same disease as scm_dom2/scm_rng2.)      **)
(**   cls_oneof                           [row] cls-oo                **)
(**   cls_uni                             [row] scm-uni (MISNAMED:    **)
(**       emits `Ci rdfs:subClassOf C` per member -- Table 8's        **)
(**       schema row, not Table 5's cls-uni type-propagation row. The **)
(**       owl:disjointUnionOf branch additionally emits an EXTENSION  **)
(**       (no W3C row): a plain-unionOf restatement plus pairwise     **)
(**       disjointWith over the member list. Corrected 2026-08-05 by  **)
(**       the licensing program.)                                     **)
(**   cls_uni_elim                        [ext] union membership      **)
(**       elimination under disjointness side conditions              **)
(**   oneof_set_equivalence               [ext] oneOf lists with      **)
(**       equal member SETS name equivalent classes                   **)
(**   prp_key                             [row] prp-key               **)
(**   xsd_datatype_axioms                 [axm] dt-type1 instantiated **)
(**       at the supported XSD datatypes                              **)
(**   dt_range_intersect                  [ext] two ranges compose    **)
(**       to their intersection — banner cites WebOnt-I5.24-002 and   **)
(**       flags NOT-a-minimality-claim                                **)
(**   scm_dom2                            [row] scm-dom1 (MISNAMED:   **)
(**       the function lifts domain up subClassOf -- scm-dom1's work; **)
(**       refuted-by-counterexample against scm-dom2, whose           **)
(**       subPropertyOf logic lives in subprop_domain_range below.    **)
(**       Corrected 2026-08-05 by the licensing program.)             **)
(**   scm_rng2                            [row] scm-rng1 (MISNAMED:   **)
(**       same story as scm_dom2 -- lifts range up subClassOf;        **)
(**       corrected 2026-08-05)                                       **)
(**   subprop_domain_range                [row] scm-dom2 + scm-rng2   **)
(**       (joint implementation over subPropertyOf)                   **)
(**   symmetric_metapredicates            [ext] Group E(a): OWL       **)
(**       metapredicates whose semantic condition is argument-        **)
(**       symmetric (complementOf, disjointWith, inverseOf, ...)      **)
(**   inverse_characteristics             [ext] characteristics       **)
(**       transfer across owl:inverseOf (symmetric/functional/...)    **)
(**   equivalent_property_characteristics [ext] characteristics       **)
(**       transfer across equivalentProperty                          **)
(**   cardinality_to_min_max              [ext] owl:cardinality       **)
(**       decomposes into min + max (OWL 2 structural equivalence)    **)
(**   cls_maxqc34                         [row] cls-maxqc3 +          **)
(**       cls-maxqc4                                                  **)
(**   fp_pinned_subproperty               [ext] banner proof          **)
(**   singleton_nominal_functionality     [ext] banner proof          **)
(**   hasvalue_card_disjoint              [ext] banner proof          **)
(**   avf_thing_to_range                  [ext] avf under owl:Thing   **)
(**       IS a range fact — banner cites WebOnt-I5.24-004             **)
(**   extensional_symmetry                [ext] Group E(h):           **)
(**       right-to-left half of Table 5.14, banner carries the        **)
(**       pinned-extension argument                                   **)
(**   xsd_core_datatype_axioms            [axm] dt-type1 core set     **)
(**   builtin_vocabulary_axioms           [axm] built-in vocabulary   **)
(**   all_disjoint_classes                [row] cax-adc               **)
(**   all_disjoint_properties             [row] prp-adp               **)
(**   allDifferent_to_differentFrom       [row] eq-diff2/eq-diff3's   **)
(**       premise expansion, materialised pairwise                    **)
(**   differentFrom_to_allDifferent       [ext] converse packaging    **)
(**   rdf_based_full_meta_axioms_mode     [mode] RDF-Based Semantics  **)
(**       meta-axioms, catalog-gated                                  **)
(**   comp_singleton_union                [ext] comprehension layer   **)
(**   comp_min1_restriction               [ext] comprehension layer   **)
(**   comp_oneof_union                    [ext] comprehension layer   **)
(**   comp_union_oneof                    [ext] comprehension layer   **)
(**   comp_enum_range_value               [ext] comprehension layer   **)
(**   comp_range_avf                      [ext] comprehension layer   **)
(**   comp_range_intersection             [ext] comprehension layer   **)
(**   comp_pinned_domain_enum             [ext] comprehension layer   **)
(**                                                                   **)
(** Count: 82 rule functions — 34 table-row implementations, 42       **)
(** extensions, 4 axiomatic-table materialisations, 2 mode-gated.     **)
(** (Was 84 with 4 mode-gated until 2026-09-07, when the two          **)
(** named_equivClass_to_sameAs entries were deleted as unsound.)      **)
(** (subprop_domain_range and cls_maxqc34 each cover two rows;        **)
(** property_chain_2/_n split one row; the arithmetic reconciles.)    **)
(**                                                                   **)
(** RDFS rows are implemented by RDFS.Closure's twelve rule           **)
(** functions, transcribed since 2026-07 in RDF.Entailment.RDFS.Spec  **)
(** and licensed in RDF.Entailment.RDFS.Refinement; the interleaved   **)
(** OWL-RL fixpoint reuses them for prp-dom (rdfs2), prp-rng (rdfs3), **)
(** prp-spo1 (rdfs7), cax-sco (rdfs9), scm-sco (rdfs11), scm-spo      **)
(** (rdfs5), so those table rows are engine-covered through the RDFS  **)
(** family.                                                           **)
(**                                                                   **)
(** WHAT THIS LEDGER IS NOT: a proof. Each [row] line is a CLAIM that **)
(** the function implements the row; turning each claim into a        **)
(** licensing lemma (every emission is `<row>_derives`-licensed, as   **)
(** RDF.Entailment.RDFS.Refinement does for the RDFS family) and a    **)
(** truth-preservation lemma (as OWL.Semantics.Soundness does for     **)
(** domain/range/symmetric/sameAs/oneOf) is the continuing            **)
(** rule-by-rule program, task #10. The ledger makes the OBLIGATIONS  **)
(** explicit; the program discharges them.                            **)
(** ================================================================= **)
