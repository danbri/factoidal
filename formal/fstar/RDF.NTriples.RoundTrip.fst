...
(** ---------------------------------------------------------------------- **)
(**                                                                          **)
(** SERIALIZER — ONE candidate now. There used to be two:                    **)
(**   - `RDF.Pretty.term_to_ntriples` — a second renderer with no literal    **)
(**     escaping at all, justified as "display, not wire". Every consumer     **)
(**     of it turned out to be a wire path, which cost issues #339 and       **)
(**     #443 (store data loss). DELETED 2026-08-14. The theorems below       **)
(**     never covered it, and that was exactly the hazard: a term-level      **)
(**     round-trip proof says nothing about which term function a consumer   **)
(**     picked.                                                              **)
