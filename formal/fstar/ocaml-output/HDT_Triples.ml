...
let parse_triples_info (a : HDT_Container.hdt_bytes) (pos : Prims.nat) :
  hdt_triples_info FStar_Pervasives_Native.option=
  match HDT_Container.parse_bitmap_info a pos with
  | FStar_Pervasives_Native.None -> FStar_Pervasives_Native.None
  | FStar_Pervasives_Native.Some bmy ->
      (match HDT_Container.parse_bitmap_info a bmy.HDT_Container.bm_end with
       | FStar_Pervasives_Native.None -> FStar_Pervasives_Native.None
       | FStar_Pervasives_Native.Some bmz ->
           (match HDT_Container.parse_log_array_info a
...
let hdt_read_triples (a : HDT_Container.hdt_bytes)
  (inv : HDT_Container.hdt_inventory) :
  hdt_triples_info FStar_Pervasives_Native.option=
  parse_triples_info a inv.HDT_Container.inv_triples_data_start
...
let bm_preamble_crc8_pos (bm : HDT_Container.hdt_bitmap_info) : Prims.nat=
  HDT_Dictionary.nat_sub bm.HDT_Container.bm_data_start Prims.int_one
let bm_preamble_crc8_ok (a : HDT_Container.hdt_bytes)
  (bm : HDT_Container.hdt_bitmap_info) : Prims.bool=
  match ((HDT_Dictionary.crc8_range a bm.HDT_Container.bm_start
            (bm_preamble_len bm) Prims.int_zero),
          (HDT_Dictionary.read_u8 a (bm_preamble_crc8_pos bm)))
  with
  | (FStar_Pervasives_Native.Some c, FStar_Pervasives_Native.Some stored) ->
      c = stored
  | (uu___, uu___1) -> false
let bm_data_crc32_ok (a : HDT_Container.hdt_bytes)
  (bm : HDT_Container.hdt_bitmap_info) : Prims.bool=
  match ((HDT_Dictionary.crc32c_of_range a bm.HDT_Container.bm_data_start
            bm.HDT_Container.bm_data_bytes),
          (HDT_Dictionary.read_u32_le a
  ...
  | (FStar_Pervasives_Native.Some c, FStar_Pervasives_Native.Some stored) ->
      c = stored
  | (uu___, uu___1) -> false
let bitmap_crc_ok (a : HDT_Container.hdt_bytes)
  (bm : HDT_Container.hdt_bitmap_info) : Prims.bool=
  (bm_preamble_crc8_ok a bm) && (bm_data_crc32_ok a bm)
let triples_crc_ok (a : HDT_Container.hdt_bytes) (t : hdt_triples_info) :
  Prims.bool=
  (((((bitmap_crc_ok a t.tri_bitmap_y) && (bitmap_crc_ok a t.tri_bitmap_z))
       && (HDT_Dictionary.la_preamble_crc8_ok a t.tri_array_y))
      && (HDT_Dictionary.la_data_crc32_ok a t.tri_array_y))
     && (HDT_Dictionary.la_preamble_crc8_ok a t.tri_array_z))
    && (HDT_Dictionary.la_data_crc32_ok a t.tri_array_z)
let bit_at (a : HDT_Container.hdt_bytes) (bm : HDT_Container.hdt_bitmap_info)
  (i : Prims.nat) : Prims.bool FStar_Pervasives_Native.option=
  if i >= bm.HDT_Container.bm_numbits
  then FStar_Pervasives_Native.None
  else
    (match HDT_Dictionary.la_bits_acc a bm.HDT_Container.bm_data_start i
             Prims.int_one Prims.int_one Prims.int_zero
     with
     | FStar_Pervasives_Native.None -> FStar_Pervasives_Native.None
     | FStar_Pervasives_Native.Some v ->
         FStar_Pervasives_Native.Some (v = Prims.int_one))
let rec rank1_upto (a : HDT_Container.hdt_bytes)
  (bm : HDT_Container.hdt_bitmap_info) (pos : Prims.nat) (fuel : Prims.nat)
  ...
  then FStar_Pervasives_Native.Some acc
    ...
    (match bit_at a bm pos with
     ...
     | FStar_Pervasives_Native.Some b ->
         rank1_upto a bm (pos + Prims.int_one) (fuel - Prims.int_one)
           (if b then acc + Prims.int_one else acc))
let rank1 (a : HDT_Container.hdt_bytes) (bm : HDT_Container.hdt_bitmap_info)
  (i : Prims.nat) : Prims.nat FStar_Pervasives_Native.option=
  if i >= bm.HDT_Container.bm_numbits
  then FStar_Pervasives_Native.None
  else rank1_upto a bm Prims.int_zero (i + Prims.int_one) Prims.int_zero
let rec select1_scan (a : HDT_Container.hdt_bytes)
  (bm : HDT_Container.hdt_bitmap_info) (pos : Prims.nat) (fuel : Prims.nat)
  (target : Prims.nat) (seen : Prims.nat) :
    ...
    (match bit_at a bm pos with
     ...
     | FStar_Pervasives_Native.Some b ->
         ...
         then FStar_Pervasives_Native.Some pos
           ...
           select1_scan a bm (pos + Prims.int_one) (fuel - Prims.int_one)
             target seen')
let select1 (a : HDT_Container.hdt_bytes)
  (bm : HDT_Container.hdt_bitmap_info) (k : Prims.nat) :
  Prims.nat FStar_Pervasives_Native.option=
  select1_scan a bm Prims.int_zero bm.HDT_Container.bm_numbits
    (k + Prims.int_one) Prims.int_zero
let children_range (a : HDT_Container.hdt_bytes)
  (bm : HDT_Container.hdt_bitmap_info) (idx : Prims.nat) :
  ...
  | FStar_Pervasives_Native.Some hi ->
      ...
      then FStar_Pervasives_Native.Some (Prims.int_zero, hi)
        ...
        (match select1 a bm (idx - Prims.int_one) with
         | FStar_Pervasives_Native.None -> FStar_Pervasives_Native.None
         | FStar_Pervasives_Native.Some prev ->
             FStar_Pervasives_Native.Some ((prev + Prims.int_one), hi))
let rec collect_range (a : HDT_Container.hdt_bytes)
  ...
  then FStar_Pervasives_Native.Some (FStar_List_Tot_Base.rev acc)
    ...
    (match HDT_Dictionary.la_entry a la pos with
     | FStar_Pervasives_Native.None -> FStar_Pervasives_Native.None
     | FStar_Pervasives_Native.Some v ->
         collect_range a la (pos + Prims.int_one) (count - Prims.int_one) (v
           :: acc))
let range_count (lo : Prims.nat) (hi : Prims.nat) : Prims.nat=
  if hi >= lo then (hi - lo) + Prims.int_one else Prims.int_zero
let rec walk_y_positions (a : HDT_Container.hdt_bytes) (t : hdt_triples_info)
  ...
  then FStar_Pervasives_Native.Some acc
    ...
    (match HDT_Dictionary.la_entry a t.tri_array_y y with
     | FStar_Pervasives_Native.None -> FStar_Pervasives_Native.None
     | FStar_Pervasives_Native.Some p ->
         (match children_range a t.tri_bitmap_z y with
          | FStar_Pervasives_Native.None -> FStar_Pervasives_Native.None
          | FStar_Pervasives_Native.Some (zlo, zhi) ->
              (match collect_range a t.tri_array_z zlo (range_count zlo zhi)
                       []
               with
               | FStar_Pervasives_Native.None -> FStar_Pervasives_Native.None
               | FStar_Pervasives_Native.Some objs ->
                   let pairs = FStar_List_Tot_Base.map (fun o -> (p, o)) objs in
                   walk_y_positions a t (y + Prims.int_one)
                     (count - Prims.int_one)
                     (FStar_List_Tot_Base.op_At acc pairs))))
let hdt_triples_for_subject (a : HDT_Container.hdt_bytes)
  (t : hdt_triples_info) (subj : Prims.pos) :
  (Prims.nat * Prims.nat) Prims.list FStar_Pervasives_Native.option=
  match children_range a t.tri_bitmap_y (subj - Prims.int_one) with
  | FStar_Pervasives_Native.None -> FStar_Pervasives_Native.None
  | FStar_Pervasives_Native.Some (ylo, yhi) ->
      walk_y_positions a t ylo (range_count ylo yhi) []
let hdt_triple_count (t : hdt_triples_info) : Prims.nat=
  (t.tri_array_z).HDT_Container.la_numentries
let hdt_num_subjects (a : HDT_Container.hdt_bytes) (t : hdt_triples_info) :
  Prims.nat FStar_Pervasives_Native.option=
  let bm = t.tri_bitmap_y in
  if bm.HDT_Container.bm_numbits = Prims.int_zero
  then FStar_Pervasives_Native.Some Prims.int_zero
  else rank1 a bm (bm.HDT_Container.bm_numbits - Prims.int_one)
type hdt_id_triple = {
  it_s: Prims.nat ;
  it_p: Prims.nat ;
  it_o: Prims.nat }
...
let rec hdt_enumerate_subjects (a : HDT_Container.hdt_bytes)
  (t : hdt_triples_info) (subj : Prims.pos) (fuel : Prims.nat)
  (acc : hdt_id_triple Prims.list) :
  ...
  then FStar_Pervasives_Native.Some acc
    ...
    (match hdt_triples_for_subject a t subj with
     ...
     | FStar_Pervasives_Native.Some pairs ->
         ...
         hdt_enumerate_subjects a t (subj + Prims.int_one)
           (fuel - Prims.int_one) (FStar_List_Tot_Base.op_At acc new_triples))
let hdt_enumerate_all (a : HDT_Container.hdt_bytes) (t : hdt_triples_info) :
  hdt_id_triple Prims.list FStar_Pervasives_Native.option=
  ...
  | FStar_Pervasives_Native.Some n ->
      hdt_enumerate_subjects a t Prims.int_one n []
