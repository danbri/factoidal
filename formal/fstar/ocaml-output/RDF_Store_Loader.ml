open Prims
let merge_pair (acc : RDF_Graph.rdf_dataset) (extra : RDF_Graph.rdf_dataset)
  : RDF_Graph.rdf_dataset=
  {
    RDF_Graph.ds_default =
      (RDF_List_Helpers.append_tr acc.RDF_Graph.ds_default
         extra.RDF_Graph.ds_default);
    RDF_Graph.ds_named =
      (RDF_List_Helpers.append_tr acc.RDF_Graph.ds_named
         extra.RDF_Graph.ds_named)
  }
let merge_datasets (base : RDF_Graph.rdf_dataset)
  (extras : RDF_Graph.rdf_dataset Prims.list) : RDF_Graph.rdf_dataset=
  FStar_List_Tot_Base.fold_left merge_pair base extras
type resolved_quad =
  {
  rq_triple: RDF_Triple.triple ;
...
let rec extend_named_bucket (iri : Prims.string) (t : RDF_Triple.triple)
  (acc : RDF_Graph.named_graph Prims.list) :
  RDF_Graph.named_graph Prims.list=
  match acc with
  | [] -> [{ RDF_Graph.ng_name = iri; RDF_Graph.ng_graph = [t] }]
  | ng::rest ->
...
let rec bucket_quads_acc (default_acc : RDF_Triple.triple Prims.list)
  (named_acc : RDF_Graph.named_graph Prims.list)
...
let reverse_named_graph (ng : RDF_Graph.named_graph) : RDF_Graph.named_graph=
  {
    RDF_Graph.ng_name = (ng.RDF_Graph.ng_name);
    RDF_Graph.ng_graph = (FStar_List_Tot_Base.rev ng.RDF_Graph.ng_graph)
  }
let bucket_quads (quads : resolved_quad Prims.list) : RDF_Graph.rdf_dataset=
  ...
  | (default_rev, named_rev) ->
      ...
      { RDF_Graph.ds_default = default_g; RDF_Graph.ds_named = named_gs }
