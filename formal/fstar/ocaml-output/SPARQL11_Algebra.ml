...
let uu___is_PP_ZeroLength (projectee : property_path) : Prims.bool=
  match projectee with | PP_ZeroLength -> true | uu___ -> false
...
let rec eval_filter_with_graph (e : expr)
  (mu : RDF_Graph_Executable.solution_mapping)
  (g : RDF_Graph_Executable.rdf_graph) (ds : named_graph_dataset) :
  eval_result=
  match e with
  | E_Exists sub_p -> ER_Bool (eval_exists_fwd sub_p mu g ds)
  | E_NotExists sub_p ->
      ER_Bool (Prims.op_Negation (eval_exists_fwd sub_p mu g ds))
  | E_And (e1, e2) ->
      ER_Bool
        ((ebv (eval_filter_with_graph e1 mu g ds)) &&
           (ebv (eval_filter_with_graph e2 mu g ds)))
  | E_Or (e1, e2) ->
      ER_Bool
        ((ebv (eval_filter_with_graph e1 mu g ds)) ||
           (ebv (eval_filter_with_graph e2 mu g ds)))
  | E_Not e1 ->
      ER_Bool (Prims.op_Negation (ebv (eval_filter_with_graph e1 mu g ds)))
...
let rec eval_pattern (p : group_graph_pattern)
  ...
  | GP_Filter (e, p') ->
      ...
      FStar_List_Tot_Base.filter
        (fun mu -> ebv (eval_filter_with_graph e mu g ds)) omega
  ...
  | GP_Graph (gt, p') ->
      (match gt with
       | PT_IRI iri ->
           (match FStar_List_Tot_Base.assoc (iri_to_string iri) ds with
                    ...
                    FStar_List_Tot_Base.map
                      (fun mu ->
                         ...
                         | FStar_Pervasives_Native.Some iri ->
                             sm_bind v (RDF_Graph_Executable.T_IRI iri) mu
                         | FStar_Pervasives_Native.None -> mu) omega) ds
       | uu___ -> [])
  | GP_Service (uu___, uu___1, uu___2) -> []
  | GP_SubSelect q -> eval_select_query_fwd q g ds
                 ...
                 | PS_IRI iri ->
                     if
                       RDF_Graph_Executable.rdf_term_eq
                         (RDF_Graph_Executable.T_IRI iri) s
                     then FStar_Pervasives_Native.Some mu0
                     else FStar_Pervasives_Native.None
                 | PS_Var v -> FStar_Pervasives_Native.Some (sm_bind v s mu0) in
               (match mu1 with
                | FStar_Pervasives_Native.None -> []
                | FStar_Pervasives_Native.Some mu11 ->
                    let mu2 = try_bind_term pt o mu11 in
                    (match mu2 with
                     | FStar_Pervasives_Native.None -> []
                     | FStar_Pervasives_Native.Some mu21 -> [mu21]))) pairs
...
let eval_select_query (q : query) (g : RDF_Graph_Executable.rdf_graph)
  (ds : named_graph_dataset) : solution_sequence=
  match q.q_form with
  | QF_Select sel ->
...
let () = eval_select_query_fwd_ref := (fun q g ds -> eval_select_query q g ds)
...
let rec eval_property_path (p : property_path)
  ...
  | PP_ZeroOrMore pp ->
      let reflexive =
        FStar_List_Tot_Base.map (fun n -> (n, n)) (graph_nodes g) in
      let step = eval_property_path pp g in
      let rec closure acc frontier fuel =
        if fuel = Prims.int_zero
        then acc
        else
          (let new_pairs =
                      ...
                      FStar_List_Tot_Base.concatMap
                        (fun s ->
                           ...
                           | (mid2, o) ->
                               ...
                               then [((FStar_Pervasives_Native.fst pair), o)]
                               else []) step) frontier in
           let novel =
             FStar_List_Tot_Base.filter
               (fun p1 ->
                  Prims.op_Negation
                    (FStar_List_Tot_Base.existsb (path_pair_eq p1) acc))
               new_pairs in
           if Prims.uu___is_Nil novel
           then acc
           else
             closure (FStar_List_Tot_Base.op_At acc novel) novel
               (fuel - Prims.int_one)) in
      let n_nodes = FStar_List_Tot_Base.length (graph_nodes g) in
      dedup_path
        (closure (FStar_List_Tot_Base.op_At reflexive step) step n_nodes)
  | PP_OneOrMore pp ->
      let step = eval_property_path pp g in
      let rec closure acc frontier fuel =
        if fuel = Prims.int_zero
  ...
  | PP_ZeroLength ->
      FStar_List_Tot_Base.map (fun n -> (n, n)) (graph_nodes g)
let () = eval_property_path_fwd_ref := (fun p g -> eval_property_path p g)
...
let eval_exists (pattern : group_graph_pattern)
  (mu : RDF_Graph_Executable.solution_mapping)
  (graph : RDF_Graph_Executable.rdf_graph) (ds : named_graph_dataset) :
  Prims.bool=
...
let eval_not_exists (pattern : group_graph_pattern)
  (mu : RDF_Graph_Executable.solution_mapping)
  (graph : RDF_Graph_Executable.rdf_graph) (ds : named_graph_dataset) :
  Prims.bool= Prims.op_Negation (eval_exists pattern mu graph ds)
let () = eval_exists_fwd_ref := (fun p mu g ds -> eval_exists p mu g ds)
