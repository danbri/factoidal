...
let rec crc8_range (a : HDT_Container.hdt_bytes) (pos : Prims.nat)
  (count : Prims.nat) (crc : Prims.nat) :
  Prims.nat FStar_Pervasives_Native.option=
  if count = Prims.int_zero
  then FStar_Pervasives_Native.Some crc
  else
    (match HDT_Container.byte_get a pos with
     | FStar_Pervasives_Native.None -> FStar_Pervasives_Native.None
     | FStar_Pervasives_Native.Some b ->
         crc8_range a (pos + Prims.int_one) (count - Prims.int_one)
           (crc8_byte crc b))
...
let rec crc32c_range (a : HDT_Container.hdt_bytes) (pos : Prims.nat)
  (count : Prims.nat) (crc : Prims.nat) :
  Prims.nat FStar_Pervasives_Native.option=
  if count = Prims.int_zero
  then FStar_Pervasives_Native.Some crc
    ...
    (match HDT_Container.byte_get a pos with
     | FStar_Pervasives_Native.None -> FStar_Pervasives_Native.None
     | FStar_Pervasives_Native.Some b ->
         crc32c_range a (pos + Prims.int_one) (count - Prims.int_one)
           (crc32c_byte crc b))
let crc32c_of_range (a : HDT_Container.hdt_bytes) (pos : Prims.nat)
  (count : Prims.nat) : Prims.nat FStar_Pervasives_Native.option=
  match crc32c_range a pos count (Prims.parse_int "0xFFFFFFFF") with
  | FStar_Pervasives_Native.None -> FStar_Pervasives_Native.None
  | FStar_Pervasives_Native.Some c ->
      FStar_Pervasives_Native.Some
        (HDT_Container.nat_xor c (Prims.parse_int "0xFFFFFFFF")
           (Prims.of_int (32)))
let read_u8 (a : HDT_Container.hdt_bytes) (pos : Prims.nat) :
  Prims.nat FStar_Pervasives_Native.option=
  match HDT_Container.byte_get a pos with
  | FStar_Pervasives_Native.None -> FStar_Pervasives_Native.None
  | FStar_Pervasives_Native.Some b -> FStar_Pervasives_Native.Some b
let read_u32_le (a : HDT_Container.hdt_bytes) (pos : Prims.nat) :
  Prims.nat FStar_Pervasives_Native.option=
  match ((HDT_Container.byte_get a pos),
          (HDT_Container.byte_get a (pos + Prims.int_one)),
          (HDT_Container.byte_get a (pos + (Prims.of_int (2)))),
          (HDT_Container.byte_get a (pos + (Prims.of_int (3)))))
  with
...
let la_preamble_crc8_ok (a : HDT_Container.hdt_bytes)
  (la : HDT_Container.hdt_log_array_info) : Prims.bool=
  match ((crc8_range a la.HDT_Container.la_start (la_preamble_len la)
            Prims.int_zero), (read_u8 a (la_preamble_crc8_pos la)))
  with
  | (FStar_Pervasives_Native.Some c, FStar_Pervasives_Native.Some stored) ->
      c = stored
  | (uu___, uu___1) -> false
let la_data_crc32_ok (a : HDT_Container.hdt_bytes)
  (la : HDT_Container.hdt_log_array_info) : Prims.bool=
  match ((crc32c_of_range a la.HDT_Container.la_data_start
            la.HDT_Container.la_data_bytes),
          (read_u32_le a (la_crc32_pos la)))
  with
  | (FStar_Pervasives_Native.Some c, FStar_Pervasives_Native.Some stored) ->
      c = stored
  | (uu___, uu___1) -> false
...
let pfc_preamble_crc8_ok (a : HDT_Container.hdt_bytes)
  (sec : HDT_Container.hdt_pfc_section) : Prims.bool=
  match ((crc8_range a sec.HDT_Container.pfc_start (pfc_preamble_len sec)
            Prims.int_zero), (read_u8 a (pfc_preamble_crc8_pos sec)))
  with
  | (FStar_Pervasives_Native.Some c, FStar_Pervasives_Native.Some stored) ->
      c = stored
  | (uu___, uu___1) -> false
let pfc_packed_crc32_ok (a : HDT_Container.hdt_bytes)
  (sec : HDT_Container.hdt_pfc_section) : Prims.bool=
  match ((crc32c_of_range a sec.HDT_Container.pfc_packed_start
  ...
  | (FStar_Pervasives_Native.Some c, FStar_Pervasives_Native.Some stored) ->
      c = stored
  | (uu___, uu___1) -> false
let pfc_section_crc_ok (a : HDT_Container.hdt_bytes)
  (sec : HDT_Container.hdt_pfc_section) : Prims.bool=
  (((pfc_preamble_crc8_ok a sec) &&
      (la_preamble_crc8_ok a sec.HDT_Container.pfc_blocks))
     && (la_data_crc32_ok a sec.HDT_Container.pfc_blocks))
...
let rec la_bits_acc (a : HDT_Container.hdt_bytes) (data_start : Prims.nat)
  ...
  then FStar_Pervasives_Native.Some acc
    ...
    (let byte_idx = data_start + (bitpos / (Prims.of_int (8))) in
     ...
     match HDT_Container.byte_get a byte_idx with
     | FStar_Pervasives_Native.None -> FStar_Pervasives_Native.None
     | FStar_Pervasives_Native.Some b ->
         let bitval = (mod) (b / (bit_divisor shift)) (Prims.of_int (2)) in
         la_bits_acc a data_start (bitpos + Prims.int_one)
           (nbits - Prims.int_one) (mult * (Prims.of_int (2)))
           (acc + (bitval * mult)))
let la_entry (a : HDT_Container.hdt_bytes)
  (la : HDT_Container.hdt_log_array_info) (idx : Prims.nat) :
  Prims.nat FStar_Pervasives_Native.option=
  la_bits_acc a la.HDT_Container.la_data_start
...
let hdt_pfc_num_blocks (sec : HDT_Container.hdt_pfc_section) : Prims.nat=
  ...
  then Prims.int_zero
    ...
    (sec.HDT_Container.pfc_blocks).HDT_Container.la_numentries -
      Prims.int_one
let pfc_block_abs_start (a : HDT_Container.hdt_bytes)
  (sec : HDT_Container.hdt_pfc_section) (b : Prims.nat) :
  Prims.nat FStar_Pervasives_Native.option=
  match la_entry a sec.HDT_Container.pfc_blocks b with
  | FStar_Pervasives_Native.None -> FStar_Pervasives_Native.None
  | FStar_Pervasives_Native.Some rel ->
      FStar_Pervasives_Native.Some (sec.HDT_Container.pfc_packed_start + rel)
let pfc_read_first (a : HDT_Container.hdt_bytes) (pos : Prims.nat) :
  (Prims.string * Prims.nat) FStar_Pervasives_Native.option=
  match HDT_Container.scan_nul a pos (HDT_Container.byte_len a) with
  | FStar_Pervasives_Native.None -> FStar_Pervasives_Native.None
  | FStar_Pervasives_Native.Some nul_pos ->
      (match HDT_Container.bytes_to_string a pos (nul_pos - pos) with
       | FStar_Pervasives_Native.None -> FStar_Pervasives_Native.None
       | FStar_Pervasives_Native.Some str ->
           FStar_Pervasives_Native.Some (str, (nul_pos + Prims.int_one)))
let pfc_read_suffix (a : HDT_Container.hdt_bytes) (pos : Prims.nat)
  (prev : Prims.string) :
  (Prims.string * Prims.nat) FStar_Pervasives_Native.option=
  match HDT_Container.vbyte_decode a pos with
  | FStar_Pervasives_Native.None -> FStar_Pervasives_Native.None
  | FStar_Pervasives_Native.Some (plen, p1) ->
      if plen > (FStar_String.strlen prev)
      then FStar_Pervasives_Native.None
      else
        (match HDT_Container.scan_nul a p1 (HDT_Container.byte_len a) with
         | FStar_Pervasives_Native.None -> FStar_Pervasives_Native.None
         | FStar_Pervasives_Native.Some nul_pos ->
             (match HDT_Container.bytes_to_string a p1 (nul_pos - p1) with
              | FStar_Pervasives_Native.None -> FStar_Pervasives_Native.None
...
let rec pfc_walk_suffixes (a : HDT_Container.hdt_bytes) (at : Prims.nat)
  ...
  | FStar_Pervasives_Native.Some (str, at') ->
      ...
      else pfc_walk_suffixes a at' str (remaining - Prims.int_one)
let rec decode_block_acc (a : HDT_Container.hdt_bytes) (pos : Prims.nat)
  ...
  then FStar_Pervasives_Native.Some (FStar_List_Tot_Base.rev acc)
    ...
    (match pfc_read_suffix a pos prev with
     ...
     | FStar_Pervasives_Native.Some (str, pos') ->
         decode_block_acc a pos' str (remaining - Prims.int_one) (str :: acc))
let decode_block (a : HDT_Container.hdt_bytes) (block_start : Prims.nat)
  (count : Prims.nat) :
  Prims.string Prims.list FStar_Pervasives_Native.option=
  if count = Prims.int_zero
  then FStar_Pervasives_Native.None
    ...
    (match pfc_read_first a block_start with
     ...
     | FStar_Pervasives_Native.Some (first, pos1) ->
         decode_block_acc a pos1 first (count - Prims.int_one) [first])
...
let rec decode_section_acc (a : HDT_Container.hdt_bytes)
     ...
     | FStar_Pervasives_Native.Some bstart ->
         ...
         (match decode_block a bstart count with
          ...
          | FStar_Pervasives_Native.Some strs ->
              decode_section_acc a sec (block_idx + Prims.int_one) numblocks
                (FStar_List_Tot_Base.op_At acc strs)))
let decode_section (a : HDT_Container.hdt_bytes)
  (sec : HDT_Container.hdt_pfc_section) :
  Prims.string Prims.list FStar_Pervasives_Native.option=
  if sec.HDT_Container.pfc_numstrings = Prims.int_zero
  then FStar_Pervasives_Native.Some []
  else decode_section_acc a sec Prims.int_zero (hdt_pfc_num_blocks sec) []
let pfc_extract (a : HDT_Container.hdt_bytes)
  (sec : HDT_Container.hdt_pfc_section) (id : Prims.pos) :
  Prims.string FStar_Pervasives_Native.option=
  if
...
let pfc_block_first_string (a : HDT_Container.hdt_bytes)
  (sec : HDT_Container.hdt_pfc_section) (block_idx : Prims.nat) :
  Prims.string FStar_Pervasives_Native.option=
  match pfc_block_abs_start a sec block_idx with
  | FStar_Pervasives_Native.None -> FStar_Pervasives_Native.None
...
let rec pfc_find_block (a : HDT_Container.hdt_bytes)
  ...
  then FStar_Pervasives_Native.Some lo
    ...
    (let mid = ((lo + hi) + Prims.int_one) / (Prims.of_int (2)) in
     ...
     | FStar_Pervasives_Native.Some head ->
         ...
         else pfc_find_block a sec target lo (mid - Prims.int_one))
let pfc_locate (a : HDT_Container.hdt_bytes)
  (sec : HDT_Container.hdt_pfc_section) (target : Prims.string) :
  ...
  then FStar_Pervasives_Native.None
    ...
    (match pfc_find_block a sec target Prims.int_zero
                    ...
                    | FStar_Pervasives_Native.Some k ->
                        FStar_Pervasives_Native.Some
                          (((block_idx * sec.HDT_Container.pfc_blocksize) + k)
                             + Prims.int_one)))))
let hdt_term_of_string (dstr : Prims.string) :
  RDF_Term.rdf_term FStar_Pervasives_Native.option=
  let len = FStar_String.strlen dstr in
  if len = Prims.int_zero
  then FStar_Pervasives_Native.None
  else
    (let c0 = FStar_String.index dstr Prims.int_zero in
     if
       (((FStar_Char.int_of_char c0) = (Prims.of_int (0x5F))) &&
          (len > Prims.int_one))
         &&
         ((FStar_Char.int_of_char (FStar_String.index dstr Prims.int_one)) =
            (Prims.of_int (0x3A)))
     then
       match Parser_NTriples.parse_bnode dstr Prims.int_zero with
       ...
       then
         (match Parser_NTriples.parse_literal dstr Prims.int_zero with
         ...
         if RDF_Term.is_iri dstr
         then FStar_Pervasives_Native.Some (RDF_Term.T_IRI dstr)
         else FStar_Pervasives_Native.None)
...
let hdt_id_to_term (a : HDT_Container.hdt_bytes)
  (inv : HDT_Container.hdt_inventory) (role : hdt_role) (id : Prims.pos) :
  RDF_Term.rdf_term FStar_Pervasives_Native.option=
  let nshared =
  ...
  let extracted =
    match role with
    | Role_Predicate ->
        pfc_extract a inv.HDT_Container.inv_dict_predicates id
    | Role_Subject ->
        if id <= nshared
        then pfc_extract a inv.HDT_Container.inv_dict_shared id
        else pfc_extract a inv.HDT_Container.inv_dict_subjects (id - nshared)
    | Role_Object ->
        if id <= nshared
        then pfc_extract a inv.HDT_Container.inv_dict_shared id
        else pfc_extract a inv.HDT_Container.inv_dict_objects (id - nshared) in
  match extracted with
  | FStar_Pervasives_Native.None -> FStar_Pervasives_Native.None
  | FStar_Pervasives_Native.Some str -> hdt_term_of_string str
let hdt_term_to_id (a : HDT_Container.hdt_bytes)
  (inv : HDT_Container.hdt_inventory) (role : hdt_role)
  (term : RDF_Term.rdf_term) : Prims.pos FStar_Pervasives_Native.option=
  let dstr = hdt_string_of_term term in
  let nshared =
    (inv.HDT_Container.inv_dict_shared).HDT_Container.pfc_numstrings in
  match role with
  | Role_Predicate -> pfc_locate a inv.HDT_Container.inv_dict_predicates dstr
  | Role_Subject ->
      (match pfc_locate a inv.HDT_Container.inv_dict_shared dstr with
       | FStar_Pervasives_Native.Some id -> FStar_Pervasives_Native.Some id
       | FStar_Pervasives_Native.None ->
           (match pfc_locate a inv.HDT_Container.inv_dict_subjects dstr with
            | FStar_Pervasives_Native.None -> FStar_Pervasives_Native.None
  ...
  | Role_Object ->
      (match pfc_locate a inv.HDT_Container.inv_dict_shared dstr with
       | FStar_Pervasives_Native.Some id -> FStar_Pervasives_Native.Some id
       | FStar_Pervasives_Native.None ->
           (match pfc_locate a inv.HDT_Container.inv_dict_objects dstr with
            | FStar_Pervasives_Native.None -> FStar_Pervasives_Native.None
            | FStar_Pervasives_Native.Some id ->
                FStar_Pervasives_Native.Some (nshared + id)))
