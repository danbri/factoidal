...
let jexp_is_framing_keyword (k : Prims.string) : Prims.bool=
...
let rec jexp_string_has_space (s : Prims.string) (pos : Prims.nat)
  ...
  then false
    ...
    (let n = Parser_FastString.fs_byte_length s in
     ...
     then false
       ...
       if (Parser_JSON.jbyte_at s pos) = (Prims.of_int (0x20))
       then true
       else
         jexp_string_has_space s (pos + Prims.int_one) (fuel - Prims.int_one))
              ...
              (let lang =
               ...
               let typ_expanded =
                 ...
                 | FStar_Pervasives_Native.Some t ->
                     JSONLD_Context.expand_iri ac t true
                 | FStar_Pervasives_Native.None ->
                 ...
                 if
                   (match v with
                    | Parser_JSON.JArray uu___6 ->
                        typ_expanded <>
                          (FStar_Pervasives_Native.Some "@json")
                   ...
                   then FStar_Pervasives_Native.None
                     ...
                     (let dir =
                      ...
                      let idx =
                        match jexp_find_aliased_field ac "@index" fields with
                        | FStar_Pervasives_Native.Some
                            (uu___8, Parser_JSON.JString s) ->
                            [("@index", (Parser_JSON.JString s))]
                        | uu___8 -> [] in
                      match (dir, typ) with
                                    ...
                                    then FStar_Pervasives_Native.None
                                      ...
                                      FStar_Pervasives_Native.Some
                                        (Parser_JSON.JObject (("@value", v)
                                           ::
                                           ("@type",
                                             (Parser_JSON.JString iri))
                                           :: idx)))
                           | (FStar_Pervasives_Native.None,
                              FStar_Pervasives_Native.None) ->
                               FStar_Pervasives_Native.Some
                                 (Parser_JSON.JObject (("@value", v) :: idx)))))
let rec jexp_canon_pattern_fields (ac : JSONLD_Context.active_context)
  (fields : (Prims.string * Parser_JSON.json_val) Prims.list) :
  ...
  | (k, v)::rest ->
      (match JSONLD_Context.expand_iri ac k true with
       | FStar_Pervasives_Native.Some e -> (e, v) ::
           (jexp_canon_pattern_fields ac rest)
       | FStar_Pervasives_Native.None -> (k, v) ::
           (jexp_canon_pattern_fields ac rest))
let jexp_json_val_is_array_or_object (v : Parser_JSON.json_val) : Prims.bool=
  match v with
  | Parser_JSON.JArray uu___ -> true
  | Parser_JSON.JObject uu___ -> true
  | uu___ -> false
let jexp_expand_value_object_frame (ac : JSONLD_Context.active_context)
  (fields : (Prims.string * Parser_JSON.json_val) Prims.list) :
  Parser_JSON.json_val FStar_Pervasives_Native.option=
  if Prims.op_Negation (jexp_value_object_keys_valid ac fields)
  then FStar_Pervasives_Native.None
    ...
    if
      (match jexp_find_aliased_field ac "@index" fields with
       | FStar_Pervasives_Native.Some (uu___1, Parser_JSON.JString uu___2) ->
           false
       | FStar_Pervasives_Native.Some uu___1 -> true
       | FStar_Pervasives_Native.None -> false)
    then FStar_Pervasives_Native.None
    else
      (let is_pattern_shaped name =
         match jexp_find_aliased_field ac name fields with
         | FStar_Pervasives_Native.Some (uu___2, v) ->
             jexp_json_val_is_array_or_object v
         | FStar_Pervasives_Native.None -> false in
       if
         ((is_pattern_shaped "@value") || (is_pattern_shaped "@type")) ||
           (is_pattern_shaped "@language")
       then
         FStar_Pervasives_Native.Some
           (Parser_JSON.JObject (jexp_canon_pattern_fields ac fields))
       ...
       | FStar_Pervasives_Native.None ->
           (match JSONLD_Context.expand_iri ac t false with
            | FStar_Pervasives_Native.Some iri -> (Parser_JSON.JString iri)
                :: (jexp_expand_type_items ac rest)
            | FStar_Pervasives_Native.None ->
                if JSONLD_Context.jldctx_keyword_lookalike t
                then jexp_expand_type_items ac rest
                else (Parser_JSON.JString t) ::
                  (jexp_expand_type_items ac rest)))
...
let rec jexp_id_frame_entries_valid (items : Parser_JSON.json_val Prims.list)
  : Prims.bool=
  match items with
  | [] -> true
  | (Parser_JSON.JString uu___)::rest -> jexp_id_frame_entries_valid rest
  | (Parser_JSON.JObject [])::rest -> jexp_id_frame_entries_valid rest
  | uu___ -> false
let rec jexp_expand_id_frame_items (ac : JSONLD_Context.active_context)
  (items : Parser_JSON.json_val Prims.list) :
  Parser_JSON.json_val Prims.list=
  match items with
  | [] -> []
  | (Parser_JSON.JString s)::rest ->
      (match JSONLD_Context.expand_iri ac s false with
       | FStar_Pervasives_Native.Some iri -> (Parser_JSON.JString iri) ::
           (jexp_expand_id_frame_items ac rest)
       | FStar_Pervasives_Native.None -> (Parser_JSON.JString s) ::
           (jexp_expand_id_frame_items ac rest))
  | (Parser_JSON.JObject [])::rest -> (Parser_JSON.JObject []) ::
      (jexp_expand_id_frame_items ac rest)
  | uu___::rest -> jexp_expand_id_frame_items ac rest
let apply_property_scoped_context (ac : JSONLD_Context.active_context)
  ...
  | FStar_Pervasives_Native.Some td ->
      (match td.JSONLD_Context.td_scoped_context with
       | FStar_Pervasives_Native.Some (scoped, def_doc_url) ->
           (match JSONLD_Context.apply_context_with_propagate
                    {
                      JSONLD_Context.ac_terms = (ac.JSONLD_Context.ac_terms);
                      JSONLD_Context.ac_vocab = (ac.JSONLD_Context.ac_vocab);
                      JSONLD_Context.ac_base = (ac.JSONLD_Context.ac_base);
                      JSONLD_Context.ac_language =
                        (ac.JSONLD_Context.ac_language);
                      JSONLD_Context.ac_direction =
                        (ac.JSONLD_Context.ac_direction);
                      JSONLD_Context.ac_previous =
                        (ac.JSONLD_Context.ac_previous);
                      JSONLD_Context.ac_mode10 =
                        (ac.JSONLD_Context.ac_mode10);
                      JSONLD_Context.ac_doc_url = def_doc_url;
                      JSONLD_Context.ac_original_base =
                        (ac.JSONLD_Context.ac_original_base);
                      JSONLD_Context.ac_suppress_pop =
                        (ac.JSONLD_Context.ac_suppress_pop);
                      JSONLD_Context.ac_frame_expansion =
                        (ac.JSONLD_Context.ac_frame_expansion)
                    } scoped true true
            with
            | FStar_Pervasives_Native.None -> FStar_Pervasives_Native.None
                  ...
                  FStar_Pervasives_Native.Some
                    {
                      ...
                      JSONLD_Context.ac_doc_url =
                        (ac_eff.JSONLD_Context.ac_doc_url);
                      JSONLD_Context.ac_original_base =
                        (ac_eff.JSONLD_Context.ac_original_base);
                      JSONLD_Context.ac_suppress_pop =
                        (ac_eff.JSONLD_Context.ac_suppress_pop);
                      JSONLD_Context.ac_frame_expansion =
                        (ac_eff.JSONLD_Context.ac_frame_expansion)
                    }
                else
                  FStar_Pervasives_Native.Some
                    {
                      ...
                      JSONLD_Context.ac_original_base =
                        (ac_eff.JSONLD_Context.ac_original_base);
                      JSONLD_Context.ac_suppress_pop = true;
                      JSONLD_Context.ac_frame_expansion =
                        (ac_eff.JSONLD_Context.ac_frame_expansion)
                    })
...
let jexp_language_map_item (key : Prims.string) (is_none : Prims.bool)
  (dir : Prims.string FStar_Pervasives_Native.option)
...
let rec jexp_language_map_entry_items (key : Prims.string)
  (is_none : Prims.bool) (dir : Prims.string FStar_Pervasives_Native.option)
  (items : Parser_JSON.json_val Prims.list) :
  Parser_JSON.json_val Prims.list=
  match items with
  | [] -> []
  | v::rest ->
      (match jexp_language_map_item key is_none dir v with
       | FStar_Pervasives_Native.Some it -> it ::
           (jexp_language_map_entry_items key is_none dir rest)
       | FStar_Pervasives_Native.None ->
           jexp_language_map_entry_items key is_none dir rest)
let rec jexp_expand_language_map (ac : JSONLD_Context.active_context)
  (dir : Prims.string FStar_Pervasives_Native.option)
  (entries : (Prims.string * Parser_JSON.json_val) Prims.list) :
  Parser_JSON.json_val Prims.list=
  match entries with
  ...
  | (k, v)::rest ->
      let is_none =
        (k = "@none") ||
          (match JSONLD_Context.expand_iri ac k true with
           | FStar_Pervasives_Native.Some "@none" -> true
           | uu___ -> false) in
      FStar_List_Tot_Base.append
        (jexp_language_map_entry_items k is_none dir (jexp_as_array v))
...
let jexp_set_id_if_absent (iri : Prims.string) (item : Parser_JSON.json_val)
  ...
  | uu___ -> item
let rec jexp_prepend_type_existing (kiri : Prims.string)
  (fields : (Prims.string * Parser_JSON.json_val) Prims.list) :
  (Prims.string * Parser_JSON.json_val) Prims.list
    FStar_Pervasives_Native.option=
  match fields with
  | [] -> FStar_Pervasives_Native.None
  | ("@type", Parser_JSON.JArray ts)::rest ->
      FStar_Pervasives_Native.Some
        (("@type", (Parser_JSON.JArray ((Parser_JSON.JString kiri) :: ts)))
        :: rest)
  | kv::rest ->
      (match jexp_prepend_type_existing kiri rest with
       | FStar_Pervasives_Native.None -> FStar_Pervasives_Native.None
       | FStar_Pervasives_Native.Some r ->
           FStar_Pervasives_Native.Some (kv :: r))
...
let jexp_inject_index_field (index_iri : Prims.string)
  ...
  | Parser_JSON.JObject fields ->
      ...
      then FStar_Pervasives_Native.None
        ...
        FStar_Pervasives_Native.Some
          (Parser_JSON.JObject ((index_iri, (Parser_JSON.JArray [keyval])) ::
             fields))
  | uu___ -> FStar_Pervasives_Native.None
...
let jexp_is_single_id_object (ac : JSONLD_Context.active_context)
  ...
  | (k, uu___)::[] ->
      (match JSONLD_Context.expand_iri ac k true with
       ...
       | FStar_Pervasives_Native.None -> false)
  | uu___ -> false
let rec jexp_keyword_aliases_of (ac : JSONLD_Context.active_context)
  (fields : (Prims.string * Parser_JSON.json_val) Prims.list) :
  Prims.string Prims.list=
  match fields with
  | [] -> []
  | (k, uu___)::rest ->
      (match JSONLD_Context.expand_iri ac k true with
       | FStar_Pervasives_Native.Some e ->
           if e = "@id"
           then e :: (jexp_keyword_aliases_of ac rest)
           else jexp_keyword_aliases_of ac rest
       | FStar_Pervasives_Native.None -> jexp_keyword_aliases_of ac rest)
let rec jexp_has_dup_string (xs : Prims.string Prims.list) : Prims.bool=
  match xs with
  | [] -> false
  | x::rest -> (FStar_List_Tot_Base.mem x rest) || (jexp_has_dup_string rest)
let jexp_has_colliding_keywords (ac : JSONLD_Context.active_context)
  (fields : (Prims.string * Parser_JSON.json_val) Prims.list) : Prims.bool=
  jexp_has_dup_string (jexp_keyword_aliases_of ac fields)
let rec jexp_map_entry_insert (kv : (Prims.string * Parser_JSON.json_val))
...
let rec jexp_sort_map_entries
  (xs : (Prims.string * Parser_JSON.json_val) Prims.list) :
  (Prims.string * Parser_JSON.json_val) Prims.list=
  match xs with
  | [] -> []
  | kv::rest -> jexp_map_entry_insert kv (jexp_sort_map_entries rest)
let rec jexp_partition_nest (ac : JSONLD_Context.active_context)
  (fields : (Prims.string * Parser_JSON.json_val) Prims.list) :
  ...
  | [] -> ([], [])
  | (k, v)::rest ->
      let uu___ = jexp_partition_nest ac rest in
      (match uu___ with
       | (others, nests) ->
           let is_nest =
             (k = "@nest") ||
               (match JSONLD_Context.expand_iri ac k true with
                | FStar_Pervasives_Native.Some "@nest" -> true
                | uu___1 -> false) in
           if is_nest
           then (others, ((k, v) :: nests))
           else (((k, v) :: others), nests))
let jexp_order_node_fields (ac : JSONLD_Context.active_context)
  (fields : (Prims.string * Parser_JSON.json_val) Prims.list) :
  (Prims.string * Parser_JSON.json_val) Prims.list=
  let sorted = jexp_sort_map_entries fields in
  let uu___ = jexp_partition_nest ac sorted in
  match uu___ with
  | (others, nests) -> FStar_List_Tot_Base.append others nests
let jexp_add_index_to_item (k : Prims.string) (item : Parser_JSON.json_val) :
  Parser_JSON.json_val=
  match item with
  | Parser_JSON.JObject fields ->
      if jexp_has_field "@index" fields
      then item
      else
        Parser_JSON.JObject (("@index", (Parser_JSON.JString k)) :: fields)
  | uu___ -> item
let rec jexp_add_index_items (k : Prims.string)
  (items : Parser_JSON.json_val Prims.list) :
  Parser_JSON.json_val Prims.list=
  match items with
  | [] -> []
  | it::rest -> (jexp_add_index_to_item k it) ::
      (jexp_add_index_items k rest)
let rec jexp_collect_dup_key (k : Prims.string)
  (fields : (Prims.string * Parser_JSON.json_val) Prims.list) :
  (Parser_JSON.json_val Prims.list * (Prims.string * Parser_JSON.json_val)
    Prims.list)=
  match fields with
  | [] -> ([], [])
  | (k2, v)::rest ->
      let uu___ = jexp_collect_dup_key k rest in
      (match uu___ with
       | (vs, others) ->
           if k2 = k then ((v :: vs), others) else (vs, ((k2, v) :: others)))
let rec jexp_merge_dup_values (v : Parser_JSON.json_val)
  (vs : Parser_JSON.json_val Prims.list) : Parser_JSON.json_val=
  match vs with
  | [] -> v
  | v2::rest ->
      let m =
        ...
        | (Parser_JSON.JArray xs, Parser_JSON.JArray ys) ->
            Parser_JSON.JArray (FStar_List_Tot_Base.append xs ys)
        | (Parser_JSON.JObject xs, Parser_JSON.JObject ys) ->
            Parser_JSON.JObject (FStar_List_Tot_Base.append xs ys)
        | (uu___, uu___1) -> v in
      jexp_merge_dup_values m rest
let rec jexp_merge_dup_fields
  (fields : (Prims.string * Parser_JSON.json_val) Prims.list)
  ...
  then fields
    ...
    (match fields with
     | [] -> []
     | (k, v)::rest ->
         let uu___1 = jexp_collect_dup_key k rest in
         (match uu___1 with
          | (vs, others) ->
              let merged = jexp_merge_dup_values v vs in
              let merged1 =
                match merged with
                | Parser_JSON.JObject inner ->
                    if k = "@reverse"
                    then
                      Parser_JSON.JObject
                        (jexp_merge_dup_fields inner (fuel - Prims.int_one))
                    else merged
                | uu___2 -> merged in
              (k, merged1) ::
                (jexp_merge_dup_fields others (fuel - Prims.int_one))))
let rec jexp_merge_item_fields (items : Parser_JSON.json_val Prims.list) :
  Parser_JSON.json_val Prims.list=
  match items with
  | [] -> []
  | (Parser_JSON.JObject fs)::rest ->
      (Parser_JSON.JObject
         (jexp_merge_dup_fields fs
            ((Parser_JSON.json_size (Parser_JSON.JObject fs)) +
               (Prims.of_int (8)))))
      :: (jexp_merge_item_fields rest)
  | v::rest -> v :: (jexp_merge_item_fields rest)
let jexp_free_floating (v : Parser_JSON.json_val) : Prims.bool=
  match v with
  | Parser_JSON.JObject [] -> true
  | Parser_JSON.JObject fields ->
      ((jexp_has_field "@value" fields) || (jexp_has_field "@list" fields))
        ||
        ((match fields with | (k, uu___)::[] -> k = "@id" | uu___ -> false))
  | uu___ -> false
let rec jexp_drop_ff_items (items : Parser_JSON.json_val Prims.list)
  ...
  then items
    ...
    (match items with
     | [] -> []
     | v::rest ->
         let v1 = jexp_drop_ff_node v (fuel - Prims.int_one) in
         if jexp_free_floating v1
         then jexp_drop_ff_items rest (fuel - Prims.int_one)
         else v1 :: (jexp_drop_ff_items rest (fuel - Prims.int_one)))
and jexp_drop_ff_node (v : Parser_JSON.json_val) (fuel : Prims.nat) :
  Parser_JSON.json_val=
  if fuel = Prims.int_zero
  then v
  else
    (match v with
     | Parser_JSON.JObject fields ->
         Parser_JSON.JObject
           (jexp_drop_ff_fields fields (fuel - Prims.int_one))
     | uu___1 -> v)
and jexp_drop_ff_fields
  (fields : (Prims.string * Parser_JSON.json_val) Prims.list)
  (fuel : Prims.nat) : (Prims.string * Parser_JSON.json_val) Prims.list=
  if fuel = Prims.int_zero
  then fields
    ...
    (match fields with
     | [] -> []
     | (k, Parser_JSON.JArray items)::rest ->
         if k = "@graph"
         then
           (k,
             (Parser_JSON.JArray
                (jexp_drop_ff_items items (fuel - Prims.int_one))))
           :: (jexp_drop_ff_fields rest (fuel - Prims.int_one))
         else
           (k,
             (Parser_JSON.JArray
                (jexp_drop_ff_walk_items items (fuel - Prims.int_one))))
           :: (jexp_drop_ff_fields rest (fuel - Prims.int_one))
     | kv::rest -> kv :: (jexp_drop_ff_fields rest (fuel - Prims.int_one)))
and jexp_drop_ff_walk_items (items : Parser_JSON.json_val Prims.list)
  (fuel : Prims.nat) : Parser_JSON.json_val Prims.list=
  if fuel = Prims.int_zero
  then items
  else
    (match items with
     | [] -> []
     | v::rest -> (jexp_drop_ff_node v (fuel - Prims.int_one)) ::
         (jexp_drop_ff_walk_items rest (fuel - Prims.int_one)))
  ...
  then FStar_Pervasives_Native.None
    ...
    (match v with
     | Parser_JSON.JObject fields ->
         ...
         let ac_popped1 =
           {
             JSONLD_Context.ac_terms = (ac_popped.JSONLD_Context.ac_terms);
             JSONLD_Context.ac_vocab = (ac_popped.JSONLD_Context.ac_vocab);
             JSONLD_Context.ac_base = (ac_popped.JSONLD_Context.ac_base);
             JSONLD_Context.ac_language =
               (ac_popped.JSONLD_Context.ac_language);
             JSONLD_Context.ac_direction =
               (ac_popped.JSONLD_Context.ac_direction);
             JSONLD_Context.ac_previous =
               (ac_popped.JSONLD_Context.ac_previous);
             JSONLD_Context.ac_mode10 = (ac_popped.JSONLD_Context.ac_mode10);
             JSONLD_Context.ac_doc_url =
               (ac_popped.JSONLD_Context.ac_doc_url);
             JSONLD_Context.ac_original_base =
               (ac_popped.JSONLD_Context.ac_original_base);
             JSONLD_Context.ac_suppress_pop = false;
             JSONLD_Context.ac_frame_expansion =
               (ac_popped.JSONLD_Context.ac_frame_expansion)
           } in
         ...
         (match uu___1 with
          | (ctxval, fields1) ->
              let ac0_opt =
                match ctxval with
                | FStar_Pervasives_Native.None ->
                    FStar_Pervasives_Native.Some ac_popped1
                | FStar_Pervasives_Native.Some cv ->
                    JSONLD_Context.apply_context_with_propagate ac_popped1 cv
                      true false in
              (match ac0_opt with
               | FStar_Pervasives_Native.None -> FStar_Pervasives_Native.None
               | FStar_Pervasives_Native.Some ac0 ->
                   (match expand_typed_ac ac0 fields1 with
                    ...
                    | FStar_Pervasives_Native.Some ac_typed ->
                        if jexp_has_colliding_keywords ac_typed fields1
                        then FStar_Pervasives_Native.None
                        else
                          (match expand_fields_list ac_typed ac0
                                   (jexp_order_node_fields ac_typed fields1)
                                   (fuel - Prims.int_one)
                           with
                           | FStar_Pervasives_Native.None ->
                               FStar_Pervasives_Native.None
                           | FStar_Pervasives_Native.Some outfields ->
                               FStar_Pervasives_Native.Some
                                 (Parser_JSON.JObject
                                    (jexp_merge_dup_fields outfields
                                       ((Parser_JSON.json_size
         ...
         let ac1 =
           {
             ...
             JSONLD_Context.ac_vocab = (ac.JSONLD_Context.ac_vocab);
             JSONLD_Context.ac_base = (ac.JSONLD_Context.ac_base);
             JSONLD_Context.ac_language = (ac.JSONLD_Context.ac_language);
             JSONLD_Context.ac_direction = (ac.JSONLD_Context.ac_direction);
             JSONLD_Context.ac_previous = (ac.JSONLD_Context.ac_previous);
             JSONLD_Context.ac_mode10 = (ac.JSONLD_Context.ac_mode10);
             JSONLD_Context.ac_doc_url = (ac.JSONLD_Context.ac_doc_url);
             JSONLD_Context.ac_original_base =
               (ac.JSONLD_Context.ac_original_base);
             JSONLD_Context.ac_suppress_pop = false;
             JSONLD_Context.ac_frame_expansion =
         ...
         (match uu___1 with
          | (ctxval, fields1) ->
              let ac0_opt =
                ...
                | FStar_Pervasives_Native.Some cv ->
                    JSONLD_Context.apply_context_with_propagate ac1 cv true
                      false in
              (match ac0_opt with
               | FStar_Pervasives_Native.None -> FStar_Pervasives_Native.None
               | FStar_Pervasives_Native.Some ac0 ->
...
and expand_one_field (ac : JSONLD_Context.active_context)
  ...
  then FStar_Pervasives_Native.None
    ...
    then
      (match value with
       | Parser_JSON.JString s ->
           if JSONLD_Context.jldctx_keyword_lookalike s
           then
             FStar_Pervasives_Native.Some
               (FStar_Pervasives_Native.Some [("@id", Parser_JSON.JNull)])
           else
             (match JSONLD_Context.expand_iri ac s false with
              | FStar_Pervasives_Native.None ->
                  FStar_Pervasives_Native.Some
                    (FStar_Pervasives_Native.Some
                       [("@id", (Parser_JSON.JString s))])
              | FStar_Pervasives_Native.Some iri ->
                  FStar_Pervasives_Native.Some
                    (FStar_Pervasives_Native.Some
       ...
       | Parser_JSON.JArray items ->
           if
             ac.JSONLD_Context.ac_frame_expansion &&
               (jexp_id_frame_entries_valid items)
           then
             FStar_Pervasives_Native.Some
               (FStar_Pervasives_Native.Some
                  [("@id",
                     (Parser_JSON.JArray
                        (jexp_expand_id_frame_items ac items)))])
           else FStar_Pervasives_Native.None
       | Parser_JSON.JObject ofields ->
           if
             ac.JSONLD_Context.ac_frame_expansion &&
               (Prims.uu___is_Nil ofields)
           then
             FStar_Pervasives_Native.Some
               (FStar_Pervasives_Native.Some
                  [("@id", (Parser_JSON.JObject []))])
           else FStar_Pervasives_Native.None
       | uu___1 -> FStar_Pervasives_Native.None)
    else
      if key = "@type"
            ...
            (match value with
             | Parser_JSON.JObject rfields ->
                 (match expand_reverse_block_fields ac rfields
                  ...
                  | FStar_Pervasives_Native.None ->
                      FStar_Pervasives_Native.None
                  | FStar_Pervasives_Native.Some (ord_entries, rev_entries)
                      ->
                      FStar_Pervasives_Native.Some
                        (FStar_Pervasives_Native.Some
                           (FStar_List_Tot_Base.append ord_entries
                              (if rev_entries = []
                               then []
                               else
                                 [("@reverse",
                                    (Parser_JSON.JObject rev_entries))]))))
                ...
                then
                  (match value with
                   | Parser_JSON.JObject nfields ->
                       (match expand_fields_list ac ac0
                                (jexp_order_node_fields ac nfields)
                                (fuel - Prims.int_one)
                        with
                        | FStar_Pervasives_Native.None ->
                            FStar_Pervasives_Native.None
                  ...
                  if
                    ac.JSONLD_Context.ac_frame_expansion &&
                      (jexp_is_framing_keyword key)
                  then
                    FStar_Pervasives_Native.Some
                      (FStar_Pervasives_Native.Some [(key, value)])
                  else
  ...
  then FStar_Pervasives_Native.None
    ...
    (match items with
     ...
     | (Parser_JSON.JObject nfields)::rest ->
         (match expand_fields_list ac ac0 (jexp_order_node_fields ac nfields)
                  (fuel - Prims.int_one)
          with
          | FStar_Pervasives_Native.None -> FStar_Pervasives_Native.None
          | FStar_Pervasives_Native.Some outs ->
    ...
    (match apply_property_scoped_context ac term_opt with
     ...
     | FStar_Pervasives_Native.Some ac_eff ->
         let is_list =
           ...
           | FStar_Pervasives_Native.None -> false in
         let already_list_obj =
           match value with
           | Parser_JSON.JObject fs ->
               jexp_has_aliased_field ac_eff "@list" fs
           | uu___1 -> false in
         let is_map_ck =
           match term_opt with
           | FStar_Pervasives_Native.Some td ->
               (match td.JSONLD_Context.td_container with
                | JSONLD_Context.CK_Index -> true
                | JSONLD_Context.CK_Language -> true
                | JSONLD_Context.CK_Id -> true
                | JSONLD_Context.CK_Type -> true
                | JSONLD_Context.CK_GraphId -> true
                | JSONLD_Context.CK_GraphIndex -> true
                | uu___1 -> false)
           | FStar_Pervasives_Native.None -> false in
         let value_nullish =
           match value with
           ...
           | Parser_JSON.JObject fs ->
               (Prims.op_Negation (jexp_has_aliased_field ac_eff "@set" fs))
                 && (Prims.op_Negation is_map_ck)
           | uu___1 -> false in
         (match expand_property_items ac_eff term_opt value
          ...
          | FStar_Pervasives_Native.None -> FStar_Pervasives_Native.None
          | FStar_Pervasives_Native.Some items ->
              if (items = []) && value_nullish
              then FStar_Pervasives_Native.Some FStar_Pervasives_Native.None
              else
                if is_list && (Prims.op_Negation already_list_obj)
                then
                  FStar_Pervasives_Native.Some
                    (FStar_Pervasives_Native.Some
                       [(prop_iri,
         ...
         then FStar_Pervasives_Native.None
           ...
           if
             FStar_Pervasives_Native.uu___is_None
               (JSONLD_Context.expand_iri ac key true)
           then expand_reverse_block_fields ac rest (fuel - Prims.int_one)
           else
             (match JSONLD_Context.expand_iri ac key true with
              | FStar_Pervasives_Native.None -> FStar_Pervasives_Native.None
              | FStar_Pervasives_Native.Some prop_iri ->
                  ...
                  then FStar_Pervasives_Native.None
                    ...
                    (let term_opt =
                     ...
                     | FStar_Pervasives_Native.Some items ->
                         if
                           Prims.op_Negation (jexp_items_all_node_like items)
                         then FStar_Pervasives_Native.None
                         else
  ...
  then FStar_Pervasives_Native.None
    ...
    (let type_map =
     ...
     then
       ...
       (match (ck, value) with
        | (JSONLD_Context.CK_Index, Parser_JSON.JObject entries0) ->
            ...
            (match idx_prop with
             ...
             | FStar_Pervasives_Native.None ->
                 jexp_expand_plain_index_map ac type_map lang_ovr dir_ovr
                   entries (fuel - Prims.int_one))
        | (JSONLD_Context.CK_Language, Parser_JSON.JObject entries0) ->
            let entries = jexp_sort_map_entries entries0 in
            let eff_dir =
              match dir_ovr with
            ...
            then
              FStar_Pervasives_Native.Some
                (jexp_expand_language_map ac eff_dir entries)
            else FStar_Pervasives_Native.None
        | (JSONLD_Context.CK_Id, Parser_JSON.JObject entries) ->
            jexp_expand_id_map ac (jexp_sort_map_entries entries)
              (fuel - Prims.int_one)
        | (JSONLD_Context.CK_Type, Parser_JSON.JObject entries) ->
            jexp_expand_type_map ac type_map (jexp_sort_map_entries entries)
              (fuel - Prims.int_one)
        | (JSONLD_Context.CK_Graph, uu___2) ->
            FStar_Pervasives_Native.Some
              (expand_graph_container_items_plain ac (jexp_as_array value)
                 (fuel - Prims.int_one))
        | (JSONLD_Context.CK_GraphIndex, Parser_JSON.JObject entries0) ->
            let entries = jexp_sort_map_entries entries0 in
            (match idx_prop with
             | FStar_Pervasives_Native.Some name ->
             ...
             | FStar_Pervasives_Native.None ->
                 FStar_Pervasives_Native.Some
                   (jexp_expand_graph_index_kw_map ac entries
                      (fuel - Prims.int_one)))
        ...
        | (JSONLD_Context.CK_GraphId, Parser_JSON.JObject entries) ->
            FStar_Pervasives_Native.Some
              (expand_graph_id_map ac (jexp_sort_map_entries entries)
                 (fuel - Prims.int_one))
        ...
        | (uu___2, uu___3) ->
            expand_property ac type_map lang_ovr dir_ovr
              (JSONLD_Context.ck_is_list ck) (jexp_as_array value)
              (fuel - Prims.int_one)))
...
and jexp_expand_property_index_map (ac : JSONLD_Context.active_context)
  ...
  then FStar_Pervasives_Native.None
    ...
    (match entries with
     | [] -> FStar_Pervasives_Native.Some []
     | (k, v)::rest ->
         (match expand_property ac type_map lang_ovr dir_ovr false
                      ...
                      | FStar_Pervasives_Native.Some (index_iri, keyval) ->
                          (match jexp_inject_index_items index_iri keyval
                           ...
                           | FStar_Pervasives_Native.Some items1 ->
                               FStar_Pervasives_Native.Some
                                 (FStar_List_Tot_Base.append
                                    (jexp_merge_item_fields items1) restout))))))
and jexp_expand_plain_index_map (ac : JSONLD_Context.active_context)
  (type_map : Prims.string FStar_Pervasives_Native.option)
  (lang_ovr :
    Prims.string FStar_Pervasives_Native.option
      FStar_Pervasives_Native.option)
              ...
              let is_none =
                (k = "@none") ||
                  (match JSONLD_Context.expand_iri ac k true with
                   ...
                   | uu___1 -> false) in
              let items1 =
                if is_none then items else jexp_add_index_items k items in
              (match jexp_expand_plain_index_map ac type_map lang_ovr dir_ovr
...
and jexp_expand_graph_index_kw_map (ac : JSONLD_Context.active_context)
  (entries : (Prims.string * Parser_JSON.json_val) Prims.list)
  (fuel : Prims.nat) : Parser_JSON.json_val Prims.list=
  if fuel = Prims.int_zero
  then []
    ...
    (match entries with
     ...
     | (k, v)::rest ->
         ...
         FStar_List_Tot_Base.append items1
           (jexp_expand_graph_index_kw_map ac rest (fuel - Prims.int_one)))
         ...
         (match apply_property_scoped_context ac_popped
          ...
          | FStar_Pervasives_Native.Some ac_for_value ->
              let map_value_type =
                ...
                | FStar_Pervasives_Native.None -> "@id" in
              (match expand_item ac_for_value
                       (FStar_Pervasives_Native.Some map_value_type)
                       FStar_Pervasives_Native.None
                       FStar_Pervasives_Native.None true v
                       (fuel - Prims.int_one)
               with
               ...
               | FStar_Pervasives_Native.Some (FStar_Pervasives_Native.None)
                   ->
                   jexp_expand_type_map ac type_map rest
                     (fuel - Prims.int_one)
               | FStar_Pervasives_Native.Some (FStar_Pervasives_Native.Some
                   item) ->
                   let item1 =
                   ...
                   (match jexp_expand_type_map ac type_map rest
                            (fuel - Prims.int_one)
                    with
                    | FStar_Pervasives_Native.None ->
                        FStar_Pervasives_Native.None
...
and expand_property (ac : JSONLD_Context.active_context)
  ...
  (dir_ovr :
    Prims.string FStar_Pervasives_Native.option
      FStar_Pervasives_Native.option)
  (in_list : Prims.bool) (items : Parser_JSON.json_val Prims.list)
  (fuel : Prims.nat) :
  Parser_JSON.json_val Prims.list FStar_Pervasives_Native.option=
  if fuel = Prims.int_zero
  then FStar_Pervasives_Native.None
    ...
    (match items with
     ...
     | v::rest ->
         let set_contents =
           match v with
           | Parser_JSON.JObject fields ->
               (match jexp_find_aliased_field ac "@set" fields with
                | FStar_Pervasives_Native.Some (uu___1, sv) ->
                    FStar_Pervasives_Native.Some sv
                | FStar_Pervasives_Native.None ->
                    FStar_Pervasives_Native.None)
           | uu___1 -> FStar_Pervasives_Native.None in
         (match set_contents with
          | FStar_Pervasives_Native.Some setval ->
              (match expand_property ac type_map lang_ovr dir_ovr in_list
                       (jexp_as_array setval) (fuel - Prims.int_one)
               with
               | FStar_Pervasives_Native.None -> FStar_Pervasives_Native.None
               | FStar_Pervasives_Native.Some setitems ->
                   (match expand_property ac type_map lang_ovr dir_ovr
                    ...
                    | FStar_Pervasives_Native.Some restout ->
                        FStar_Pervasives_Native.Some
                          (FStar_List_Tot_Base.append setitems restout)))
          | FStar_Pervasives_Native.None ->
              (match v with
               | Parser_JSON.JArray inner ->
                   if Prims.op_Negation in_list
                   then
                     (match expand_property ac type_map lang_ovr dir_ovr
                              false inner (fuel - Prims.int_one)
                      with
                      | FStar_Pervasives_Native.None ->
                          FStar_Pervasives_Native.None
                      | FStar_Pervasives_Native.Some innerout ->
                          (match expand_property ac type_map lang_ovr dir_ovr
                                   in_list rest (fuel - Prims.int_one)
                           with
                           | FStar_Pervasives_Native.None ->
               ...
               | uu___1 ->
                   (match expand_item ac type_map lang_ovr dir_ovr false v
                    ...
                    | FStar_Pervasives_Native.Some
                        (FStar_Pervasives_Native.None) ->
                        expand_property ac type_map lang_ovr dir_ovr in_list
                          rest (fuel - Prims.int_one)
                    | FStar_Pervasives_Native.Some
                        (FStar_Pervasives_Native.Some one) ->
                        if
    ...
    (match v with
     ...
     | Parser_JSON.JObject fields ->
         ...
         then
           let json_typed =
             match jexp_find_aliased_field ac "@type" fields with
             | FStar_Pervasives_Native.Some (uu___1, Parser_JSON.JString t)
                 ->
                 (JSONLD_Context.expand_iri ac t true) =
                   (FStar_Pervasives_Native.Some "@json")
             | uu___1 -> false in
           let expand_vo fs =
             if ac.JSONLD_Context.ac_frame_expansion
             then jexp_expand_value_object_frame ac fs
             else jexp_expand_value_object ac fs in
           (match jexp_find_aliased_field ac "@value" fields with
            | FStar_Pervasives_Native.Some (uu___1, Parser_JSON.JNull) ->
                if json_typed
                then
                  ...
                  if jexp_value_object_keys_valid ac fields
                  then
                    FStar_Pervasives_Native.Some FStar_Pervasives_Native.None
                  else FStar_Pervasives_Native.None
            | uu___1 ->
              ...
              then FStar_Pervasives_Native.None
                ...
                (match jexp_find_aliased_field ac "@list" fields with
                 ...
                 | FStar_Pervasives_Native.Some (uu___3, lstval) ->
                     (match expand_property ac type_map lang_ovr dir_ovr true
     ...
     | Parser_JSON.JNumber uu___1 ->
         FStar_Pervasives_Native.Some
           (FStar_Pervasives_Native.Some (jexp_wrap_scalar type_map v))
     | Parser_JSON.JArray items ->
         (match expand_property ac type_map lang_ovr dir_ovr true items
                  (fuel - Prims.int_one)
          with
          | FStar_Pervasives_Native.None -> FStar_Pervasives_Native.None
          | FStar_Pervasives_Native.Some out_items ->
              FStar_Pervasives_Native.Some
                (FStar_Pervasives_Native.Some
                   (Parser_JSON.JObject
                      [("@list", (Parser_JSON.JArray out_items))]))))
...
let expand (ac : JSONLD_Context.active_context) (doc : Parser_JSON.json_val)
  ...
  let ac1 =
    if
      (ac.JSONLD_Context.ac_doc_url = FStar_Pervasives_Native.None) &&
        (ac.JSONLD_Context.ac_original_base = FStar_Pervasives_Native.None)
    then
      {
        JSONLD_Context.ac_terms = (ac.JSONLD_Context.ac_terms);
        JSONLD_Context.ac_vocab = (ac.JSONLD_Context.ac_vocab);
        ...
        JSONLD_Context.ac_previous = (ac.JSONLD_Context.ac_previous);
        JSONLD_Context.ac_mode10 = (ac.JSONLD_Context.ac_mode10);
        JSONLD_Context.ac_doc_url = (ac.JSONLD_Context.ac_base);
        JSONLD_Context.ac_original_base = (ac.JSONLD_Context.ac_base);
        JSONLD_Context.ac_suppress_pop = (ac.JSONLD_Context.ac_suppress_pop);
        JSONLD_Context.ac_frame_expansion =
          (ac.JSONLD_Context.ac_frame_expansion)
      }
    else ac in
  ...
  | Parser_JSON.JObject fields0 ->
      ...
      then FStar_Pervasives_Native.Some (Parser_JSON.JArray [])
        ...
        (match expand_node ac1 doc fuel with
         | FStar_Pervasives_Native.None -> FStar_Pervasives_Native.None
         ...
         | FStar_Pervasives_Native.Some (Parser_JSON.JObject fields1) ->
             ...
             then
               FStar_Pervasives_Native.Some
                 (Parser_JSON.JArray
                    (jexp_drop_ff_items (jexp_collect_graph_values fields1)
                       ((Prims.of_int (4)) * fuel)))
             else
               FStar_Pervasives_Native.Some
                 (Parser_JSON.JArray
                    (jexp_drop_ff_items [Parser_JSON.JObject fields1]
                       ((Prims.of_int (4)) * fuel)))
         | FStar_Pervasives_Native.Some nodeobj ->
             FStar_Pervasives_Native.Some
               (Parser_JSON.JArray
                  (jexp_drop_ff_items [nodeobj] ((Prims.of_int (4)) * fuel))))
  | Parser_JSON.JArray items ->
      (match expand_top_items ac1 items fuel with
       | FStar_Pervasives_Native.None -> FStar_Pervasives_Native.None
       | FStar_Pervasives_Native.Some outs ->
           FStar_Pervasives_Native.Some
             (Parser_JSON.JArray
                (jexp_drop_ff_items outs ((Prims.of_int (4)) * fuel))))
  | uu___ -> FStar_Pervasives_Native.None
