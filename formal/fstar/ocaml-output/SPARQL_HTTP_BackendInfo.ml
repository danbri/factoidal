...
type backend_kind =
  ...
  | BK_Hybrid 
  | BK_Empty 
let uu___is_BK_InMem (projectee : backend_kind) : Prims.bool=
  match projectee with | BK_InMem -> true | uu___ -> false
let uu___is_BK_CottasOnDisk (projectee : backend_kind) : Prims.bool=
  match projectee with | BK_CottasOnDisk -> true | uu___ -> false
let uu___is_BK_Hybrid (projectee : backend_kind) : Prims.bool=
  match projectee with | BK_Hybrid -> true | uu___ -> false
let uu___is_BK_Empty (projectee : backend_kind) : Prims.bool=
  match projectee with | BK_Empty -> true | uu___ -> false
let backend_kind_string (k : backend_kind) : Prims.string=
  match k with
  | BK_InMem -> "in-memory"
...
let __proj__Mkbackend_info__item__bi_kind (projectee : backend_info) :
  backend_kind=
  match projectee with
...
let __proj__Mkbackend_info__item__bi_source (projectee : backend_info) :
  Prims.string=
  match projectee with
  | { bi_kind; bi_source; bi_in_memory_triples;
      bi_in_memory_default_graph_triples; bi_in_memory_named_graphs;
...
let __proj__Mkbackend_info__item__bi_in_memory_triples
  (projectee : backend_info) : Prims.int=
  match projectee with
  | { bi_kind; bi_source; bi_in_memory_triples;
      bi_in_memory_default_graph_triples; bi_in_memory_named_graphs;
      bi_in_memory_named_graph_triples; bi_cottas;_} -> bi_in_memory_triples
...
let __proj__Mkbackend_info__item__bi_cottas (projectee : backend_info) :
  cottas_summary Prims.list=
  match projectee with
  | { bi_kind; bi_source; bi_in_memory_triples;
      bi_in_memory_default_graph_triples; bi_in_memory_named_graphs;
      bi_in_memory_named_graph_triples; bi_cottas;_} -> bi_cottas
let rec sum_cottas_quads (xs : cottas_summary Prims.list) : Prims.int=
  match xs with
  | [] -> Prims.int_zero
  | x::rest -> x.cs_quads + (sum_cottas_quads rest)
let render_backend_info (info : backend_info) : Prims.string=
...
let backend_source_string
  (dataset_basename : Prims.string FStar_Pervasives_Native.option)
  (cottas_basenames : Prims.string Prims.list) : Prims.string=
  match (dataset_basename, cottas_basenames) with
  | (FStar_Pervasives_Native.None, []) -> "(none)"
  | (FStar_Pervasives_Native.Some f, []) -> f
  | (FStar_Pervasives_Native.None, paths) -> FStar_String.concat ", " paths
