...
let pending_extra_paths : group_group_graph_pattern list ref = ref []
...
(* ============================================================================
   Post-parse validation -- reject queries that are syntactically structured
   but violate SPARQL 1.1 static constraints.
   ============================================================================ *)

let rec expr_free_vars (e : expr) : string list =
  match e with
  | E_Var v -> [v]
  | E_Bound v -> [v]
  | E_IRI _ | E_Literal _ | E_BoolLit _ | E_NumericLit _ | E_DecimalLit _
  | E_DoubleLit _ | E_Now -> []
  | E_Aggregate (_, _, _) -> []
  | E_Arith (_, e1, e2) | E_Compare (_, e1, e2)
  | E_And (e1, e2) | E_Or (e1, e2)
  | E_StrDt (e1, e2) | E_StrLang (e1, e2) | E_SameTerm (e1, e2)
  | E_StrStarts (e1, e2) | E_StrEnds (e1, e2) | E_Contains (e1, e2)
  | E_StrBefore (e1, e2) | E_StrAfter (e1, e2) ->
    expr_free_vars e1 @ expr_free_vars e2
  | E_Not e1 | E_UnaryMinus e1 | E_UnaryPlus e1
  ...
  | E_Seconds e1 | E_Timezone e1 | E_Tz e1 ->
    expr_free_vars e1
  | E_If (e1, e2, e3) ->
    expr_free_vars e1 @ expr_free_vars e2 @ expr_free_vars e3
  | E_Coalesce args | E_Concat args -> List.concat_map expr_free_vars args
  | E_FunctionCall (_, args) -> List.concat_map expr_free_vars args
  | E_Exists _ | E_NotExists _ -> []
  | E_In (e1, es) | E_NotIn (e1, es) ->
    expr_free_vars e1 @ List.concat_map expr_free_vars es
  | E_Substr (e1, e2, e3_opt) ->
    expr_free_vars e1 @ expr_free_vars e2 @
    (match e3_opt with Some e3 -> expr_free_vars e3 | None -> [])
  | E_Replace (e1, e2, e3, e4_opt) ->
    expr_free_vars e1 @ expr_free_vars e2 @ expr_free_vars e3 @
    (match e4_opt with Some e4 -> expr_free_vars e4 | None -> [])
...
let rec expr_has_aggregate (e : expr) : bool =
  ...
  | E_Var _ | E_Bound _ | E_IRI _ | E_Literal _ | E_BoolLit _
  | E_NumericLit _ | E_DecimalLit _ | E_DoubleLit _ | E_Now -> false
  | E_Arith (_, e1, e2) | E_Compare (_, e1, e2)
  | E_And (e1, e2) | E_Or (e1, e2)
  | E_StrDt (e1, e2) | E_StrLang (e1, e2) | E_SameTerm (e1, e2)
  | E_StrStarts (e1, e2) | E_StrEnds (e1, e2) | E_Contains (e1, e2)
  ...
  | E_Year e1 | E_Month e1 | E_Day e1 | E_Hours e1 | E_Minutes e1
  | E_Seconds e1 | E_Timezone e1 | E_Tz e1 ->
    expr_has_aggregate e1
  | E_If (e1, e2, e3) ->
    expr_has_aggregate e1 || expr_has_aggregate e2 || expr_has_aggregate e3
  | E_Coalesce args | E_Concat args -> List.exists expr_has_aggregate args
  | E_FunctionCall (_, args) -> List.exists expr_has_aggregate args
  | E_Exists _ | E_NotExists _ -> false
  | E_In (e1, es) | E_NotIn (e1, es) ->
    expr_has_aggregate e1 || List.exists expr_has_aggregate es
  | E_Substr (e1, e2, e3_opt) ->
    expr_has_aggregate e1 || expr_has_aggregate e2 ||
    (match e3_opt with Some e3 -> expr_has_aggregate e3 | None -> false)
  | E_Replace (e1, e2, e3, e4_opt) ->
    expr_has_aggregate e1 || expr_has_aggregate e2 || expr_has_aggregate e3 ||
    (match e4_opt with Some e4 -> expr_has_aggregate e4 | None -> false)
  | E_Regex (e1, e2, e3_opt) ->
    expr_has_aggregate e1 || expr_has_aggregate e2 ||
    (match e3_opt with Some e3 -> expr_has_aggregate e3 | None -> false)

let select_clause_has_aggregate (sc : select_clause) : bool =
  match sc with
  | Select_All -> false
  | Select_Vars items -> List.exists (fun item ->
    match item with
    | SI_Var _ -> false
    | SI_Expr (e, _) -> expr_has_aggregate e) items

let group_by_vars (gbs : group_condition list) : string list =
  List.filter_map (fun gc ->
    match gc with
    | GC_Var v -> Some v
    | GC_Expr (_, Some v) -> Some v
    | _ -> None) gbs

let rec pattern_vars (p : group_graph_pattern) : string list =
  match p with
  | GP_BGP bgp -> List.concat_map (fun tp ->
    (match tp.tp_s with PS_Var v -> [v] | _ -> []) @
    (match tp.tp_o with PT_Var v -> [v] | _ -> [])) bgp
  | GP_Join (p1, p2) -> pattern_vars p1 @ pattern_vars p2
  | GP_LeftJoin (p1, p2, _) -> pattern_vars p1 @ pattern_vars p2
  | GP_Filter (_, p1) -> pattern_vars p1
  | GP_Union (p1, p2) -> pattern_vars p1 @ pattern_vars p2
  | GP_Graph (_, p1) -> pattern_vars p1
  | GP_Minus (p1, p2) -> pattern_vars p1 @ pattern_vars p2
  | GP_Bind (_, v, p1) -> v :: pattern_vars p1
  | GP_Values (vs, _) -> vs
  | GP_Service (_, p1, _) -> pattern_vars p1
  | GP_SubSelect q -> pattern_vars_query q
  | GP_PropertyPath (s, _, o) ->
    (match s with PS_Var v -> [v] | _ -> []) @
    (match o with PT_Var v -> [v] | _ -> [])
  | GP_Empty -> []
and pattern_vars_query (q : query) : string list =
  match q.q_form with
  | QF_Select (Select_Vars items) ->
    List.filter_map (fun item ->
      match item with SI_Var v -> Some v | SI_Expr (_, v) -> Some v) items
  | QF_Select Select_All -> pattern_vars q.q_pattern
  | _ -> pattern_vars q.q_pattern

let rec check_bind_scope (p : group_graph_pattern) : unit =
  match p with
  | GP_Bind (_, v, preceding) ->
    let preceding_vars = pattern_vars preceding in
    if List.mem v preceding_vars then
      raise (Parse_error (Printf.sprintf "BIND variable ?%s already in scope" v));
    check_bind_scope preceding
  | GP_Join (p1, p2) -> check_bind_scope p1; check_bind_scope p2
  | GP_LeftJoin (p1, p2, _) -> check_bind_scope p1; check_bind_scope p2
  | GP_Filter (_, p1) -> check_bind_scope p1
  | GP_Union (p1, p2) -> check_bind_scope p1; check_bind_scope p2
  | GP_Graph (_, p1) -> check_bind_scope p1
  | GP_Minus (p1, p2) -> check_bind_scope p1; check_bind_scope p2
  | GP_Service (_, p1, _) -> check_bind_scope p1
  | GP_SubSelect q -> check_bind_scope q.q_pattern
  | _ -> ()

let rec check_construct_where_pattern (p : group_graph_pattern) : unit =
  match p with
  | GP_BGP _ | GP_Empty | GP_PropertyPath _ -> ()
  | GP_Join (p1, p2) ->
    check_construct_where_pattern p1; check_construct_where_pattern p2
  | GP_Filter _ ->
    raise (Parse_error "FILTER not allowed in CONSTRUCT WHERE short form")
  | GP_Graph _ ->
    raise (Parse_error "GRAPH not allowed in CONSTRUCT WHERE short form")
  | GP_LeftJoin _ ->
    raise (Parse_error "OPTIONAL not allowed in CONSTRUCT WHERE short form")
  | GP_Union _ ->
    raise (Parse_error "UNION not allowed in CONSTRUCT WHERE short form")
  | GP_Minus _ ->
    raise (Parse_error "MINUS not allowed in CONSTRUCT WHERE short form")
  | GP_Bind _ ->
    raise (Parse_error "BIND not allowed in CONSTRUCT WHERE short form")
  | GP_SubSelect _ ->
    raise (Parse_error "Sub-SELECT not allowed in CONSTRUCT WHERE short form")
  | GP_Service _ ->
    raise (Parse_error "SERVICE not allowed in CONSTRUCT WHERE short form")
  | GP_Values _ ->
    raise (Parse_error "VALUES not allowed in CONSTRUCT WHERE short form")

let validate_query (q : query) : query =
  (match q.q_form with
  | QF_Select select_clause ->
    let has_group_by = q.q_group_by <> None in
    let has_aggregate = select_clause_has_aggregate select_clause in
    let is_grouped = has_group_by || has_aggregate in
    if is_grouped then begin
      (match select_clause with
      | Select_All ->
        raise (Parse_error "SELECT * not allowed with GROUP BY")
      | Select_Vars items ->
        let gvars = match q.q_group_by with
          | Some gbs -> group_by_vars gbs
          | None -> [] in
        List.iter (fun item ->
          match item with
          | SI_Var v ->
            if not (List.mem v gvars) then
              raise (Parse_error (Printf.sprintf
                "Variable ?%s in SELECT not in GROUP BY" v))
          | SI_Expr (e, _) ->
            let fvs = expr_free_vars e in
            List.iter (fun v ->
              if not (List.mem v gvars) then
                raise (Parse_error (Printf.sprintf
                  "Variable ?%s in SELECT expression not in GROUP BY" v))
            ) fvs
        ) items)
    end;
    (match select_clause with
      ...
      let rec check_dups = function
        ...
        | v :: rest ->
          if List.mem v rest then
            raise (Parse_error (Printf.sprintf
              "Duplicate variable ?%s in SELECT aliases" v));
          check_dups rest
  ...
  | QF_Construct template when template = [] ->
    check_construct_where_pattern q.q_pattern
  | _ -> ());
  q
...
let parse_query input =
      ...
      let prefix = match peek ps with
        | T_PNAME pn ->
          ...
          (match String.index_opt pn ':' with
           | Some i ->
             let local = String.sub pn (i + 1) (String.length pn - i - 1) in
             if local <> "" then
               raise (Parse_error (Printf.sprintf
                 "Invalid PREFIX declaration: '%s'" pn));
             String.sub pn 0 i
           | None -> pn)
  ...
  | _ -> raise (Parse_error (Printf.sprintf
      "Expected SELECT, ASK, or CONSTRUCT at pos %d" ps.lx.pos))
  in
  if peek ps <> T_EOF then
    raise (Parse_error (Printf.sprintf
      "Unexpected token at pos %d after query" ps.lx.pos));
  validate_query q
