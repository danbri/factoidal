...
type graph_store =
  {
  gs_default: RDF_Graph.rdf_graph ;
...
let empty_store : graph_store=
  { gs_default = RDF_Graph.empty_graph; gs_named = [] }
...
let rec replace_named (key : graph_key) (g : RDF_Graph.rdf_graph)
...
let gsp_put (t : gs_target) (g : RDF_Graph.rdf_graph) (s : graph_store) :
...
let gsp_post (t : gs_target) (g : RDF_Graph.rdf_graph) (s : graph_store) :
  (graph_store * Prims.bool)=
  match t with
  | GT_Default ->
      let did = Prims.uu___is_Cons s.gs_default in
...
let gsp_delete (t : gs_target) (s : graph_store) :
  ...
  | GT_Default ->
      let did = Prims.uu___is_Cons s.gs_default in
      ({ gs_default = RDF_Graph.empty_graph; gs_named = (s.gs_named) }, did)
  | GT_Named k ->
      let did = named_exists k s.gs_named in
