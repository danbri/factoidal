open Prims
let rec replace_all_aux (haystack : Prims.string) (needle : Prims.string)
  (replacement : Prims.string) (nlen : Prims.nat) (pos : Prims.nat)
  ...
  then acc
    ...
    then
      (let c = FStar_String.index haystack pos in
       replace_all_aux haystack needle replacement nlen (pos + Prims.int_one)
         (c :: acc))
    else
...
let string_replace_all (needle : Prims.string) (replacement : Prims.string)
  (haystack : Prims.string) : Prims.string=
  let nlen = FStar_String.strlen needle in
  if nlen = Prims.int_zero
  then haystack
  else
    (let revc =
       replace_all_aux haystack needle replacement nlen Prims.int_zero [] in
     FStar_String.string_of_list (FStar_List_Tot_Base.rev revc))
let expand_user_graph (template : Prims.string) (authid : Prims.string) :
  Prims.string= string_replace_all "{authid}" authid template
let rec find_authid_placeholder (s : Prims.string) (pos : Prims.nat)
    ...
    then FStar_Pervasives_Native.None
      ...
      then FStar_Pervasives_Native.Some pos
        ...
        find_authid_placeholder s (pos + Prims.int_one)
          (fuel - Prims.int_one)
let template_prefix (template : Prims.string) : Prims.string=
...
type ggp_target_status =
  ...
  | GTS_NonIri 
let uu___is_GTS_Ok (projectee : ggp_target_status) : Prims.bool=
  match projectee with | GTS_Ok -> true | uu___ -> false
let uu___is_GTS_Mismatch (projectee : ggp_target_status) : Prims.bool=
  match projectee with | GTS_Mismatch iri -> true | uu___ -> false
let __proj__GTS_Mismatch__item__iri (projectee : ggp_target_status) :
  Prims.string= match projectee with | GTS_Mismatch iri -> iri
...
let uu___is_GCR_Rewrite (projectee : ggp_check_result) : Prims.bool=
  match projectee with | GCR_Rewrite _0 -> true | uu___ -> false
let __proj__GCR_Rewrite__item___0 (projectee : ggp_check_result) :
  SPARQL11_Algebra.group_graph_pattern=
  match projectee with | GCR_Rewrite _0 -> _0
let uu___is_GCR_Reject (projectee : ggp_check_result) : Prims.bool=
...
let uu___is_GRCR_Ok (projectee : gref_check_result) : Prims.bool=
  match projectee with | GRCR_Ok -> true | uu___ -> false
let uu___is_GRCR_Reject (projectee : gref_check_result) : Prims.bool=
...
let check_gref (which : Prims.string) (gr : SPARQL11_Algebra.graph_ref)
  ...
  | SPARQL11_Algebra.GR_Default ->
      GRCR_Reject
        (Prims.strcat which
           (Prims.strcat " targets the default graph; your sandbox is <"
              (Prims.strcat usergraph ">")))
...
let uu___is_SB_Ok (projectee : sandbox_result) : Prims.bool=
  match projectee with | SB_Ok _0 -> true | uu___ -> false
let __proj__SB_Ok__item___0 (projectee : sandbox_result) :
  SPARQL11_Algebra.update_op= match projectee with | SB_Ok _0 -> _0
let uu___is_SB_Reject (projectee : sandbox_result) : Prims.bool=
  match projectee with | SB_Reject _0 -> true | uu___ -> false
let __proj__SB_Reject__item___0 (projectee : sandbox_result) : Prims.string=
  match projectee with | SB_Reject _0 -> _0
type tpl_opt_result =
  | TOR_Rewrite of SPARQL11_Algebra.group_graph_pattern
  FStar_Pervasives_Native.option 
  | TOR_Reject of Prims.string 
let uu___is_TOR_Rewrite (projectee : tpl_opt_result) : Prims.bool=
  match projectee with | TOR_Rewrite _0 -> true | uu___ -> false
let __proj__TOR_Rewrite__item___0 (projectee : tpl_opt_result) :
  SPARQL11_Algebra.group_graph_pattern FStar_Pervasives_Native.option=
  match projectee with | TOR_Rewrite _0 -> _0
let uu___is_TOR_Reject (projectee : tpl_opt_result) : Prims.bool=
  match projectee with | TOR_Reject _0 -> true | uu___ -> false
let __proj__TOR_Reject__item___0 (projectee : tpl_opt_result) : Prims.string=
...
let sandbox_op (usergraph : RDF_Graph_Executable.wf_iri)
  ...
  | SPARQL11_Algebra.U_Modify (w, del_tpl, ins_tpl, using, where) ->
      (match check_tpl_opt "INSERT/DELETE: DELETE clause" del_tpl usergraph
       with
      ...
      (match check_gref "ADD source" src usergraph with
       ...
       | GRCR_Ok ->
           (match check_gref "ADD dest" dst usergraph with
            | GRCR_Ok -> SB_Ok (SPARQL11_Algebra.U_Add (silent, src, dst))
            | GRCR_Reject msg -> SB_Reject msg))
      ...
      (match check_gref "MOVE source" src usergraph with
       ...
       | GRCR_Ok ->
           (match check_gref "MOVE dest" dst usergraph with
            | GRCR_Ok -> SB_Ok (SPARQL11_Algebra.U_Move (silent, src, dst))
            | GRCR_Reject msg -> SB_Reject msg))
      ...
      (match check_gref "COPY source" src usergraph with
       ...
       | GRCR_Ok ->
           (match check_gref "COPY dest" dst usergraph with
            | GRCR_Ok -> SB_Ok (SPARQL11_Algebra.U_Copy (silent, src, dst))
            | GRCR_Reject msg -> SB_Reject msg))
...
let uu___is_USR_Ok (projectee : update_sandbox_result) : Prims.bool=
  match projectee with | USR_Ok _0 -> true | uu___ -> false
let __proj__USR_Ok__item___0 (projectee : update_sandbox_result) :
  SPARQL11_Algebra.sparql_update= match projectee with | USR_Ok _0 -> _0
let uu___is_USR_Error (projectee : update_sandbox_result) : Prims.bool=
