...
module Util_Log_runtime = struct
  ...
  let level_of_int : int -> lvl = function
    | 0 -> Error | 1 -> Warn | 2 -> Info | 3 -> Debug | _ -> Trace

  let int_of_level : lvl -> int = function
  ...
  let set_level (l : lvl) : unit =
    Mutex.lock mutex; threshold := l; Mutex.unlock mutex
  ...
  let do_emit (l : lvl) (modname : string) (msg : string) : unit =
    ensure_init ();
    if int_of_level l <= int_of_level !threshold then begin
