#!/bin/bash
# Experimental runtime glue for RDF.CottasInMem.fst — Phase A scaffold.
#
# Design note: docs/designissues/in-memory-cottas-encoder.md
# Branch: claude/inmem-cottas-phase-a
#
# This patch realises the `cottas_inmem_open` assume-val. Phase A: stub
# returns None so the skeleton compiles, links, and stays W3C-clean.
# Phase A.5 will replace the stub body with a real encoder + buffer
# registry hookup; that work is intentionally NOT in this commit.
#
# Rule #11 boundary: when filled out in Phase A.5, this file will
# contain pure byte-layout code (footer + 4 string columns + row
# count) plus a process-wide buffer registry. NO semantic logic
# (which rows match which bound, which terms are equal, etc.) lives
# here — that's all in RDF.CottasStore.fst (extracted F* code). This
# patch only produces bytes the existing F* reader will consume.
...
FILE="$OUTDIR/RDF_CottasInMem.ml"
if [[ ! -f "$FILE" ]]; then
  echo "  Note: $FILE not found, skipping COTTAS in-mem encoder glue" >&2
  exit 0
fi

if grep -q 'module Cottas_inmem_encoder' "$FILE"; then
  echo "  COTTAS in-mem encoder glue already present."
  exit 0
fi
...
# Phase A skeleton: replace the extracted assume-val stub
#   `failwith "Not yet implemented: RDF.CottasInMem.cottas_inmem_open"`
# with a buffer-registry-aware body that, for now, always returns None.
# Phase A.5 will swap in the real encoder.
stub_pattern = re.compile(
    r'let cottas_inmem_open \(graph_id : Prims\.string\) \(g : [^)]+\) :\n'
    r'  RDF_CottasStore\.cottas_ondisk_store FStar_Pervasives_Native\.option\n?'
    r'(?:  =\n)?'
    r'  failwith "Not yet implemented: RDF\.CottasInMem\.cottas_inmem_open"\n?'
)

# Loose fallback: extraction may emit slightly different argument types
# (e.g. Prims.list of triples) — match by function name + failwith only.
loose_pattern = re.compile(
    r'(let cottas_inmem_open [^\n]*\n(?:[^\n]*\n)?'
    r'  (?:[^\n]*\n)?'
    r')  failwith "Not yet implemented: RDF\.CottasInMem\.cottas_inmem_open"\n'
)

impl = '''(* ---- Phase A scaffold ----------------------------------------------
...
if stub_pattern.search(content):
    content = stub_pattern.sub(impl, content, count=1)
elif loose_pattern.search(content):
    content = loose_pattern.sub(impl, content, count=1)
else:
    raise SystemExit("cottas_inmem_open stub not found in RDF_CottasInMem.ml")

path.write_text(content)
PYEOF

echo "  Applied COTTAS in-mem encoder Phase A scaffold to $FILE"
