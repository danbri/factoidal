...
def main (args : List String) : IO UInt32 := do
  ...
  | .ok root =>
    ...
    IO.println   "  F*  (skills/test-suites row):   399 pass, 57 fail, 11 skip (out of 467)"
    IO.println   "  CAVEAT: that F* row is dated 2026-07-05. jsonld_runner.ml's own comments"
    IO.println   "  record later fix waves (2026-07-16 skip audit, 2026-07-17 term-redefinition"
    IO.println   "  wave) that emptied its skip allowlist, so \"11 skip\" cannot still be current."
    IO.println   "  No darwin-arm64 jsonld_runner binary is committed, so the F* side was NOT"
    IO.println   "  re-measured here. Treat the F* column as a stale lower bound, not a live"
