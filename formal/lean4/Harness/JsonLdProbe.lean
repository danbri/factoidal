...
def main (args : List String) : IO UInt32 := do
  ...
  | .ok root =>
    ...
    IO.println ""
    IO.println s!"F* reference (formal/fstar, same manifest): 399 pass, 57 fail, 11 skip (out of 467)"
    IO.println s!"Lean  (this port):                          {pass} pass, {failN} fail, {skipN} skip (out of {total})"
    IO.println ""
