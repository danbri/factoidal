/- Reusable physical publication of an already-admitted RDF graph as fresh
   predicate-local IBK2 artifacts and an SBM2 Merkle manifest. -/
...
private def publishBlocks (output : String) : PublishState →
...
def publishTriples (output : String) (sourceIdentity : ByteArray)
    (layout : String) (triples : List L4Factoidal.RDF.Triple) : IO Nat := do
  ...
  let published ← publishBlocks output {} (blocksOfBuckets (addTriples {} triples))
  let entries := published.entriesRev.reverse
  let lines := published.linesRev.reverse
  let manifest : Manifest :=
    { version := 2, sourceIdentity, termRegistryVersion := "local-ibk2-dict-v0", layout, entries }
