...
Three rules this module keeps:
  ...
  * a test type this tree cannot attempt is `unsupported <type>`, named.
    ...
    `unsupported <type>`. An entailment-regime evaluation test runs
    when the regime list names RDFS, RDF or D (the closure of
    `RDF/Entailment.lean` is applied to the data before evaluation —
    see "Entailment regimes" below); a test naming only OWL-Direct,
    OWL-RDF-Based or RIF is `unsupported entailment regime <R>`. The
    rdf-mt `PositiveEntailmentTest` / `NegativeEntailmentTest` types
    run through the same module (see "RDF 1.1 Semantics").
