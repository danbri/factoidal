/-
Cutting a predicate's rows into several IBK4 blocks is a PARTITION of the row
...
the blocks of one predicate denote the same quads in the same order as the one
...
theorem chunkGo_flatten (quads : List QuadRow) :
    ∀ (accRev : List QuadRow) (rows bytes : Nat) (graph : Option GraphRef),
      (chunkGo quads accRev rows bytes graph).flatten = accRev.reverse ++ quads := by
  induction quads with
  | nil =>
      intro accRev rows bytes graph
      cases accRev with
      | nil => simp [chunkGo]
...
/-- Cutting a predicate's rows loses nothing, adds nothing and reorders
    nothing. -/
theorem chunkQuadRows_flatten (quads : List QuadRow) :
    (chunkQuadRows quads).flatten = quads := by
  rw [chunkQuadRows]
  simpa using chunkGo_flatten quads [] 0 0 none
