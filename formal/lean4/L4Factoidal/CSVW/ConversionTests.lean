...
#guard prepareLexical (some (.named "string")) "  x  " == "  x  "
#guard prepareLexical (some (.named "date")) " 2010-10-18 " == "2010-10-18"
#guard prepareLexical none "  x  " == "  x  "   -- absent datatype is string
...
#guard (convertCell nullCol "c" look "NA").literals == []
...
#guard (convertCell urlCol "c" look "ignored").literals == []

-- A separated cell yields one literal per element.
private def listCol : Inherited := { separator := some "," }
#guard (convertCell listCol "c" look "a,b,c").literals == ["a", "b", "c"]
