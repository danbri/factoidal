...
This increment establishes two reusable Lean theorems in
...
The next DLE1 theorem must be explicitly bounded:
...
length field.  Before lifting the theorem, normalize the `drop`/`take`/
append chain into small list lemmas rather than relying on one large `simp`.
The later proof chain should make public writer refusal explicit for all u32
length fields, then port string, RDF term, triple, entry, batch, whole-log,
and compaction-filter refinement results in that order.
