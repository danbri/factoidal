...
| **JSON-LD 1.1** | toRdf 461 pass, 0 fail, 6 skip (of 467); fromRdf 53 pass, 0 fail, 1 skip (of 54); expand 379 pass, 0 fail, 6 skip (of 385) [2026-07-10] | **compact / flatten / frame / html API suites NOT RUN — counted as failed until measured**. expand is now MEASURED: `bin/jsonld-expand-runner` compares expanded JSON via `Parser.JSONLD.jsonld_expanded_equal` (JCS-canonical structural equality — object member order insignificant, array order significant, numbers by canonical value); the expansion-conformance work that landed with it in `JSONLD.Expand.fst` (free-floating drop, null-value entry drop, duplicate-expanded-key merge, lexicographic member/container-map ordering with @nest deferral, @index retention + container-key injection, language-map @direction, keyword-lookalike @id→null, literal unresolvable @type) is toRdf/fromRdf-neutral (floors re-measured, held). The 13 skips across the three suites are enumerated JSON-LD-1.0-version fixtures (documented per-ID in the runners; expand's 6: t0038, t0071, t0115, t0116, ter24, ter32 — run under JLD_NO_SKIP=1 to re-measure) | partial / ✓ (post 07) |
