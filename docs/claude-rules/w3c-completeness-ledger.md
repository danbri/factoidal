...
| **SPARQL 1.1** | 623/8; regimes 66/70 | runner-integrity fix (strict RDFC-1.0 iso for CONSTRUCT/UPDATE graphs, Jena-style row reification for bnode SELECT results, ASK boolean now compared) surfaced 8 real failures the lenient runner hid — **BNODE()** collapses distinct blank nodes (BNODE(), BNODE(str)); the OWL min/max-cardinality rewrite leaks internal `?_mc_`/`?_mxqc1_` variables into results (parent hasChild min-1, parent hasChild max-1 Female — this is why the anchor rewrite's "70/70" was partly leniency, per CLAUDE.md Known-rewrites #236); ASK-boolean inference/function gaps (RDF inference test, RIF Brain Anatomy, NOW(), UUID() per binding). 0 canonicalization-budget fallbacks. Also: parser `--admit_smt_queries` (#91); Protocol/GSP parked | ✓ / ✓ |
