...
|---|---|---|---|
| **CSVW** (all four output modes) | csv2rdf 270 pass, 0 fail (out of 270) ✅ · csv2json 270 pass, 0 fail (out of 270) ✅ [2026-07-24, `CSVW.Json.fst` serializer, runner `--json`] · validation 281 pass, 1 fail (out of 282) [2026-07-24, `CSVW.Validate.fst`, runner `--validate`] · non-normative 4 pass, 14 fail (out of 18) [dialect `delimiter`/TSV added] | **csv2rdf + csv2json GREEN.** csv2json reuses the annotated-table machinery and re-serializes per the Recommendation (datatype→JSON-primitive, propertyUrl compaction, aboutUrl `@id` grouping, valueUrl→aboutUrl nesting, JSON-LD-to-JSON for notes/common). **validation** (259→280 this session): metadata-structural (@id/@type role, required url, columns/tableSchema/titles shape, language-tag well-formedness) + data-level (cell datatype validity, single-column primaryKey uniqueness, required-column, column-count compat, case-sensitive title compat). This session added: language-aware title compat (test148), multi-column primaryKey uniqueness (test234), cross-table foreign keys incl. `schemaReference`/referenced-schema resolution (test257/258/034/035), and section-4 ignore-non-object-array-members (test094/096). Remaining 1: test308 (a non-built-in absolute-URL datatype string expected as an error, but the near-identical test238 is a WarningValidationTest expecting conformance — mutually exclusive under any value-agnostic rule). **non-normative** (csv2json, non-conformance-gating) 4/18: dialect `delimiter`/TSV added this session (test050). Remaining 14 are the dialect layer, decoded but not yet applied in the shared conversion: trim modes true/start/end/false (test020/021/022/262 — correct handling needs the RFC-4180 tokenizer to track quoted-ness so quoted whitespace is preserved), `commentPrefix` (test051/052), `skipColumns` (test054), `skipBlankRows` (test055), `skipInitialSpace` (test056), dialect-as-URL reference (test057), table-vs-tablegroup dialect precedence (test058), header=false+skipRows (test025), no-header (test019), and same-column-count-per-row (test091, negative). Descriptor: `.github/test-suites/csvw-nonnorm.yaml`. All CSVW F\* modules verify under z3 4.13.3, no `--lax`. Descriptors: `.github/test-suites/csvw-{csv2rdf,csv2json,validation,nonnorm}.yaml` | ✓ / ✓ (post 13) |
