...
| **SPARQL 1.2** | 240 pass, 14 fail (out of 254) [2026-07-18, was 237→232→219→100] | Triple-term/reifier syntax + VERSION decl + codepoint escapes + directional lang tags + base-direction builtins, all gated behind the `sparql12` lexer/parser bool so SPARQL 1.1 is byte-identical (631/0 protected). Families fully green: syntax-triple-terms-positive **113/0**, syntax-triple-terms-negative **65/0**, **version 9/0**, **codepoint-escapes 8/0**, rdf11 3/0, grouping 1/0. Near-green: **lang-basedir 10/1** (hasLANG/hasLANGDIR/LANGDIR/STRLANGDIR builtins + CONCAT direction-preservation landed; the `E_HasLang`/`E_StrLangDir`/etc. constructors ripple through SHACL.Validation + Parser.JSONResults exhaustive matches). Partial: eval-triple-terms **31/10** (triple-term value equality in value_compare + CONSTRUCT-WHERE reifier bnode-rewriting fixed — basic-9/op-2/construct-2). Remaining 14 fails: **10 eval-triple-terms** = a reifier-enumeration over/under-counting cluster (pattern-01/02/08, construct-5 share one root cause) + triple-term-in-`GRAPH` matching (graphs-1/2) + BIND-CONSTRUCT (expr-01) + ORDER (order-2) + UPDATE reifier templates (update-1/2); 1 lang-basedir; syntax 0/2 (TriG dataset-splitting); expression 0/1. Six waves landed today (100→240), SPARQL 1.1 631/0 held throughout. Verified clean (no `--lax`/admits). | 🟡 syntax+aux green / eval residue = reifier-enumeration cluster |
