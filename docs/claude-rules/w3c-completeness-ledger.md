...
|---|---|---|---|
| **CSVW** (all four output modes) | csv2rdf 270 pass, 0 fail (out of 270) ✅ · csv2json 270 pass, 0 fail (out of 270) ✅ [2026-07-24, `CSVW.Json.fst` serializer, runner `--json`] · validation 281 pass, 1 fail (out of 282) [2026-07-24, `CSVW.Validate.fst`, runner `--validate`] · non-normative 10 pass, 8 fail (out of 18) [trim family + skipColumns added] | **csv2rdf + csv2json GREEN.** csv2json reuses the annotated-table machinery and re-serializes per the Recommendation (datatype→JSON-primitive, propertyUrl compaction, aboutUrl `@id` grouping, valueUrl→aboutUrl nesting, JSON-LD-to-JSON for notes/common). **validation** (259→280 this session): metadata-structural (@id/@type role, required url, columns/tableSchema/titles shape, language-tag well-formedness) + data-level (cell datatype validity, single-column primaryKey uniqueness, required-column, column-count compat, case-sensitive title compat). This session added: language-aware title compat (test148), multi-column primaryKey uniqueness (test234), cross-table foreign keys incl. `schemaReference`/referenced-schema resolution (test257/258/034/035), and section-4 ignore-non-object-array-members (test094/096). Remaining 1: test308 (a non-built-in absolute-URL datatype string expected as an error, but the near-identical test238 is a WarningValidationTest expecting conformance — mutually exclusive under any value-agnostic rule). **non-normative** (csv2json, non-conformance-gating) 4→10/18: this session added the trim family + skipColumns. The trim family (test020 start, test021 end, test022 true, test262 false, test056 skipInitialSpace) is handled the way the W3C expected outputs actually encode it — string cells KEEP their surrounding whitespace (base `string`, or no datatype which defaults to `string`), while non-string datatypes strip leading/trailing whitespace before lexical parsing (tabular-data-model 6.4.2), so ` 10/18/2010 ` parses to `2010-10-18` (`CSVW.Conversion.csvw_dt_preserves_ws` + `csvw_trim`); `skipColumns` (test054) drops the leading N cells of every row uniformly (`RML.Sources.csv_parse_rows_dialect`). Remaining 8 all need physical-row vs logical-row numbering (the `#row=N` URL fragment is the PHYSICAL source-line number while `rownum` counts only retained data rows; the conversion derives one from the other today, valid only with a one-line header and no skips): `commentPrefix`+`skipRows` (test051/052), `skipBlankRows` (test055), header=false+skipRows (test025), no-header (test019), dialect-as-URL reference (test057), table-vs-tablegroup dialect precedence (test058), and same-column-count-per-row (test091, negative). Descriptor: `.github/test-suites/csvw-nonnorm.yaml`. All CSVW F\* modules verify under z3 4.13.3, no `--lax`. Descriptors: `.github/test-suites/csvw-{csv2rdf,csv2json,validation,nonnorm}.yaml` | ✓ / ✓ (post 13) |
...
| OWL 2 DL type-inconsistency (tinc) | 126 pass, 1 fail (out of 127), 1 skip, zero oracle-assisted [re-measured 2026-07-30 in the single all-nine-catalog pass that landed the #326 absence-verdict fix]; unchanged by #326 — InconsistencyTest is fail-safe under a truncated closure (a missing derivation shows up as FAIL), so the absence-verdict gate does not touch it | ✅ fully dispositioned on `/web/conformance/owl2/`; the earlier same-day 124-vs-125 flip was a wall-clock refuter cap sitting on dl-502's ~5.5-CPU-second boundary (load-dependent verdict, both measurements correct for their machine state); caps are CPU-time now with a 30s inconsistency-path refuter budget, and 125 pass, 2 fail reproduces under load |
| OWL 2 DL positive entailment (PE) | 195 pass, 9 fail, 2 skip (out of 204) [re-measured 2026-07-30 in the single all-nine-catalog pass that landed the #326 absence-verdict fix]; the 173/31 this row used to carry came from a committed log that predated the `--semantics` work and had never been refreshed; reproduces identically in all three catalogs that carry the PE section (type-positive-entailment, type-consistency, semantics-direct); unchanged by #326 (fail-safe kind) | ✅ fully dispositioned: all 31 named per-test on `/web/conformance/owl2/` (20 by-design, 10 planned-family, 1 dependency-blocked) |
| OWL 2 DL negative entailment (NE) | 23 pass, 0 fail (out of 23) [re-measured 2026-07-30 in the single all-nine-catalog pass that landed the #326 absence-verdict fix] — ZERO NegativeEntailmentTests escaped their budget in any catalog, so #326 moved no NE verdict; earlier note: [re-measured 2026-07-28, `type-consistency.rdf`'s NE section — was 22 pass, 1 fail; `WebOnt-imports-002` fixed by gating `owl_runner.ml`'s `load_imports_into_premise` on an actual `owl:imports` triple in the premise, iterated to a fixpoint for the corpus's transitive-imports test] | ✅ fully dispositioned |
| OWL 2 RL positive entailment | 30 pass, 0 fail (out of 30) [re-measured 2026-07-30 in the single all-nine-catalog pass that landed the #326 absence-verdict fix]; the 28/2 was the stale `owl_profile_rl_results.log` #326 flagged (item 2) | ✅ fully dispositioned |
| OWL 2 DL syntax-dl species | 319 pass, 2 fail, 2 skip (out of 323) | ✅ fully dispositioned; ⚠️ ledger headline stale (302/2/19) |
| OWL 2 profile EL | 119 pass, 1 fail (out of 120) [re-measured 2026-07-30 in the single all-nine-catalog pass that landed the #326 absence-verdict fix] — the single fail is an `unsupported` Consistency verdict on `WebOnt-Thing-004` (closure cap-trip, #326), not a wrong answer; it does NOT recover at a 3x or 9x cap (23s -> 63s -> 182s catalog wall, still unsupported), so it is the #262 `owl:sameAs` blow-up rather than a budget to buy off | ✅ triaged 2026-07-28 (#308): remaining 2 = `WebOnt-I5.26-010` + `WebOnt-I5.5-005`, both **by-design**(OWL 1 Full comprehension, `scope.md`); named on `/web/conformance/owl2/` |
| OWL 2 profile QL | 87 pass, 0 fail (out of 87) [re-measured 2026-07-30 in the single all-nine-catalog pass that landed the #326 absence-verdict fix] — zero cap escapes | ✅ triaged 2026-07-28 (#308): remaining 2 = same 2 tests as EL, same by-design disposition; named on `/web/conformance/owl2/` |
| OWL 2 DL type-consistency | 337 pass, 15 fail (out of 352), 2 skip [re-measured 2026-07-30 in the single all-nine-catalog pass that landed the #326 absence-verdict fix] — all 15 fails are `unsupported` (cap escape, #326); was 352 pass, 0 fail in the same run before the absence-verdict gate | ✅ all 15 named on `/web/conformance/owl2/`; performance gap tracked in #326 / #262. Cap-raise measured 2026-07-30: closure 20->120 + refuter 10->60 CPU-seconds recovers FOUR as genuine completed passes (dl-206/-208/-209/-501, near-misses just past the 10s refuter cap) and costs 3.3x catalog wall (699s -> 2329s); the other eleven do not recover — Thing-004 and miscellaneous-001/-002/-011 never converge (9x measured on the EL instance), and dl-661..-664 merely swap a closure escape for a marker-scan escape because is_inconsistent is cubic in closure size. Published budgets therefore stay put |
| OWL 2 DL semantics-direct | Consistency 336 pass, 15 fail (out of 351), 2 skip; PE 195 pass, 9 fail (out of 204), 2 skip; NE 23 pass, 0 fail; Inconsistency 126 pass, 1 fail (out of 127), 1 skip [re-measured 2026-07-30 in the single all-nine-catalog pass that landed the #326 absence-verdict fix] — the 15 Consistency fails are all `unsupported` (cap escape, #326); the committed log this row's predecessors read from was two engine waves stale | ✅ agrees test-for-test with the dedicated catalogs, measured in the same pass |
...
| ShEx validation | 1182 pass, 0 fail (out of 1182) | ✅ upstream reconciled the fixture 2026-07-29; local override retired |
...
- `Minus Infinity is not in owl:real` — **FIXED 2026-07-29** by the XSD
  value-space decision procedure (`XSD.Facets.fst` + `Tableau.Refute.fst`
  section 5a'/5b'; design note
  `docs/designissues/2026-07-29-xsd-value-space-decision-procedure.md`).
  `owl:real` is now a first-class dense value space, OWL 2 Syntax
  section 4.1 keeps the `xsd:float` value space outside it, and a
  `NegativeDataPropertyAssertion` removes the one remaining admissible
  value. type-inconsistency is 126 pass, 1 fail (out of 127).
...
- **20 by-design.** 17 of them carry `test:species FULL` plus an
  ...
  `test:species DL`, i.e. the W3C suite states they are not OWL DL
  entailments: `WebOnt-Class-001/002/003`,
  ...
  `WebOnt-I5.26-010` (OWL 1 Full comprehension, `scope.md`).
  ⚠️ The owner approved a flag-gated OWL Full engine mode on
  2026-07-28 ("If we can do it - eg under a flag or engine mode or api
...
- **1 dependency-blocked.** `WebOnt-equivalentProperty-004` — premise
  property-inclusion outside the refuter's role hierarchy (#298).

The former grouped breakdown against 69 fails is superseded: the
tableau-classification wave (`5eed66e`) and the closure-rule wave
(`0e9406b`) closed the whole DL98 stress group
(`description-logic-201/202/206/208/661/662/901/903`,
`Consistent-but-all-unsat`), the AllDifferent/oneOf/unionOf
comprehension rules, the vocabulary axiom family, and
`rdfsext-domain/range-subprop` (previously listed here as
dependency-blocked — it now passes).
...
### OWL 2 profile EL / QL — 118 pass, 2 fail (of 120) / 85 pass, 2 fail (of 87) [re-measured 2026-07-28]

🧭 Both rows were new in `docs/test-results/latest.json` (commit
`0f4090d5`, ancestor-confirmed) with no matching row in this ledger and
no GitHub issue found referencing either suite name at the time. Since
then, an `OWL.Closure.fsti` fix (2026-07-28) flipped 10 EL tests + 2 QL
tests that `latest.json` had not yet picked up (stale dashboard, no
code change needed) — regenerated via `formal/fstar/generate-report.sh`;
numbers above re-confirmed by a fresh catalog re-run 2026-07-28
(EL 118 pass, 2 fail out of 120, 1 skipped; QL 85 pass, 2 fail out of
87).

✅ Triaged 2026-07-28 (#308). Remaining 2 fails on each catalog are
`WebOnt-I5.26-010` + `WebOnt-I5.5-005`, both **by-design**(OWL 1 Full
comprehension-principle entailments, deliberately outside the OWL 2
RL/EL/QL closure) — the disposition `docs/claude-rules/scope.md`'s
"OWL 1 Full comprehension-principle entailments" section already
records, now applied per-catalog. Public statement with per-test
reasons: `docs/web/conformance/owl2.md`. The EL/QL 1-skip difference
is `WebOnt-Thing-005` (`test:semantics RDF-BASED` only, `DIRECT`
explicitly denied), present in the EL catalog and absent from QL.
...
### ShEx validation — 1182 pass, 0 fail (out of 1182)
