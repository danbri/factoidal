...
| OWL 2 profile EL | 118 pass, 2 fail (out of 120) [re-measured 2026-07-28; was 108 pass, 12 fail — `latest.json` had not picked up an `OWL.Closure.fsti` fix landed earlier the same day] | 🧭 untriaged — no owner disposition on file; remaining 2 = `WebOnt-I5.26-010` + `WebOnt-I5.5-005` |
| OWL 2 profile QL | 85 pass, 2 fail (out of 87) [re-measured 2026-07-28; was 83 pass, 4 fail, same cause] | 🧭 untriaged — no owner disposition on file; remaining 2 = same 2 tests as EL |
...
this session; numbers above are the fresh measurement. Still not
individually triaged. Remaining 2 fails on each catalog are
`WebOnt-I5.26-010` + `WebOnt-I5.5-005` — see `docs/claude-rules/
scope.md`'s "OWL 1 Full comprehension-principle entailments" section,
which already dispositions these two as permanent (OWL 1 Full
comprehension, out of DL/RL scope). Original note, otherwise
unchanged: these look like the "OWL profiles" work the task brief
