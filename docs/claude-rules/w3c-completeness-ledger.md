...
| **SPARQL 1.2** | 219 pass, 35 fail (out of 254) [2026-07-18, was 100/154] | Triple-term / reifier SYNTAX landed (#305): bare `<< s p o (~ reifier)? >>` in subject/object/standalone slots, `{| predicateObjectList |}` annotations, VALUES `TripleTermData`, all gated behind the `sparql12` lexer/parser bool so SPARQL 1.1 is byte-identical (631/0 protected). Per-family: syntax-triple-terms-positive **113/0**, syntax-triple-terms-negative **65/0** (no false-accepts), eval-triple-terms 28/13, rdf11 3/0, grouping 1/0. Remaining 35 fails are SEPARATE 1.2 features + pre-existing eval bugs, each named in the landing commit: version 3/6 + lang-basedir 4/7 + codepoint-escapes 2/6 (distinct feature families, next wave); syntax 0/2 (TriG dataset-splitting in the runner); eval residue = a pre-existing CONSTRUCT-WHERE-blank-node-template bug, a FILTER IN triple-term equality gap, a runner graph-dedup gap (2 fixtures), and a SPARQL 1.1 Update `GRAPH ?g` fixed-IRI-only limitation. Verified clean (no `--lax`/admits). | 🟡 syntax green / eval + aux families named |
...
| XSLT 1.0 conformance (Apache Xalan mirror) | **997 pass, 685 fail, 8 skip (out of 1690)** [2026-07-18, was 970/712/8 at vendoring, then 988/694 after the with-param fidelity fix, now 997/685 after the `namespace::` axis (#302) — measured burndown, not a pin]. Corpus: OASIS XSLT 1.0 conformance cases as mirrored by `apache/xalan-test` (Apache-2.0), vendored as submodule `third_party/testing/xslt1-xalan/xalan-test-src` + generated `manifest.json`; run via `xslt_runner --base third_party/testing/xslt1-xalan` (same F\*-extracted `XSLT.Transform` oracle). ⚠️ 4 of the 685 fails are PRE-EXISTING non-ASCII engine crashes (`Char.chr` on codepoint>255 — split-brain string primitive): sort-alphabet-polish/russian, output28/73; caught by the runner's per-test guard (ENGINE-EXN → counted fail, not corpus abort). Tracked separately (#307). Remaining fails cluster onto the matrix holes: numberformat 44 (#6), impincl 29 (#7), numbering ~91 (#4), namespace-alias (#16), attribset 44 (#14), idkey 44 (#5/#10), output ~75 (#18), reluri+mdocs (document() partial), string 40 (translate #2), boolean+expression (lang #3), extend 5 (fallback #17), whitespace 15 (#15), message 2 (#9), processorinfo 1 (system-property #11), plus general axis/match/select/copy corners. Per-category table: `third_party/testing/xslt1-xalan/INFO.txt`. Provenance + license-verdict table: `third_party/testing/xslt1-xalan/PROVENANCE.md`. | 🟡 gap named / burndown target (matrix) |
