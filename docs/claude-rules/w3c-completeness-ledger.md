...
| **SPARQL 1.2** | 232 pass, 22 fail (out of 254) [2026-07-18, was 219/35 then 100/154] | Triple-term/reifier syntax + VERSION decl + codepoint escapes + directional lang tags, all gated behind the `sparql12` lexer/parser bool so SPARQL 1.1 is byte-identical (631/0 protected). Families fully green: syntax-triple-terms-positive **113/0**, syntax-triple-terms-negative **65/0**, **version 9/0**, **codepoint-escapes 8/0**, rdf11 3/0, grouping 1/0. Partial: eval-triple-terms 28/13, lang-basedir 5/11 (DATATYPE fixed; the `<< s p o ~r >>` reifier + `{|...|}` annotations parse). Remaining 22 fails, each named in the landing commits: **6 lang-basedir** need new builtins hasLANG/hasLANGDIR/LANGDIR/STRLANGDIR + CONCAT direction-preservation (evaluator work, SPARQL11.Algebra); **13 eval-triple-terms** = a pre-existing CONSTRUCT-WHERE-blank-node-template bug + FILTER IN triple-term equality gap + runner graph-dedup gap + Update `GRAPH ?g` fixed-IRI-only; syntax 0/2 (TriG dataset-splitting), expression 0/1. Verified clean (no `--lax`/admits). | 🟡 syntax+aux green / eval families named |
