...
|---|---|---|---|
| **CSVW** (all four output modes) | csv2rdf 270 pass, 0 fail (out of 270) ✅ · csv2json 270 pass, 0 fail (out of 270) ✅ [2026-07-24, `CSVW.Json.fst` serializer, runner `--json`] · validation 272 pass, 10 fail (out of 282) [2026-07-24, `CSVW.Validate.fst`, runner `--validate`] · non-normative 3 pass, 15 fail (out of 18) | **csv2rdf + csv2json GREEN.** csv2json reuses the annotated-table machinery and re-serializes per the Recommendation (datatype→JSON-primitive, propertyUrl compaction, aboutUrl `@id` grouping, valueUrl→aboutUrl nesting, JSON-LD-to-JSON for notes/common). **validation** (259→272 this session): metadata-structural (@id/@type role, required url, columns/tableSchema/titles shape, language-tag well-formedness) + data-level (cell datatype validity, single-column primaryKey uniqueness, required-column, column-count compat, case-sensitive title compat). Remaining 10: foreign keys (test257/258, cross-table), language-title compat (test148), referenced-schema roles (test034/035), multi-column primaryKey (test234), string regex `format` (test154), datatype-URL edge (test308), 2 decoder graceful-degradation warnings (test094/096). **non-normative** 15 fails are shared conversion-layer gaps (TSV delimiter dialect, `contentType` header=absent, date `format` facets), not serializer bugs; non-conformance-gating. All CSVW F\* modules verify under z3 4.13.3, no `--lax`. Descriptors: `.github/test-suites/csvw-{csv2rdf,csv2json,validation}.yaml` | ✓ / ✓ (post 13) |
