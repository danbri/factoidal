...
|---|---|---|---|
| **CSVW** (all four output modes) | csv2rdf 270 pass, 0 fail (out of 270) ✅ · csv2json 270 pass, 0 fail (out of 270) ✅ [2026-07-24, `CSVW.Json.fst` serializer, runner `--json`] · validation 280 pass, 2 fail (out of 282) [2026-07-24, `CSVW.Validate.fst`, runner `--validate`] · non-normative 3 pass, 15 fail (out of 18) | **csv2rdf + csv2json GREEN.** csv2json reuses the annotated-table machinery and re-serializes per the Recommendation (datatype→JSON-primitive, propertyUrl compaction, aboutUrl `@id` grouping, valueUrl→aboutUrl nesting, JSON-LD-to-JSON for notes/common). **validation** (259→280 this session): metadata-structural (@id/@type role, required url, columns/tableSchema/titles shape, language-tag well-formedness) + data-level (cell datatype validity, single-column primaryKey uniqueness, required-column, column-count compat, case-sensitive title compat). This session added: language-aware title compat (test148), multi-column primaryKey uniqueness (test234), cross-table foreign keys incl. `schemaReference`/referenced-schema resolution (test257/258/034/035), and section-4 ignore-non-object-array-members (test094/096). Remaining 2: string regex `format` validation (test154, needs a regex matcher in the cell checks) and test308 (a non-built-in absolute-URL datatype string expected as an error, but the near-identical test238 is a WarningValidationTest expecting conformance — mutually exclusive under any value-agnostic rule). **non-normative** 15 fails are shared conversion-layer gaps (TSV delimiter dialect, `contentType` header=absent, date `format` facets), not serializer bugs; non-conformance-gating. All CSVW F\* modules verify under z3 4.13.3, no `--lax`. Descriptors: `.github/test-suites/csvw-{csv2rdf,csv2json,validation}.yaml` | ✓ / ✓ (post 13) |
