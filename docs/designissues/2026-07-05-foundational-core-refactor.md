...
with reduced scope, and step 1 (`RDF.IRI` consolidation, §2.7) landed
2026-07-05 once the JSON-LD agents that owned `SPARQL11.IRI.Resolve.
fst` finished** — see the step-3.3-table update below for all three
steps' scope and measured evidence. Step 3's row also records a
cyclic-module-dependency finding that changes what step 6 must carry
(the RDF-specific `indexed_graph` glue this step could not move out).
Everything else below is still design
