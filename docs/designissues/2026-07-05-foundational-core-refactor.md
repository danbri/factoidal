...
fst` finished, and step 5 (`RDF.Term`/`RDF.Triple`/`RDF.Graph` split,
§2.1-2.2 — the owner's skimmable-core deliverable) landed
2026-07-05** — see the step-3.3-table update below for each step's
scope and measured evidence. Step 3's row also records a
cyclic-module-dependency finding that step 5's landing has now
unblocked (the RDF-specific `indexed_graph` glue folds into
`RDF.Indexed` at step 6). Steps 4, 6, and 7 below are still design
proposal, no code changes. Extends the roadmap in
