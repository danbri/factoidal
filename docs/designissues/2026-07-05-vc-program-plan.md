...
| 2 | `VC.Credential.fst` complete: `validFrom`/`validUntil` ordering, `credentialStatus`/`credentialSchema`/`termsOfUse` shape checks | **DONE (2026-07-06).** Measured: 109 pass, 5 fail, 6 skip (out of 120). Every check from the Stage 1 deferral list landed: issuer presence/shape (bare-string or object, object `id` optional-but-IRI-shaped when present), `credentialStatus`/`credentialSchema`/`termsOfUse`/`evidence`/`refreshService`/`proof` inner-shape checks (`type` required on all six, `id` additionally required on `credentialSchema` only — the one asymmetry the corpus's own `-ok` fixtures establish), `validFrom`/`validUntil` well-formedness + ordering (reuses `XSD.Datatypes.dt_parse_ms`/`dt_cmp`), top-level `id` cardinality/format plus the same rule one level down on `credentialSubject.id`, `holder` shape (object `id` REQUIRED, unlike issuer's optional `id` — `presentation-holder-name-fail.json` vs. `credential-issuer-object-ok.json` is the corpus's own evidence for the asymmetry), and `name`/`description` language-map extra-property rejection (closed 3-key set: `@value` required, `@language`/`@direction` optional) on both the document and `issuer`. Zero false-fails on any `-ok` fixture (the hard constraint). Residual 5 fails split into two documented, distinct-cause clusters: **4** are the context-driven type-redefinition cluster (`credential-redef-type-fail`, `credential-redef-type2-fail`, `credential-type-mapped-nonurl-fail`, `credential-type-unmapped-fail`) — genuinely needs real JSON-LD term resolution, out of scope until a context-processing module exists; **1** is `credential-validUntil-validFrom-fail.json`, a newly-identified residual (not anticipated in the Stage 1 deferral list): its `-ok` sibling uses literal placeholder text (`"PAST DATE"`/`"FUTURE DATE"`) that the vendored corpus's own `tests/assertions.js` `testTemporality()` helper overwrites with real computed timestamps at run time before ever issuing them — the raw vendored bytes this offline runner reads still carry the un-substituted sentinel strings, which are structurally identical (same shape, swapped) between the `-ok` and `-fail` file. Validating them as literal dateTime lexical forms would false-FAIL the `-ok` sibling, which the hard constraint rules out, so both are left unenforced — see `VC.Credential.fst`'s `vc_check_temporal_lexical`/`vc_check_validity_period` comments for the full reasoning. Achievable ceiling for a purely-structural (no JSON-LD term resolution) checker, against the 114 fixtures that carry a plain `-ok`/`-fail` verdict (120 total minus the 6 filename-ambiguous skips): **109 pass — already reached this landing.** The 4 type-redefinition fixtures are permanently out of reach without a JSON-LD context-processing module (114 - 4 = 110 would be the ceiling if only that cluster were excluded); the 5th (`credential-validUntil-validFrom-fail.json`) is additionally unreachable for the template-substitution reason above, which is why the achieved ceiling is 109, not 110. Floors reverified from repo root: SPARQL 631 pass, 0 fail; RDF 1031 pass, 0 fail; `tests/unit/run-all.sh` 28 file(s) pass, 0 fail. | Stage 1 |
| 2b (type-resolution) | `VC.Context.fst` — a minimal, pure, offline JSON-LD context TERM-RESOLUTION walker (NOT a general JSON-LD processor) wired into `VC.Credential.fst`'s `type`-value checks | **DONE (2026-07-09).** Measured: **113 pass, 1 fail, 6 skip (out of 120)** — up from 109. The four-fixture type-resolution cluster deferred at Stage 2 now passes: protected-term redefinition (`credential-redef-type-fail`, `credential-redef-type2-fail`), a type term whose context mapping is a non-URL (`credential-type-mapped-nonurl-fail`), and an unmapped type term with `@vocab` nullified (`credential-type-unmapped-fail`). `VC.Context.fst` walks the document's own `@context` array in order over `Parser.JSON.json_val`: it reads the vendored W3C VCDM v2 base context (`third_party/contexts/credentials-v2.jsonld`, parsed once by the consumer and passed in as `v2ctx` — the F\* side stays pure, no I/O, no `assume val`) to build a base term→(IRI, protected?) map, folds inline-context term definitions and `@vocab` set/nullify on top, records redefinition of a protected term as a violation, and resolves each `type` value (URL-shaped passthrough; term → its mapped IRI, which must be URL-shaped; else `@vocab`-expand; else UNMAPPED). Conservative choices keep the `-ok` corpus green (zero false-fails, the hard constraint): an unknown remote context URL (e.g. `.../credentials/examples/v2`) makes unmapped-term checks stand down (`credential-optional-type-ok`'s `RelationshipCredential` stays valid), and an unmapped `type` term is flagged ONLY when `@vocab` was explicitly set to `null` with no unknown remote context. New ceiling: **113** — the remaining 1 fail is `credential-validUntil-validFrom-fail.json`, the corpus template-substitution artifact from Stage 2 (unreachable for an offline reader). Floors reverified: SPARQL 631 pass, 0 fail; RDF 1031 pass, 0 fail. See `formal/fstar/VC.Context.fst`'s header and `third_party/contexts/PROVENANCE.md`. | Stage 2 |
