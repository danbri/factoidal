...
|---|---|---|---|
| 1 | Initialize `third_party/testing/vc` submodule; `VC.Credential.fst` skeleton (required-property + type-membership checks only) | A subset of the 120 `tests/input/*.json` fixtures (whichever checks land first) — measure once built, do not predict | none |
