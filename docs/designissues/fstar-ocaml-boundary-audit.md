...
| Glue patches that aren't `ASSUME-*` (Section 3) | 0 | **~7** (`VIOLATION-SEM`: `ballyhoo_hdt_runtime.sh`, `cottas_ondisk_runtime.sh`, `cottas_ondisk_z_lazy_open.sh`, #68. `MIXED`: `cottas_ondisk_zzzzz_ondisk_index.sh` (policy half — writer half done), `cottas_inmem_encoder_runtime.sh` (Phase A.5 deferred), `cottas_ondisk_zzzzzzzzzzzzzzzzz_token_lookup_runtime.sh`, `cottas_runtime.sh`, #62, #63, #103. — #65 retired in `db95b73`; #64 retired in `5c3535c`; #69 deleted as dead patch.) |
