...
| `cottas_ondisk_z_lazy_open.sh` | **Bet7.** Rewrites `build_handle_and_tables` to skip subject + object dictionary populate at open; adds `Cottas_ondisk_lazy` module that builds them on first call. | `VIOLATION-SEM` (in-flight #254 migration; Commits 1+3 + manifest landed `e116fed` / `c9bf634` / `ed39c2d`; Commit 2 — `cottas_ondisk_handle` integration — pending) | MIGRATE — `RDF.CottasStore.LazyDict.fst` (`assume new type lazy_dict (a : Type)` realised by `cottas_lazy_dict_runtime.sh` patch). 9-fn API: `mk_lazy_dict`, `decode_by_id`, `decode_raw_by_id`, `encode_by_key`, `encode_by_raw_token`, `is_populated`, `size`, `to_typed_list`, `to_raw_list`. |
