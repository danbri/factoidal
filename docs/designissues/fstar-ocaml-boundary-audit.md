...
| `103_parquet_ascii_string_fast_path.sh` | ASCII-only fast-path for `FStar_String` ops in `Parquet_Footer.ml` (works because hex-encoded payloads are pure ASCII). | `ASSUME-IO`-ish — observationally equivalent host-language optimisation; same caveat as #95. | Phase A landed 2026-05-14: `fs_string_of_list_ascii` primitive added to `Parser.FastString.fst` with realisation in `89_fast_string_primitives.sh`. Phases B-D pending — see [`docs/designissues/2026-05-14-issue-103-parquet-fastring-migration-plan.md`](2026-05-14-issue-103-parquet-fastring-migration-plan.md). Issue #103. |
