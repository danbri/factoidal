...
|---|---|---|---|
| `ballyhoo_hdt_runtime.sh` | **RETIRED 2026-07-06, patch deleted at `57cb2ee`** (HDT stage 4): `Parser.BallyhooHDT.fst` is pure Tot F\* — the 12 `assume val`s + `assume type hdt_handle` replaced by calls into the verified HDT.Container/Dictionary/Triples readers; only remaining I/O is the shared `parquet_read_range_hex` byte-range boundary. | `RETIRED` (was `VIOLATION-SEM`, #253) | `RDF.Store.LazyTermCache.fst` + `hdt_lazy_term_cache_runtime.sh` remain as an unused candidate stage-5 memoization seam (rule-#11 `ASSUME-IO`); HDT itself no longer consumes them. |
...
| `cottas_ondisk_runtime.sh` | (1) Implements `cottas_ondisk_open` (parquet decode + dict/revmap construction). (2) **Replaces** the F\*-extracted `cottas_ondisk_search` / `_estimate` / `_decode_*` with `Hashtbl`-backed equivalents. Per the patch's own header: "PERFORMANCE: replace the extracted F\* … the F\* definitions in `RDF.CottasStore.fst` remain the verification spec." | `VIOLATION-SEM` (in-flight #118 migration; Phase 2.5b+c landed at `cfa10ab` + `62d1acb` — `RDF.CottasStore.OnDiskRuntime.fst` exposes 8 `assume_val` perf primitives + realisation patch routes them to `Cottas_ondisk_runtime.{decode,encode}_*_fast`; BUILD_STATUS=OK, W3C 169/169 sanity passing. Phase 2.5d/e/f consumer migrations + Phase 2.5g round-trip lemmas pending) | MIGRATE — Phases 4-7 per design plan. Phase 2.5b+c done. **CORRECTION 2026-07-19:** the `cottas_ondisk_encode_*` / `_decode_*` (Tot + `_ml`) bodies Phase 2.5d planned to migrate are now confirmed DEAD (zero live callers — production is token-based); the `OnDiskRuntime` registry `assume val`s are likewise orphaned (no live F\* caller). So Phase 2.5d is moot: the dead id-based decode chain is being deleted (Step A, `9f482943` removed the row→quad half; the encode/decode + non-tok search/estimate/count bodies + their orphaned `cottas_ondisk_runtime.sh` sed-swaps are the remaining Step-A subtraction). The only perf-relevant live COTTAS glue after Step A is the predicate/graph lazy-populate (see the Bet7 row). |
| `cottas_ondisk_z_lazy_open.sh` | **Bet7.** Rewrites `build_handle_and_tables` to skip subject + object dictionary populate at open; adds `Cottas_ondisk_lazy` module that builds them on first call. | `VIOLATION-SEM` (in-flight #254 migration; Phases 1, 3, 2b/2c/2d/2e landed: spec + realisation + manifest + universe-safe LazyDictRegistry + cottas_ondisk_open wiring + 6 _ml encode/decode variants in RDF.CottasStore.fst + factoidal-explain external consumer migration. **CORRECTION 2026-07-19:** the prior "remaining" note here (a Tot→ML cascade through `cottas_ondisk_row_to_quad`) was STALE — that whole id-based chain was already dead (zero live callers repo-wide); it was deleted as Step A at `9f482943` (row→quad chain) + `04c466c2` (the wider
dead surface: non-tok search/estimate/count, build_bound_qp_opt, all
encode/decode + `_ml`, and the orphaned OCaml glue sed-swaps) — production is
entirely token-based. **Step A is COMPLETE.** The genuine remaining Bet7 blocker is the still-LIVE predicate/graph lazy-populate: `cottas_ondisk_predicate_present` + `cottas_ondisk_named_graphs`, both declared `Tot` in `store_caps` but realised with Hashtbl-mutation side effects. Retiring them is Step B — it touches the documented 106s→0.023s open-time win and a 5-builder `sc_predicate_present` effect ripple, so it needs a perf-review checkpoint and belongs in `SPARQL.Plan.Loader.fst`, NOT a quick effect-signature change) | MIGRATE — `RDF.CottasStore.LazyDict.fst` (`assume new type lazy_dict (a : Type0)` realised by `cottas_lazy_dict_runtime.sh` patch). 9-fn API + 2 pure-Tot list_assoc helpers (`lookup_id_in_list`, `lookup_key_in_list`) for Tot consumers that want F\*-pure semantics with a materialised list. Plus `RDF.CottasStore.LazyDictRegistry` (5 path-keyed lookups, universe-safe). |
...
| `cottas_ondisk_zzzzzzzzzzzzzzzzz_token_lookup_runtime.sh` | Realises ONE `assume val`, `ondisk_token_tables_global`, by calling the lazy `ensure_*_loaded` + `Hashtbl.find_opt` once per dictionary direction. **UPDATED 2026-08-24:** it used to realise EIGHT — `ondisk_id_to_{subj,pred,obj,graph}_token_global` and `ondisk_lookup_{subj,pred,obj,graph}_id_global`, the two directions of one token dictionary, whose correctness requirement lived only in a prose comment in `RDF.CottasStore.fst`. The eight are now the fields of `cottas_token_tables`, the requirement is the `Type0` predicate `token_tables_agree_with`, `tables_of_handle_agree` proves the populated-handle instance satisfies it, and `build_qp_row_agrees` derives the consequence: under agreement the lazily-populated tables and the handle's own assoc-lists build the same `cottas_qp_row`. | `ASSUME-IO` (deferred read of the store file's four dictionary columns; rule #11 dispatch shim, no semantic decision, no byte layout) — was `MIXED` | RETIRED as a boundary question. What remains is the lazy-populate TIMING decision (Bet7 row), not the dictionary semantics. |
...
[**STALE — CORRECTION 2026-07-19:** all three have since migrated to the
`_tok` entry points (`cottas_memory_buffer_unit.ml` now has 10 `_tok` call
sites and zero id-based calls, verified; smoketest + factoidal-explain
likewise). The id-based path is dead; its row→quad half was deleted at
`9f482943` (Step A). Remaining live COTTAS glue = predicate/graph
lazy-populate only (Bet7 row). The three bullets below are archaeology:]
...
| `63_regex_hash_uuid_stubs.sh` | Realises `hash_md5`/`sha1`/…/`sha512` and UUID/STRUUID. **regex_match/regex_replace retired from this patch** — both are now pure, verified F\* over the Regex.Syntax/Exec/XSDPattern derivative engine (issue #304 phases 4-5); the patch's own header records the retirement. | `MIXED` — `hash_*` should become `ASSUME-CRYPTO` (HACL\* binding — partially landed for VC via `286_vc_hacl_ed25519_sha256_stubs.sh`, not yet for this general-purpose seam); UUID should become an F\*-defined `mk_v4` + `assume val random_bytes` per the IO-verification doc. | SPLIT — migrate UUID byte-format to F\* (`ThirdParty.UUID`); migrate hashes to HACL\* binding (`ThirdParty.HACL`). Issue #63. |
| `64_sparql_parser_escape_stubs.sh` | RETIRED 2026-05-11. All three escape functions now in pure F* `SPARQL11.Parser.fst`: `utf8_of_codepoint` (`1d2b669`), `process_iri_escapes` (`e79c387`), `process_string_escapes_opt` (`d92311d`). The hard one (`process_string_escapes` w/ surrogate `failwith` path) was retired by adding `Tok_INVALID : string -> token` + `first_invalid_token_msg` short-circuit in `parse_sparql_with_base` (`5c3535c`). The patch is now a no-op stub kept as audit trail. | `PURE-FSTAR` — was `VIOLATION-SEM` pre-`5c3535c` | DONE — #200 (`5c3535c`); 631/631 SPARQL 1.1 + 1031/1031 RDF 1.1 pass; W3C `syn-invalid-codepoint-escaped-bad-01.rq` correctly rejected. Issue #64 closed. |
...
| ~~`68_unicode_boundary_workarounds.sh`~~ | RETIRED 2026-05-13. Patch deleted entirely. F* `Parser.NTriples.fst` `valid_codepoint` now uses the spec-correct inclusive `<= 0xD7FF` bound; `safe_char_of_int` dispatches the boundary case to the new `Parser.FastString.unsafe_char_of_d7ff` `assume val`, realised in OCaml as a 1-line `Z.to_int` (`89_fast_string_primitives.sh`). The F\* stdlib's strict-`<` off-by-one in `FStar.Char.char_of_int` is bypassed — in the OCaml runtime, `FStar_Char.char` is just `int` and `char_of_int = Z.to_int` with no value check, so the shim is by-construction sound. Same one-character fix applied to the latent same-bug typo in `_fs_safe_char_of_int` (line 218 of patch 89) which would have silently corrupted U+D7FF in the #240 JSON/XML escape path. | `PURE-FSTAR` — was `VIOLATION-SEM` pre-`631b819` | DONE — #200 (`631b819` + `aae8457` patch-deletion); Issue #68 closed. |
| ~~`69_runner_io_glue.sh`~~ | DELETED 2026-05-11. The patch targeted `ocaml-output/w3c_runner.ml` — but `w3c_runner.ml` lives in `bin/w3c-runner/` (Phase 8 already done) and `ocaml-output/w3c_runner.ml` no longer exists, so the script's main body was dead (`exit 0` at the file-not-found check). All the I/O-glue logic the patch used to inject (namespace constants, sd:entailmentRegime detection, file_to_base_uri, query-file-as-base, negative-syntax exception handling) is now in `bin/w3c-runner/w3c_runner.ml` directly. The `current_base_iri_ref` / `parse_sparql` shape it referenced is gone after #65 Step 3 (`db95b73`). Patch file removed. | RETIRED |
| `89_fast_string_primitives.sh` | **CUT DOWN 2026-08-10** (FastString re-founding Steps 2/3, `docs/designissues/2026-08-10-faststring-refounding-plan.md`): used to realise 6 `assume val`s (`fs_byte_*`, `fs_cp_*`) in `Parser.FastString.fst`; those six are now real `Parser.FastString.Spec`-backed DEFINITIONS, no longer acknowledged GAPs under rule #3(a). Their fast OCaml body moved to `experimental_ocaml_glue/parser_faststring_ops_runtime.sh` as a rule-11(b) Option-B PERFORMANCE realisation (overrides `fs_*`, leaves the independently-verified `fs_*_spec` twins untouched; deleting it falls back to the Spec-backed body — slower, never wrong). What remains in patch 89: the ONE `assume val` this migration could not eliminate, `unsafe_char_of_d7ff` (now declared in `Parser.FastString.CharBoundary.fst`) — realised here as a 1-line `Z.to_int`. Step 4 (same day) proved all 8 facts `Parser.FastString.Axioms.fsti` states about the re-founded primitives as real theorems (`Parser.FastString.Axioms.fst`) — off the trust surface entirely. | `ASSUME-IO` (1 assume val: `unsafe_char_of_d7ff`, byte-indexed and host-fast-pathed for what F\* already proves correct) | ALLOWED — keep; document under `assume val` realisation policy. The former 6-primitive realisation is `PURE-FSTAR` now (Spec-backed definitions); its OCaml fast-path lives in `experimental_ocaml_glue/parser_faststring_ops_runtime.sh` as an Option-B perf realisation, not an `assume val` realisation. |
...
| Glue patches that aren't `ASSUME-*` (Section 3) | 0 | **~5** (`VIOLATION-SEM`: `cottas_ondisk_runtime.sh` (#118), `cottas_ondisk_z_lazy_open.sh` (#254). `MIXED`: `cottas_ondisk_zzzzz_ondisk_index.sh` (policy half — writer half done), `cottas_inmem_encoder_runtime.sh` (Phase A.5 deferred), `cottas_ondisk_zzzzzzzzzzzzzzzzz_token_lookup_runtime.sh`, `cottas_runtime.sh`, #62, #63, #103. — #65 retired in `db95b73`; #64 retired in `5c3535c`; #68 retired in `631b819` (+ patch deletion `aae8457`); #69 deleted as dead patch. Retirement plans for the remaining `VIOLATION-SEM` backend-perf shims landed at `5408641` / `e802765` (#254 / #118 design docs); `ballyhoo_hdt_runtime.sh` (#253) retired outright at `57cb2ee`.) |
