...
|---|---|---|---|
| `ballyhoo_hdt_runtime.sh` | Adds an HDT runtime-cache module to `Parser_BallyhooHDT.ml` (Hashtbl-backed term cache populated from F\*-verified probes). | `VIOLATION-SEM` (in-flight #253 migration; Commits 1+3 landed at `34c7a8d` — `RDF.Store.LazyTermCache.fst` spec + `hdt_lazy_term_cache_runtime.sh` realisation + manifest; Commit 2 — Parser.BallyhooHDT integration — pending) | MIGRATE — `RDF.Store.LazyTermCache.fst` (`assume new type lazy_term_cache (a : eqtype)` realised by `hdt_lazy_term_cache_runtime.sh`). 6-fn API: `mk_lazy_term_cache`, `lookup_by_id`, `lookup_by_value`, `is_populated`, `size`, `to_list`. Distinction from #254's `LazyDict`: 2 directions (HDT's Front-Coded dict has no raw view) vs 4 (cottas needs raw column tokens). |
