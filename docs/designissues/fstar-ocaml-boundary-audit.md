...
|---|---|---|---|
| `ballyhoo_hdt_runtime.sh` | Adds an HDT runtime-cache module to `Parser_BallyhooHDT.ml` (Hashtbl-backed term cache populated from F\*-verified probes). | `VIOLATION-SEM` (in-flight #253 migration; Commits 1+3+2.5b landed at `34c7a8d`/`e2841d2`/`8ab096a` — `RDF.Store.LazyTermCache.fst` spec + `hdt_lazy_term_cache_runtime.sh` realisation + `RDF.Store.HDTTermCacheRegistry.fst` (universe-safe path-keyed) + manifest. Phase 2.5c wiring deferred: HDT's incremental term-intern model conflicts with LazyTermCache's populate-once-then-immutable semantics; reconciling requires a populate-once snapshot-at-first-lookup design step that needs careful F\* spec work) | MIGRATE — `RDF.Store.LazyTermCache.fst` (`assume new type lazy_term_cache (a : Type0)` realised by `hdt_lazy_term_cache_runtime.sh`). Plus `RDF.Store.HDTTermCacheRegistry.fst` for path-keyed lookup. 6+4 fn API. Distinction from #254's `LazyDict`: 2 directions (HDT's Front-Coded dict has no raw view) vs 4 (cottas needs raw column tokens). |
...
| `103_parquet_ascii_string_fast_path.sh` | ASCII-only fast-path for `FStar_String` ops in `Parquet_Footer.ml` (works because hex-encoded payloads are pure ASCII). | `ASSUME-IO`-ish — patch surface shrunk to 1 of 5 overrides (`string_of_list` only); other 4 (index/strlen/length/sub) now dead post-Phase B. | Phases A + B landed 2026-05-14: Phase A added `fs_string_of_list_ascii` primitive (commit `d9f5c43`); Phase B migrated all 120 `String.length` + 24 `String.sub` + 2 `String.index` callsites in `Parquet.Footer.fst` to `Parser.FastString.fs_byte_*` primitives. F\* verifies clean (`fstar.exe --z3version 4.13.3 Parquet.Footer.fst`); W3C bind 10/10, rdf-turtle 313/313 unregressed. Phases C-D (3 `String.string_of_list` callsites + patch deletion) pending — see [`docs/designissues/2026-05-14-issue-103-parquet-fastring-migration-plan.md`](2026-05-14-issue-103-parquet-fastring-migration-plan.md). Issue #103. |
...
| Glue patches that aren't `ASSUME-*` (Section 3) | 0 | **~6** (`VIOLATION-SEM`: `ballyhoo_hdt_runtime.sh` (#253), `cottas_ondisk_runtime.sh` (#118), `cottas_ondisk_z_lazy_open.sh` (#254). `MIXED`: `cottas_ondisk_zzzzz_ondisk_index.sh` (policy half — writer half done), `cottas_inmem_encoder_runtime.sh` (Phase A.5 deferred), `cottas_ondisk_zzzzzzzzzzzzzzzzz_token_lookup_runtime.sh`, `cottas_runtime.sh`, #62, #63, #103. — #65 retired in `db95b73`; #64 retired in `5c3535c`; #68 retired in `631b819` (+ patch deletion `aae8457`); #69 deleted as dead patch. Retirement plans for the three remaining `VIOLATION-SEM` backend-perf shims landed at `5408641` / `e802765` (#254 / #253 / #118 design docs).) |
