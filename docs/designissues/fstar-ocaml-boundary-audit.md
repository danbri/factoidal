...
| ~~`103_parquet_ascii_string_fast_path.sh`~~ | ASCII-only fast-path for `FStar_String` ops in `Parquet_Footer.ml`. | `PURE-FSTAR` — was `ASSUME-IO`-ish pre-2026-05-14. | RETIRED 2026-05-14. Phases A/B/C/D all landed: A added `fs_string_of_list_ascii` to `Parser.FastString.fst` + realisation in `89_fast_string_primitives.sh`; B migrated 120 `String.length` + 24 `String.sub` + 2 `String.index` callsites to `Parser.FastString.fs_byte_*`; C rewrote the 3 `String.string_of_list` callsites onto Phase A's primitive (and lifted intermediate accumulators from `list FStar.Char.char` to `list (n:nat{n < 256})`); D deleted the patch. F\* verifies clean (`fstar.exe --z3version 4.13.3 Parquet.Footer.fst`). W3C bind 10/10, rdf-turtle 313/313, functions 75/75 all unregressed. Plan doc: [`docs/designissues/2026-05-14-issue-103-parquet-fastring-migration-plan.md`](2026-05-14-issue-103-parquet-fastring-migration-plan.md). |
