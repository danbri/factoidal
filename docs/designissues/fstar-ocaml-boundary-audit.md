...
| `MIXED` entries (Section 2) | 0 | **2** (`parse_and_run_timed`, `run_query` — both in the relocated `bin/factoidal-http/factoidal_http.ml`; target `SPARQL.HTTP.RunQuery.fst`. `try_static_route` retired into `SPARQL.HTTP.Routes.fst`.) |
