...
| `MIXED` entries (Section 2) | 0 | **3** (`parse_and_run_timed`, `run_query`, `try_static_route` — all in the relocated `bin/factoidal-http/factoidal_http.ml`; targets `SPARQL.HTTP.RunQuery.fst` and `SPARQL.HTTP.Routes.fst`) |
...
| Glue patches that aren't `ASSUME-*` (Section 3) | 0 | **~10** (`VIOLATION-SEM`: `ballyhoo_hdt_runtime.sh`, `cottas_ondisk_runtime.sh`, `cottas_ondisk_z_lazy_open.sh`, #64, #65, #68. `MIXED`: `cottas_ondisk_zzzzz_ondisk_index.sh` (policy half — writer half done), `cottas_inmem_encoder_runtime.sh` (Phase A.5 deferred), `cottas_ondisk_zzzzzzzzzzzzzzzzz_token_lookup_runtime.sh`, `cottas_runtime.sh`, #62, #63, #103. Plus `CONSUMER` #69 which should relocate to `bin/w3c-runner/` now Phase 8 is done.) |
