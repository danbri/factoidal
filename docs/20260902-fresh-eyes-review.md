...
| # | Fix | Where | Effort |
|---|---|---|---|
| 1 | Symlink `blockengine` and `lean4-performance` into `.claude/skills/`; add `lean4-performance` to CLAUDE.md | `.claude/skills/`, `CLAUDE.md` | small |
| 2 | Delete the three contradictory score entries; keep the 631/0 line | `formal/lean4/README.md:50-54` | small |
| 3 | Add `#guard` decode∘encode round trips, then theorems, for `SubjectRowIndexWireV2`, `TermLocalIndexWire`, `PagedTermDictionary`, `IndexedBlockWireV3` | `L4Factoidal/Storage/` | small / medium |
| 4 | Move byte helpers into `Bytes.lean`; one `fitsU32`; delete 8+ copies | `L4Factoidal/Storage/*` | medium |
| 5 | Promote `Cursor` to `Bytes.lean`; retire whole-artifact `List` copies at every decode entry | 8 decoders | large |
| 6 | Single source for layout strings: call `layoutConsistent`, delete 11+ Harness literals | `Harness/ShardActivate.lean:27-57` and two siblings | medium |
| 7 | Move `nextWriteEpoch`/`foldedThrough` into `DeltaLog.lean`; prove monotonicity | `Harness/CompactedEpoch.lean:43-52` | small / medium |
| 8 | Quarantine or delete manifest versions 0/1/3/4/5 arms and the V0 pipeline after a fixture check | `ShardManifest.lean`, `lakefile.lean:159-163` | small (owner call) |
| 9 | Fix `Bytes.lean` header; rename `decode`→`decode?` uniformly; rename the five `Prefix`s to `Header` with format prefix | `Storage/` | small |
| 10 | Refresh six stale skills; write `shardborough-storage` skill (lifecycle CLIs, format registry, corpus ladder, census); document the sync-file protocol and mediated-shipping rule in-repo | `skills/`, `CLAUDE.md` | medium |
| 11 | Worknote index with status column; pin each note to a commit; move the three non-Shardborough notes out of the series | `docs/` | small |
| 12 | Give `current-state.md` a Lean section or a pointer; regenerate CLAUDE.md's `assume val` count by script | `docs/claude-rules/`, `CLAUDE.md` | small |
