...
| `lean-wasm-native-smoke` | Checks of every Wasm dispatch ABI op | `formal/lean4/Wasm/native-smoke.sh` | ~5s | PASS — 120 pass, 0 fail (out of 120), re-measured 2026-09-07 | no |
...
| `lean-pin-l4geo` | `formal/lean4/Harness/GeoRun.lean`, the Lean twin of the F* unit pin `tests/unit/geosparql_v0_unit.ml` — WKT parse/serialize round trip, Simple Features predicates (including a point exactly on a polygon edge), `geof:distance`/`geof:envelope` | `lake exe l4geo`, no arguments | <1s | PASS — `geosparql-v0: 37 pass, 0 fail, 0 skip (out of 37)`, `HARNESS-DIAG geosparql-v0: registered=37` | no |
| `lean-pin-l4rdf-semantics` | `formal/lean4/Harness/RdfSemanticsRun.lean`, the Lean twin of the F* `--rdf12entail` runner — walks the real W3C RDF 1.2 Semantics entailment manifest (`third_party/testing/w3c/rdf/rdf12/rdf-semantics/manifest.ttl`, 47 entries: 32 `mf:PositiveEntailmentTest` + 15 `mf:NegativeEntailmentTest`) via the shared `Harness.loadManifest` / `Harness.runTest` (`Mode.rdf12`) | `lake exe l4rdf-semantics`, no arguments (or a manifest path) | <1s | FAIL (has failures) — `rdf-semantics: 40 pass, 7 fail, 0 skip, 0 unsupported (out of 47)` (2026-09-07, up from 21 pass / 15 unsupported); F* comparison point (`bin/w3c-runner/w3c_runner.ml --rdf12entail`): 41 pass, 3 fail, 3 skip. The 15 `unsupported` are CLOSED: `xsd:double`, `xsd:float` and `rdf:JSON` now carry lexical spaces in `RDF/Datatypes.lean` and are in `modelledDatatypes`, and `Regime.literalEq`'s `.rdf` arm uses `dtValueLeq`. The 7 remaining fails and their causes are listed by test id in `docs/designissues/2026-09-07-rdf12-sparql12-semantics.md` § Status: 2 need `DInterpCond` clause 2 narrowed to the asserted object position, 1 needs the literal-typing RDF rule, 2 need an IRI reifier from the `{| |}` shorthand (the F* tree fails these identically), 1 needs a literal or triple term in subject position (term-model limit shared with F*), and 1 needs the `rdf:reifies` range step proved sound inside the `.rdfs` closure | no |
| `lean-pin-l4csvw-validate` | `formal/lean4/Harness/CsvwValidateRun.lean`, walking the real W3C CSVW VALIDATION manifest (`third_party/testing/csvw/tests/manifest-validation.jsonld`, 282 entries: 76 `csvt:PositiveValidationTest`, 61 `csvt:WarningValidationTest`, 145 `csvt:NegativeValidationTest`) against `L4Factoidal.CSVW.Validate`'s structural (raw-JSON shape) and data-level (cell format/facets, `required`, `primaryKey`, schema/CSV width) checks. Landed after the 2026-09-03 snapshot this document is otherwise dated to — the placeholder `lean_exe` this superseded was counted, trivially passing, inside the `lean-probe-*` row's "CSVW ×3" tally above; it is broken out here instead, the same way `lean-pin-l4rdf-semantics` is broken out | `lake exe l4csvw-validate`, no arguments (or a tests-dir path) | <1s | FAIL (has failures) — `csvw-validation: 266 pass, 14 fail, 2 skip (out of 282)` (positive 76/76, warning 59/61, negative 131/145); F* comparison point (`bin/csvw-runner/csvw_runner.ml --validate`): 281 pass, 1 fail (out of 282). Gap: foreign-key `schemaReference` resolution (test034/035) and cross-table referential integrity (test257/258) not yet ported, plus assorted structural/title-compatibility gaps (test094, test100, test107, test109, test111, test124, test127, test147, test148); `test308` is a documented shared limit, not a Lean-specific gap (`.github/test-suites/csvw-validation.yaml`) | no |
| `lean-probe-*` (31 zero-arg probes) | Corpus census/conformance probes declared `lean_exe` in `formal/lean4/lakefile.lean` (OWL, Turtle, RDFC-1.0, JSON-LD ×3, CSVW ×3, SPARQL syntax, VC, SHACL ×3, XML conf, XSLT, GRDDL, ShEx ×2, RIF, RML, Schematron, MathML, TOAN, JSON Schema, property-based, HDT, RDFS semi-naive) | `lake exe <name>`, no arguments | most <5s; `l4owl-probe` ~30s, `l4rdfs-semi` ~90-115s | 27 PASS, 3 FAIL (`l4owl-probe`, `l4xslt`, `l4grddl` — see Findings), 2 SKIP (`l4w3c`, `l4diff` — these take required arguments, see below). Those counts are the 2026-09-03 run of the then-30 probes and were NOT re-measured when `l4toan` was added; `l4toan` has its own row below with its own run | no |
| `lean-probe-l4toan` | The "Think of a Number" parity battery: every check of the F* consumer test `tests/unit/toan_tests.ml`, run against the Lean `Math/*` and `MathML/*` functions, plus four operator-token checks read off `MathML.Present.fst` directly | `lake exe l4toan`, no arguments | <1s | PASS — 114 pass, 0 fail (out of 114) | no |
...
| `node-tests/lws/server` | The Linked Web Storage 1.0 host: a Create/Read/Update/Delete sequence over a socket, `Last-Modified` on GET and HEAD, the PATCH blank-node refusal | `node tests/lws/server.mjs` (Node drives Deno too) | <5s | 0 pass, 0 fail, 8 skipped (out of 8) — the committed WebAssembly module does not carry `lwsOpen`/`lwsStep`/`lwsClose` yet | no |
| `node-tests/solid/server/protocol` | The Solid Protocol v0.11.0 server host, replaying the specification's own HTTP examples: storage discovery, `Allow`/`Accept-*`, containment after PUT and POST, 409 on a containment edit, 405 on root DELETE, the auxiliary lifecycle, N3 Patch, CORS, LDN inbox | `node tests/solid/server/protocol.mjs` | <5s | 0 pass, 0 fail, 12 skipped (out of 12) — the module does not carry `solidOpen`/`solidStep`/`solidClose` yet | no |
| `node-tests/solid/client/against-own-server` | The Solid client host against an in-process instance of our own server: discoverStorage, replace, read, create, delete | `node tests/solid/client/against-own-server.mjs` | <5s | 0 pass, 0 fail, 7 skipped (out of 7) — the module does not carry `solidClientRequest`/`solidClientResponse` yet | no |
| Solid interop scripts (3) | `solid-crud-tests` and `web-access-control-tests` against our server; our client against Community Solid Server; the Gherkin conformance harness (Docker) | `bash tools/solid-server-interop.sh`, `bash tools/solid-client-interop.sh`, `bash tools/solid-conformance-harness.sh` | minutes when they run | all three exit 2 ("could not run") — the module carries none of the ops; the two access-control suites additionally need an external Solid-OIDC issuer, which our server does not verify yet | no |
