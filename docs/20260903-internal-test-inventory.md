...
| `lean-pin-l4rdf-semantics` | `formal/lean4/Harness/RdfSemanticsRun.lean`, the Lean twin of the F* `--rdf12entail` runner — walks the real W3C RDF 1.2 Semantics entailment manifest (`third_party/testing/w3c/rdf/rdf12/rdf-semantics/manifest.ttl`, 47 entries: 32 `mf:PositiveEntailmentTest` + 15 `mf:NegativeEntailmentTest`) via the shared `Harness.loadManifest` / `Harness.runTest` (`Mode.rdf12`) | `lake exe l4rdf-semantics`, no arguments (or a manifest path) | <1s | FAIL (has failures) — `rdf-semantics: 19 pass, 11 fail, 0 skip, 17 unsupported (out of 47)`; F* comparison point (`bin/w3c-runner/w3c_runner.ml --rdf12entail`): 41 pass, 3 fail, 3 skip. Gap: no `RDFS-Plus` regime, no RDF 1.2 `rdf:reifies`-range closure step, `xsd:double`/`xsd:float`/`rdf:JSON` value equality unmodelled, and this manifest's `mf:entailmentRegime "simple"` fixtures need value-aware literal comparison that `RDF.Entailment`'s `.simple` case does not give them yet | no |
