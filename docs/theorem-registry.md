...
decided-corollary hypotheses; RIF Core deferred to its own stage).
...
| RIF CORE AS A DATALOG INSTANCE NOT LANDED — §4.7's `unified_adequate_rifCore` deferred to its own stage (correction note 15); the n-ary machinery it needs (arity ≠ 2 atoms) is landed and generic | — | `RIF/Translation.lean`, `RIF/Engine.lean` | ⬜ OPEN (deferred stage) | — |
...
Row-family counts: 66 plain Horn rows as `DRule`s indexed by `RlRowId`;
5 guarded / table-indexed `DRule` families (eq-ref predicate
conclusion at a non-reserved IRI, cax-adc-dw at a distinct IRI pair,
dt-type1 over `builtinDatatypeAxioms`, dt-rng-intersect over
`rangeIntersectLicenses`, xsd-axioms over datatype-position predicate x
XSD IRI x `xsdAxiomTriples`); 9 plain clash rows plus 1 clash family
(cax-adc) as falsity-headed `DNeg` sentences; 9 rows in the
interpretation-class condition `OwlRlInterpCond`. Total 91 rows: 82 as
object-language sentences, 9 as interpretation conditions.
...
| `RlNegRowId` + `rlNegRowRule` + `negCaxAdc` + `owlRlClashSchema` + `ncond_*` — 9 plain clash rows plus the cax-adc distinct-IRI-pair family | `Unified/OwlRlSchema.lean` | `OWL.RL.Clash` | ✅ PROVED (2026-08-26) | — |
| `owlRlSchema` + `OwlRlInterpCond` + `owlRlSchema_conditions` — the schema (Horn ∪ families ∪ clash) and the bridge delivering the FULL `RlConditions` / `RlClashConditions` pair, taking the nine unexpressible rows as an interpretation-class condition | `Unified/OwlRlSchema.lean` | `Unified/Theory.lean`'s `EntailsSchema` bundle slot | ✅ PROVED (2026-08-26) | `OwlRlInterpCond` (correction note 18) |
...
| SCHEMA-RELATIVE GROUND COMPLETENESS NOT PROVED — `unified_owlRl_complete_ground` over `EntailsSchema … owlRlSchema` needs `liftInterp (rlHerb c)` to satisfy every row sentence; `liftInterp` reads a predication as `r.iext p.2 x.2 y.2`, so satisfaction quantifies the PREDICATE position over the whole domain, where `RlCond*` quantifies over `WfIri`. True for `rlHerb c`, but a second pass over all 79 rows | — | design doc §4.4 | ⬜ OPEN (recorded gap, correction note 19) | — |
| NINE ROWS NOT IN THE SCHEMA — prp-spo2 and prp-key (ternary premise relation vs binary `iext`; each needs a per-list-length sentence family); cls-maxc2 / cls-maxc1 / cls-maxqc1 / cls-maxqc2 (a cardinality literal embeds as a `funapp`, not a `DTerm`); cax-dw-comp / cls-maxqc1-comp / minc1-comp (existential heads, excluded by `DRule.definiteB`). Carried by `OwlRlInterpCond`, named in the soundness statement | `Unified/OwlRlSchema.lean` module header | design doc §4.4 | ⬜ OPEN (recorded boundary, correction note 18) | — |
| HERBRAND FRAGMENT NARROWNESS — `RlHerbFrag` clause (a) (every object an IRI or blank node) excludes every graph whose closure carries a cardinality literal. Because the minc1 comprehension row emits `owl:minCardinality "1"` for each `owl:ObjectProperty` declaration, the completeness direction does not reach graphs declaring object properties | `OWL/RLHerbrand.lean` module header | design doc §4.4 | ⬜ OPEN (recorded boundary, correction note 20) | — |
...
not to the specification. `unified_adequate_bgp_bnodeFree` is the
corollary on the fragment where the two readings provably coincide.
...
| `unified_adequate_bgp_bnodeFree` — the §18.3.1-clean corollary: on a pattern with no blank node the gate is a claim about SPARQL 1.1, not only about this engine | `Unified/SparqlAdequacy.lean` | [#607](https://github.com/danbri/factoidal/issues/607) | ✅ PROVED (2026-08-26) — full iff | `bgpBnodeFree b = true` (decidable), plus the gate's own guards |
...
| `iklPremises` + `ikl_extend_entailed` + `regime_sound_ikl` — the `x-ikl-*` family is a PREMISE-LIST TRANSFORM, not a closure: `CL.IklRegime.extendDataset`'s extended default graph is entailed by the dataset's own graphs, read Skolem-wise; an engine answer over the extended graph is a unified answer from those premises | `Unified/SparqlAdequacy.lean` | `CL/IklRegime.lean` `extendDataset` | ✅ PROVED (2026-08-26) — soundness, unconditional, suffix-independent | named subsets deferred to [#581](https://github.com/danbri/factoidal/issues/581); correction note 31 |
| `mergeWhere` + `mergeWhere_entailed` + `mergeAll_entailed` — `ikl_extend_entailed` holds for EVERY selection predicate over the named graphs, the regime's `urn:cl:def:asserts` test and the merge-everything predicate alike. The landed soundness theorem therefore certifies nothing about the CHOICE of predicate, and does not see [#581](https://github.com/danbri/factoidal/issues/581)'s narrowing (a link decoration does not assert) | `Unified/ClBridge.lean` | `CL/IklRegime.lean` `extendDataset`; `ikl_extend_entailed` | ✅ PROVED (2026-08-26) — strength delimitation | — |
| `premises_entail_content` + `embedding_refutes_content` + `ikl_reading_diverges_from_dataset_embedding` — **the two renderings of a proposition inside RDF DISAGREE**: on `wDs`, the real `CL.toRdfDataset` output for `((that (Dead OBL)))`, `iklPremises` entails the asserted proposition's content triple and `datasetToTheory` does not — and does not under `PropAlphaInvariant` either. Separating model `typeBlindInterp` | `Unified/ClBridge.lean` | `CL/ToRdf.lean` translation; `Unified/DatasetEmbed.lean` `datasetToTheory` | ✅ PROVED (2026-08-26) — refutation witness, [#609](https://github.com/danbri/factoidal/issues/609) item 3 | witness pinned to the parser + translator by `#guard`; premise AND conclusion each shown satisfiable |
| `embedding_refutes_content_ikl` — the divergence survives `CL.IklRespectsThat`: coherence constrains a proposition's ZERO-ARY relation extension, and the dataset embedding puts the `that`-term in the second argument of `urn:cl:def:names`, where coherence says nothing | `Unified/ClBridge.lean` | `CL.IklRespectsThat` (`CL/Semantics.lean`) | ✅ PROVED (2026-08-26) — refutation witness | separating model `coherentBlind` |
...
| `IklAssertionCommitment` + `embed_entails_extension` — **the repair**: the `x-ikl-*` regime's own encoding commitment (`CL/IklRegime.lean`, "Encoding commitment"), stated over the decoration vocabulary alone — if `x` stands in `urn:cl:def:names` to `q` and something asserts `x` through `urn:cl:def:asserts`, then `q` holds. Under it plus IKL coherence, `datasetToTheory ds` entails the WHOLE extended default graph | `Unified/ClBridge.lean` | `CL.IklRegime.extendDataset` | ✅ PROVED (2026-08-26) — soundness, every suffix, any asserting subject | `datasetBnodeNames ds = []` (the fragment every `ToRdf` output occupies; `#guard` pins it for the witness, not proved for all texts) |
| `commitment_not_derivable`, `commitModel_respectsThat` / `_commitment`, `embed_entails_extension_nonvacuous`, `commitModel_not_everything`, `divergence_premise_satisfiable` / `_conclusion_satisfiable` — non-vacuity: the commitment FAILS in an IKL-coherent model (so it is not a consequence of coherence); the repair theorem's bundle has a model that also satisfies the witness embedding; that model refutes something | `Unified/ClBridge.lean` | — | ✅ PROVED (2026-08-26) | non-vacuity guards |
| `regime_sound_ikl` IS RELATIVE TO `iklPremises`, NOT TO THE DATASET EMBEDDING — its premise reading asserts EVERY named graph, so assertion and mention are already identified in it. `embedding_refutes_content` is the machine-checked statement that `datasetToTheory` does not license the same conclusions. Closing this means either adopting `IklAssertionCommitment` as a stated regime condition or restating regime soundness over `datasetToTheory` | `Unified/SparqlAdequacy.lean`, `Unified/ClBridge.lean` | [#609](https://github.com/danbri/factoidal/issues/609) item 3 | ⬜ OPEN (recorded divergence, correction note 33) | — |
| CL/IKL EXECUTABLE STACK STILL NOT ADEQUATE TO `CL/Semantics.lean` — [#609](https://github.com/danbri/factoidal/issues/609) items 1 and 2 are untouched: `satFin_eq` (`CL/FiniteSat.lean`'s satisfaction half, whose term-denotation half `denotTermFin_eq` / `denotSeqFin_eq` is proved) and `clifParse_adequate` (the CLIF reader against ISO 24707 Annex A). The host logic remains the one language whose own executable stack has no adequacy theorem | `CL/FiniteSat.lean`, `CL/Clif.lean` | [#609](https://github.com/danbri/factoidal/issues/609) | ⬜ OPEN (recorded gap) | — |
...
| ENGINE MATCHES PATTERN BLANK NODES AS CONSTANTS — SPARQL 1.1 §18.3.1's pattern instance mapping makes a query blank node a non-distinguished variable; `tryBindSubject` / `tryBindTerm` require label equality. Every stage 6 row outside `unified_adequate_bgp_bnodeFree` is therefore adequate to the ENGINE on blank-node-carrying patterns, not to the specification | `SPARQL/Algebra.lean` | [#607](https://github.com/danbri/factoidal/issues/607) | ⬜ OPEN (recorded divergence, correction note 32) | — |
