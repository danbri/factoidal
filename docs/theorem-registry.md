...
| RDFS entailment, rho-df completeness | `rdfs_entails d_minimal g e <==> closure-then-simple-entailment` | UNATTEMPTED — the one rung with zero completeness coverage | blocked behind index-completeness (#347/#348) | Ranked gap #1 in the coverage survey; Herbrand technique from the Simple rung is expected to reuse. |
...
| Closure-step no-repeats (Gap B) | `RDF.Entailment.RDFS.FixedPoint.fst` | ✅ PROVED (e137b5d) — UNCONDITIONAL | `lemma_rdfs_closure_step_no_repeats`; the noeq-vs-`sortWith_sorted` blocker fell to the Completeness module's eqtype-free sortedness proof. `lemma_len_eq_saturated_gapB` leaves only Gap A's `no_dup_keys` hypothesis (finisher dispatched, 4-step path recorded in-file). |
