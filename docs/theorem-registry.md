...
| `graphAsserted` + `assertedGraphAtom` + `sat_assertedGraphAtom` — **THE [#609](https://github.com/danbri/factoidal/issues/609) ITEM-3 REPAIR**: a named graph the DEFAULT graph decorates with `urn:cl:def:asserts` contributes the conjunct `atom (that (rdfBody G)) []` — CLIF's cancelling-parentheses assertion, and the position `CL.IklRespectsThat` constrains. `sat_assertedGraphAtom` is `CL.sat_assert_that` at that conjunct, so under IKL coherence it forces the graph's content exactly. (`graphAsserted_eq_assertsDecorated`, which pinned this test to a second spelling in the deleted `CL/IklRegime.lean`, is REMOVED with [#626](https://github.com/danbri/factoidal/issues/626) — there is now one spelling.) | `Unified/DatasetEmbed.lean`, `Unified/RdfEmbed.lean` | `CL.sat_assert_that` (`CL/Semantics.lean`) | ✅ PROVED (2026-08-26) | none |
...
| `disjointGoal_iff` / `propDisjointGoal_iff` / `rangeGoal_iff` / `domainGoal_iff` / `diffGoal_iff` / `sameGoal_iff` — the six negation-goal laws: an interpretation REFUTES `DisjointClasses` / `DisjointObjectProperties` / `ObjectPropertyRange` / `ObjectPropertyDomain` / `DifferentIndividuals` / identity exactly when it MODELS the goal `OWL/NegationGoals.lean` builds for it | `OWL/TableauTheorems.lean` | OWL 2 Direct Semantics Tables 5-6; `OWL/NegationGoals.lean` builders | ✅ PROVED (2026-09-07) — full iff each | `propext`, `Classical.choice`, `Quot.sound` |
| `canon_models_all` / `canon_rejects_bot` — MODEL EXISTENCE, literal fragment: a label set of literals (atom, negated atom, `owl:Thing`) with no atom clash is satisfied by its own canonical one-element interpretation, and `owl:Nothing` is in no model. Together: `Refuted.clash` and `Refuted.botClash` are exactly the obstructions to a model at the leaf. The converse direction to `refuted_sound`, restricted to the leaf | `OWL/TableauTheorems.lean` | `OWL.Refuted.clash`, `OWL.Refuted.botClash` | ✅ PROVED (2026-09-07) | NONE — axiom-free |
| GOAL-BUILDER SOUNDNESS AT THE GRAPH LEVEL NOT PROVED — the six laws above are about the FORM of the axiom and its goal. Lifting them to `OWL/NegationGoals.lean`'s RDF-graph builders needs a Direct-Semantics satisfaction relation on `Graph`, which this tree does not have | — | design record `2026-09-07-lean-owl-corpus-gap.md` § 11.3 | ⬜ OPEN (recorded obligation) | — |
| SEARCH COMPLETENESS NOT PROVED, WITH A FRAGMENT AND TWO BLOCKERS — the target is ALC ABox consistency over an acyclic TBox with role depth ≤ 3 and ≤ 6 existentials per node, the fragment where `Refute.lean`'s caps (`maxWitnessDepth = 3`, `maxGeneratedWitnesses = 6`) do not bite. Blocked on (1) the inductive lift from `canon_models_all` through ⊓ / ⊔ / ∀ / ∃, (2) `Refute.search` being a `partial def` with no induction principle, which needs a fuel-indexed reformulation and an agreement proof like `RLClosureIndexed.indexedClosure_eq` | — | design record `2026-09-07-lean-owl-corpus-gap.md` § 11.4 | ⬜ OPEN (stated theorem + plan) | — |
...
| `embed_entails_asserted_merge` + `asserted_merge_sound` — **ASSERTION-DECORATION MERGE SOUNDNESS AGAINST THE UNIFIED LAYER'S OWN DATASET READING**: `datasetToTheory ds` entails the whole merged default graph `mergeAsserted` computes, and an engine answer over that graph is a unified answer from the dataset's embedding. Condition: `CL.IklRespectsThat` alone. RESTATED 2026-08-26 ([#609](https://github.com/danbri/factoidal/issues/609) item 3) — until then both were stated in `Unified/SparqlAdequacy.lean` over `iklPremises`, a reading that asserts EVERY named graph. RENAMED and GENERALISED 2026-08-26 ([#626](https://github.com/danbri/factoidal/issues/626)): they were `ikl_extend_entailed` / `regime_sound_ikl` over the deleted `CL.IklRegime.extendDataset`, whose selection predicate also tested a content-address graph-name prefix. No proof used that prefix, so the statements are now over the assertion decoration alone — strictly more general | `Unified/ClBridge.lean` | `Unified/DatasetEmbed.lean` `graphAsserted`, `datasetToTheory` | ✅ PROVED (2026-08-26) — soundness, any asserting subject | `datasetBnodeNames ds = []` (`#guard` pins it for the witness, not proved in general) |
| `mergeWhere` + `mergeWhere_entailed` + `mergeAll_entailed` + `asserted_merge_premises_entailed` — the SUPERSEDED `iklPremises` statement holds for EVERY selection predicate over the named graphs, the `urn:cl:def:asserts` test and the merge-everything predicate alike, so it certified nothing about the CHOICE of predicate and did not see that a link decoration does not assert. This is the measurement that moved merge soundness onto `datasetToTheory` | `Unified/ClBridge.lean` | `Unified/DatasetEmbed.lean` `graphAsserted` | ✅ PROVED (2026-08-26) — strength delimitation of the superseded statement | — |
| `wDsMentioned` + `embedding_sees_the_assertion_decoration` (`embedding_entails_content` / `embedding_refutes_mentioned_content` / `premises_entail_mentioned_content`) — **the replacement statement IS predicate-sensitive**: over `datasetToTheory` the proposition's content is entailed on `wDs` and REFUTED on `wDsMentioned` — the same dataset with the assertion decoration deleted — while `iklPremises` entails it on both. `#guard`s pin the executable half (the assertion-decoration merge does not carry the mentioned graph; the merge-everything predicate does) | `Unified/ClBridge.lean` | `mergeAsserted` | ✅ PROVED (2026-08-26) — sensitivity, one refutation witness | separating model `coherentBlind` |
| `premises_entail_content` + `decorationOnly_refutes_content` + `ikl_reading_diverges_from_decoration_only_embedding` — **the two renderings of a proposition inside RDF DISAGREED, over the SUPERSEDED embedding**: on the witness dataset `wDs`, `iklPremises` entails the asserted proposition's content triple and `decorationOnlyToTheory` does not — and does not under `PropAlphaInvariant` either. Separating model `typeBlindInterp`. RESTATED 2026-08-26: these were stated about `datasetToTheory`, which the [#609](https://github.com/danbri/factoidal/issues/609) item-3 repair changed; the statements are FALSE of the repaired reading (`embedding_entails_content`) and are kept about the reading they were evidence against — the `topLevel_exclusion_insufficient_for_tt` pattern of the [#602](https://github.com/danbri/factoidal/issues/602) repair | `Unified/ClBridge.lean` | `Unified/DatasetEmbed.lean` `decorationOnlyToTheory` | ✅ PROVED (2026-08-26) — refutation witness, [#609](https://github.com/danbri/factoidal/issues/609) item 3 | `wDs` was a real `CL.toRdfDataset` output until [#626](https://github.com/danbri/factoidal/issues/626) deleted that translator; it is now built from plain IRIs in `ClBridge.lean` §2. Premise AND conclusion each shown satisfiable |
...
| `embed_asserts_decorated_graphs` + `embedding_entails_content` + `ikl_reading_agrees_with_dataset_embedding` — **the repair, as a DERIVATION**: under `CL.IklRespectsThat` ALONE, an interpretation satisfying `datasetToTheory ds` satisfies the content of every named graph the default graph decorates with `urn:cl:def:asserts`. The encoding commitment is therefore a theorem, not a condition. REMOVED with this landing: `IklAssertionCommitment` (the adopted condition) and `commitment_not_derivable` (its non-derivability), both of which described the superseded embedding | `Unified/ClBridge.lean` | `Unified/DatasetEmbed.lean` `graphAsserted`; `CL.sat_assert_that` | ✅ PROVED (2026-08-26) — derivation, replaces an adopted condition | `datasetBnodeNames ds = []` (`#guard` pins it for the witness, not proved in general) |
| `objModel` / `objModel_respectsThat` / `embed_entails_asserted_merge_nonvacuous` / `objModel_not_everything`, `decorationOnly_strictly_weaker`, `divergence_premise_satisfiable` / `_conclusion_satisfiable` — non-vacuity: `objModel` is IKL-coherent, satisfies the REPAIRED embedding of the witness dataset, and refutes a sentence; `coherentBlind` satisfies the superseded reading of the same dataset and refutes the repaired one, so the refinement `datasetToTheory_entails_decorationOnly` is proper | `Unified/ClBridge.lean` | — | ✅ PROVED (2026-08-26) | non-vacuity guards |
| Merge soundness RELATIVE TO `iklPremises` — CLOSED 2026-08-26 by changing the embedding, not the condition. It is now stated over `[datasetToTheory ds]` under `CL.IklRespectsThat` and is sensitive to the `urn:cl:def:asserts` test. What is NOT claimed: anything outside `datasetBnodeNames ds = []`, and completeness in either direction | `Unified/ClBridge.lean` | [#609](https://github.com/danbri/factoidal/issues/609) item 3 | ✅ CLOSED (2026-08-26, correction note 33) | fragment guard above |
...
## IKL normalization (Hayes 2009) — Lean tree

Issue [#580](https://github.com/danbri/factoidal/issues/580), landed
2026-08-26. Source: Pat Hayes, *A Satisfiability-Preserving Reduction
of IKL to Common Logic*, IHMC 2009,
<https://jfsowa.com/ikl/PHnsfs09.pdf>. Design note:
[`docs/designissues/2026-08-26-ikl-normalization.md`](designissues/2026-08-26-ikl-normalization.md).
Axiom audit on every row below: `propext`, `Classical.choice`,
`Quot.sound` only (in-source `#print axioms`, 18 declarations).

| Theorem | Module | Native anchor | Status | Fragment / hypotheses |
|---|---|---|---|---|
| `normTerm` / `normSeq` / `normBinds` / `normSent` / `normSents` — the transformation. STRUCTURALLY RECURSIVE: every recursive call is on a strict subterm (the body of a `that` is a subsentence), so termination is kernel-checked with no `partial`, no `termination_by` and no `decreasing_by`. The paper gives the algorithm as a loop and says only that "an obvious inductive argument shows that this process of normalization must terminate with a CL text" | `CL/Normalize.lean` | Hayes 2009 pseudocode, section "Multiple proposition names" | ✅ DEFINED (2026-08-26) — total | — |
| `thatWeight_that` — the measure the LOOP form needs, and why the obvious one fails. Counting proposition names does not decrease: eliminating an outermost `that` moves the names nested inside it into a tail. `Sentence.thatWeight` weights each occurrence by two raised to its `that`-nesting depth (`w (that S) = 1 + 2 * w S`), and a loop step drops the text's weight by at least `1 + w S` | `CL/Normalize.lean` | Hayes 2009, "an obvious inductive argument" | ✅ PROVED (2026-08-26) — strict drop per step | stated about the loop step; the shipped traversal does not use it |
| `normalizeFrom_pureCL` — the OTHER half of "terminates with a CL text": head and every tail satisfy `Sentence.isPureCL`, so no proposition name survives anywhere in the output. Two mutual inductions over the traversal | `CL/Normalize.lean` | `Sentence.isPureCL` (`CL/Syntax.lean`) | ✅ PROVED (2026-08-26) — every input, unconditional | NONE — holds outside the no-intrusion fragment too |
| The four paradox pins — That Nothing Is True, the Liar, Kripke's semantic paradox and the Knower, each matching the paper's printed output verbatim with the counter started where the paper's `gensym` had reached. The Knower's ITERATION COUNT is pinned at 2 (`clifThatCount == some 2`, two tail sentences), the paper's only example needing more than one iteration. Kripke's pin is what fixes intrusion as enclosing-bound INTERSECT free-in-body | `CL/Normalize.lean` | Hayes 2009, "Normalizing the paradoxes" | ✅ PINNED (2026-08-26) — `#guard`, so `lake build` is the test run | — |
| `pSat` + `iklProp` + `sat_iklProp` + `iklProp_respects` + `exists_ikl_interp` — **a coherent IKL interpretation, in `CL/`.** `IklRespectsThat` had no model behind it in the `CL` tree, so every theorem quantified over coherent interpretations was empty. `pSat` breaks the circularity between `CL.Sat` and `Interp.iProp` by resolving `that` through recursion into the body. Universe `Prop`, `rel x args := x`, `fn x args := x` | `CL/IklModels.lean` | `CL.IklRespectsThat` (`CL/Semantics.lean`) | ✅ PROVED (2026-08-26) — satisfiability witness | none. DUPLICATES `Unified/ClBridge.lean`'s `propModel` (same day, more general in `iN iS R F`); the dependency runs Unified → CL so the copy could not be reused — resolution recorded in the design note §8 |
| `IklPropLocal` + `pSat_coincide` (with `pDenot_coincide` / `pSatAll_coincide` / `pSatAny_coincide` / `pSatFA_coincide` / `pSatEX_coincide`) + `iklProp_local` — the LOCALITY condition: the proposition a sentence expresses depends only on what its free names and free sequence markers denote. Hayes's construction turns on it, because the fresh name gets ONE denotation while the occurrence it replaces sits under quantifiers that change the valuation at names the body does not use | `CL/IklModels.lean` | `Interp.iProp` (`CL/Semantics.lean`) | ✅ PROVED (2026-08-26) — condition stated, and proved for `iklProp` | — |
| `normT_head` / `normSeq_head` / `normS_head` / `normSs_head` / `normSsAny_head` / `normFA_head` / `normEX_head` — the head clause: the transformed sentence, read under the constructed valuation, has the truth value the original has under the original valuation. Uses LOCALITY only, not coherence | `CL/NormalizeSemantics.lean` | `CL.Sat` (`CL/Semantics.lean`) | ✅ PROVED (2026-08-26) — full iff | no-intrusion fragment; no name from the fresh-name space; the allocation record known to the valuation |
| `normT_tails` / `normSeq_tails` / `normBinds_tails` / `normS_tails` / `normSs_tails` — the tail clause: every emitted tail sentence holds under the constructed valuation. Uses COHERENCE, and NO premise that the original sentence is satisfied — which is what makes the conjecture below follow | `CL/NormalizeSemantics.lean` | `CL.IklRespectsThat` (`CL/Semantics.lean`) | ✅ PROVED (2026-08-26) | same three side conditions |
| `normalize_preserves` + `ikl_sat_to_cl_sat` — Hayes's claim, forward half: "any IKL interpretation I of PHI defines a CL interpretation J of S(PHI) which satisfies the tail ... such that in every case, J assigns the same truthvalue to the head as I does to PHI". `kapOf` is J: the original name valuation with each allocated fresh name pointed at the proposition its `that`-body denotes | `CL/NormalizeSemantics.lean` | Hayes 2009, "Eliminating Proposition names" | ✅ PROVED (2026-08-26) — forward direction, full iff on the head | three side conditions, all decidable |
| `tails_satisfiable` — **HAYES'S 2009 OPEN CONJECTURE, for the no-intrusion fragment.** The paper: "We have not found any such example, and believe that it is impossible, but do not at the time of writing have a conclusive proof." The tail set of a normalized IKL sentence is ALWAYS CL-satisfiable, whether or not the sentence is. The proof is that the tail induction never uses the sentence's truth, so a model of the tails is built from any coherent local IKL interpretation, and `iklProp` is one | `CL/NormalizeSemantics.lean` | Hayes 2009, "IKL Satisfiability and Entailment" | ✅ PROVED (2026-08-26) — no-intrusion fragment | three side conditions; NOT proved for the intrusion case (design note §7.1) |
| `tnit_cl_unsat` / `liar_cl_unsat` / `kripke_cl_unsat` + `tnit_not_ikl_sat` / `liar_not_ikl_sat` / `kripke_not_ikl_sat` — three of the four normalized paradoxes are CL-UNSATISFIABLE, so by `ikl_sat_to_cl_sat` none of the three sentences has a coherent IKL model | `CL/NormalizeSemantics.lean` | Hayes 2009, "Normalizing the paradoxes" | ✅ PROVED (2026-08-26) — refutations | concrete sentences; side conditions by `decide` |
| `knower_cl_sat` + `knower_two_iterations` + `tnit_tails_sat` / `liar_tails_sat` / `kripke_tails_sat` / `knower_tails_sat` — non-vacuity. The Knower's fourth sentence taken alone IS satisfiable and a model is exhibited; and against each of the three refutations sits a model of the TAIL SET alone. For the Liar the whole text is inconsistent while its tail set is not, so the conjecture is not being confirmed by a theorem that would confirm anything | `CL/NormalizeSemantics.lean` | `CL/IklModels.lean` `iklProp` | ✅ PROVED (2026-08-26) — non-vacuity guards | — |
| THE INTRUSION CASE IS NOT COVERED — with a non-empty intrusion list the replacement term is `(K U1 ... Un)` and the model must give `K` a non-trivial FUNCTIONAL extension, one proposition per argument tuple. `iklProp`'s `Prop` universe collapses them (`fn x args := x`). A universe with a surjection onto `List D -> Prop` is impossible by cardinality, so this needs a universe built for the finitely many extensions the tails require. Both inductions already carry the intrusion lists | `CL/NormalizeSemantics.lean` module header | design note §7.1 | ⬜ OPEN (recorded boundary with its obstruction) | — |
| THE CONVERSE DIRECTION IS NOT PROVED — from a CL model of head-plus-tails back to an IKL model. Two obstructions: coherence forces an individual with empty zero-ary extension and one without, which a CL model need not have; and a `that`-term can sit in an EQUATION (the Liar does), so the converse `iProp` must return the individual the fresh name denotes, not one with the same truth value — it has to be keyed by the sentence | `CL/NormalizeSemantics.lean` module header | design note §7.2 | ⬜ OPEN (recorded boundary with its obstruction) | — |
| `propName` INJECTIVITY NOT PROVED — key-distinctness of the allocation list is what needs it, and it reduces to injectivity of `Nat.repr`, absent from this toolchain's core library (`exact?` finds nothing for `toString a = toString b → a = b` nor for `(toString a).toNat? = some a`). It stays a hypothesis of the general theorems; the four paradox instances discharge it by `decide` and carry no such hypothesis | `CL/NormalizeSemantics.lean` module header | design note §7.3 | ⬜ OPEN (recorded gap) | — |
| NO SKOLEMIZATION — the paper Skolemizes first so that every intrusion is universal, and re-Skolemizes each tail. Not done here, and not needed: the tail holds in the constructed model whichever quantifier bound the intruding name, because the functional extension given to the fresh name is TOTAL. Visible on the paper's second quantifier-intrusion example, where Hayes prints `(prop1 x)` after Skolemizing and this tree prints `(prop1 x y)`; both are pinned | `CL/Normalize.lean` module header | Hayes 2009, "Quantifier intrusions" | ✅ RECORDED (2026-08-26) — divergence, both forms pinned | — |
...
| `evalPatternBackend_restrict` — the pattern induction. Covers `BIND(e AS ?v)` for an `Expr.backendLocal` `e` since 2026-09-06 (second landing), through the backend-native `.bind` arm added to `evalPatternBackend` for that admission | `SPARQL/DatasetRestriction.lean` | `StoreDataset.evalPatternBackend` | ✅ PROVED (2026-09-06) — list equality | `plannerFragment`, `PatternKept`, graph names readable, `GRAPH ?v` only when no graph-name selection was made |
| `evalPatternBackend_nil` — a `graphFree` fragment pattern over the empty active graph answers nothing, or `keep` holds of every triple. The side condition the two empty leaves and the `BIND` arm need | `SPARQL/DatasetRestriction.lean` | `StoreDataset.evalPatternBackend` | ✅ PROVED (2026-09-06) | `plannerFragment`, `graphFree`, `PatternKept` |
| `PatternKept_mono`, `patternKept_of_quadPredicates`, `patternKept_of_queryPredicates` — the predicate collector establishes the evaluator theorem's `PatternKept` hypothesis, `BIND` arm included | `Storage/PlannerSoundness.lean` | `ShardManifest.quadNativeConstantPredicates?` | ✅ PROVED (2026-09-06) | collector returns `some P` |
