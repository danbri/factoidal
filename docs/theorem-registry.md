...
landing): 67 plain Horn rows as `DRule`s indexed by `RlRowId`;
...
XSD IRI x `xsdAxiomTriples`); 12 plain clash rows plus 1 clash family
(cax-adc) as falsity-headed `DNeg` sentences; 5 rows in the
interpretation-class condition `OwlRlInterpCond`. Total 91 rows: 86 as
object-language sentences, 5 as interpretation conditions. (Before
2026-08-26: 66 / 9 / 9, i.e. 82 and 9.)
...
| FIVE ROWS NOT IN THE SCHEMA (was nine before 2026-08-26) — **prp-spo2, prp-key**: a collection premise, needing a per-list-length sentence family over generated names `l0..ln` / `q0..q(n-1)` / `v0..v(n-1)`. A ternary reserved helper predicate WOULD avoid the family (`DAtom` is n-ary and the helper need never appear in `restrictInterp i` — the earlier "a helper predicate cannot name it" was wrong), but `liftInterp` reads `rel p args` as `False` at every arity other than 2, so a ternary head is false at every model schema-relative completeness needs. **cax-dw-comp, cls-maxqc1-comp, minc1-comp**: existential heads, excluded by `DRule.definiteB`; costed and DECIDED against below. Carried by `OwlRlInterpCond`, named in the soundness statement | `Unified/OwlRlSchema.lean` module header | design doc §4.4 | ⬜ OPEN (recorded boundary, correction notes 18, 21) | — |
