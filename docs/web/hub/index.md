...
<ul class="post-series">
  ...
  <li><a href="./36-lean-in-the-browser/">Lean 4 in the browser: a second engine on the page</a> — the <a href="https://github.com/danbri/factoidal/issues/466">Lean 4 port</a> compiled Lean → C → wasm32 (Lean's runtime and core library rebuilt for the target, GMP-free, one 1.5 MB module for browser + Node + Deno) running <code>evalBgp</code> beside the F*-derived engine on the same four triples, and agreeing — with an exact statement of what the Lean side cannot do yet (no SPARQL parser, no Turtle reader, BGP only) (none)</li>
</ul>
