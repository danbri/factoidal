...
<ul class="post-series">
  ...
  <li><a href="./39-propositions-as-first-class-citizens/">Propositions as first-class citizens</a> — Common Logic and <a href="https://www.ihmc.us/users/phayes/IKL/GUIDE/GUIDE.html">IKL</a> read live by the Lean engine: CLIF sentences parsed, the IKL <code>(that S)</code> proposition term translated to an RDF named graph, SPARQL <code>GRAPH</code> patterns reading inside a proposition, and the finale joining an IKL knowledge base against ordinary RDF data through <code>SERVICE &lt;urn:ikl:kb&gt;</code> in one query (none)</li>
  ...
  <li><a href="./41-a-walkthrough-of-the-ikl-guide/">A walkthrough of the IKL GUIDE</a> — worked examples straight from Pat Hayes and Chris Menzel's <a href="https://www.ihmc.us/users/phayes/IKL/GUIDE/GUIDE.html">IKL GUIDE</a>, section by section: quantifier forms, proposition names and referential transparency, quantifying-in, <code>ist</code> contexts as GRAPH queries, alpha-variant sentences landing in one content-addressed proposition graph, the stated <code>=p</code> boundary, and a KB-plus-RDF SERVICE join — with the reader's fragment limits stated where a GUIDE example goes beyond them (none)</li>
