...
The next correct implementation step is therefore an explicit split:
...
The physical library must have no top-level `main`; the two front ends can
then share the exact scan implementation rather than duplicating file-range
logic. This is a packaging boundary only: it does not weaken the existing
pure byte-decoding or SPARQL-evaluation contracts.
