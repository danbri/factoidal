(* entry_extras — every capability outside the lite bundle's surface:
   RDF/XML + JSON-LD parsing, RIF Core, XML well-formedness + XPath,
   SHACL, ShEx, RDFS/OWL-RL/rho-df/RDFS-Plus closures, the OWL tableau
   reasoners, RML, CSVW, the durable-UPDATE browser persistence
   (delta-log) ops, the in-memory COTTAS bytes store, VC Data
   Integrity crypto, did:key resolution, and the typed-engine wrappers
   (XSLT, MathML, XForms, JSON Schema, Schematron, TOAN, matrix
   algebra, sigmoid).

   This is a CONSUMER (rule #11): hand-written OCaml glue exposing
   F*-extracted engine functions over a string/JSON ABI. No RDF/SPARQL/
   OWL/etc. semantics live here — every semantic operation delegates to
   an F*-extracted module (named in each section's own comment below,
   matching entry_jsoo.ml's full ABI header).

   Only linked into the FULL bundle (entry_core.ml + entry_extras.ml +
   entry_jsoo.ml): the lite bundle (entry_core.ml + entry_lite_jsoo.ml)
   never references this module, so the OCaml linker (no -linkall)
   never pulls its modules' compilation units into npm_entry_lite.byte.
   The one bridge back into entry_core.ml is `Entry_core.extra_parsers`,
   installed below, which gives parse_document (Entry_core) access to
   RDF/XML and JSON-LD parsing when this module IS linked. *)
...
(* Install RDF/XML and JSON-LD parsing behind Entry_core's
   extra_parsers hook (https://github.com/danbri/factoidal/issues/684):
   dispatch only (rule #11) -- entry_core.ml's parse_document already
   decided which of the two formats applies before calling this, so
   `canonical_tag` is always exactly "rdfxml" or "jsonld". *)
let parse_extra (text : string) (canonical_tag : string) (base_iri : string)
    : (rdf_dataset, string) result =
  let base = if base_iri = "" then None else Some base_iri in
  match canonical_tag with
  | "rdfxml" ->
    let triples =
      match base with
  ...
  | "jsonld" ->
    ...
    (match Parser_JSONLD.parse_jsonld text fs_base
     ...
     | FStar_Pervasives_Native.Some ds -> Ok ds
     | FStar_Pervasives_Native.None ->
       Error ("invalid or unsupported JSON-LD (parse error, or a " ^
              "feature needing a remote-context loader this entry " ^
              "does not have)"))
  | _ -> Error (Printf.sprintf "parse_extra: unrecognized format tag '%s'" canonical_tag)

let () = Entry_core.extra_parsers := Some parse_extra
...
let rif_result_json
  ...
  "{\"ok\":true"
  ^ ",\"inputNquads\":" ^ jstr (construct_triples_to_ntriples input)
  ^ ",\"saturatedNquads\":" ^ jstr (construct_triples_to_ntriples saturated)
